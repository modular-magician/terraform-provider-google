...
func BootstrapBigLakeIcebergNamespace(t *testing.T, catalogId, namespaceId string) string {
	project := envvar.GetTestProjectFromEnv()
	config := BootstrapConfig(t)
	if config == nil {
		return ""
	...
	log.Printf("[DEBUG] Creating BigLake Iceberg namespace %q in catalog %q", namespaceId, catalogId)

	catalogPath := catalogId
	if !strings.HasPrefix(catalogId, "projects/") {
		catalogPath = fmt.Sprintf("projects/%s/catalogs/%s", project, catalogId)
	}

	url := fmt.Sprintf("%siceberg/v1/restcatalog/v1/%s/namespaces", config.BiglakeIcebergBasePath, catalogPath)
	...
	if err != nil {
		if !transport_tpg.IsGoogleApiErrorWithCode(err, 409) {
			t.Fatalf("Error bootstrapping BigLake Iceberg namespace %q: %s", namespaceId, err)
		}
		log.Printf("[DEBUG] BigLake Iceberg namespace %q already exists", namespaceId)
	}

	return namespaceId
}

func CleanupBigLakeIcebergNamespace(t *testing.T, catalogId, namespaceId string) {
	project := envvar.GetTestProjectFromEnv()
	config := BootstrapConfig(t)
	if config == nil {
		return
	...
	log.Printf("[DEBUG] Deleting BigLake Iceberg namespace %q in catalog %q", namespaceId, catalogId)
	...
	url := fmt.Sprintf("%siceberg/v1/restcatalog/v1/%s/namespaces/%s", config.BiglakeIcebergBasePath, catalogPath, namespaceId)
	...
	if err != nil && !transport_tpg.IsGoogleApiErrorWithCode(err, 404) {
		log.Printf("[ERROR] Error cleaning up BigLake Iceberg namespace %q: %s", namespaceId, err)
	}
}

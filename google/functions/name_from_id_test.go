...
func TestAccProviderFunction_name_from_id(t *testing.T) {
	...
	resourceName := fmt.Sprintf("tf-test-my-resource-%s", acctest.RandString(t, 10))
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				// user's usecase
				Config: testProviderFunction_generic_element_from_id(context, truncatedValidSelfLink),
				Check: resource.ComposeTestCheckFunc(

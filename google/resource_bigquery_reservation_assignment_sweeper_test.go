...
func testSweepBigqueryReservationAssignment(region string) error {
	...
	client := NewDCLBigqueryReservationClient(config, config.userAgent, "")
	err = client.DeleteAllAssignment(context.Background(), d["project"], d["location"], d["reservation"], isDeletableBigqueryReservationAssignment)
	if err != nil {
		return err
	}
	return nil

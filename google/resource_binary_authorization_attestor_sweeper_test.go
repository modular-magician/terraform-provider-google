...
func testSweepBinaryAuthorizationAttestor(region string) error {
	resource_name := "BinaryAuthorizationAttestor"
	...
	list_template := strings.Split("https://binaryauthorization.googleapis.com/v1/projects/{{project}}/attestors?attestorId={{name}}", "?")[0]
	...
	resource_list, ok := res["attestors"]
	if !ok {
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://binaryauthorization.googleapis.com/v1/projects/{{project}}/attestors/{{name}}"

...
func resourceComputeAddressCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return err
	} else if v, ok := d.GetOkExists("address"); ok || !reflect.DeepEqual(v, addressProp) {
		obj["address"] = addressProp
	}
	addressTypeProp, err := expandComputeAddressAddressType(d.Get("address_type"), d, config)
	if err != nil {
		return err
	} else if v, ok := d.GetOkExists("address_type"); ok || !reflect.DeepEqual(v, addressTypeProp) {
	...
	if err != nil {
		return err
	} else if v, ok := d.GetOkExists("network_tier"); ok || !reflect.DeepEqual(v, networkTierProp) {

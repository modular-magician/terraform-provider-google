...
func init() {
	resource.AddTestSweepers("ComputeBackendBucketSignedUrlKey", &resource.Sweeper{
		Name: "ComputeBackendBucketSignedUrlKey",
		F:    testSweepComputeBackendBucketSignedUrlKey,
	})
}

func testSweepComputeBackendBucketSignedUrlKey(region string) error {
	resource_name := "ComputeBackendBucketSignedUrlKey"
	...
	list_template := strings.Split("https://www.googleapis.com/compute/v1/projects/{{project}}/global/backendBuckets/{{backend_bucket}}", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://www.googleapis.com/compute/v1/projects/{{project}}/global/backendBuckets/{{backend_bucket}}/deleteSignedUrlKey?keyName={{name}}"

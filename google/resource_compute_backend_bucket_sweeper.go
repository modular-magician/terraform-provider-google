...
func init() {
	resource.AddTestSweepers("ComputeBackendBucket", &resource.Sweeper{
		Name: "ComputeBackendBucket",
		F:    testSweepComputeBackendBucket,
	})
}

func testSweepComputeBackendBucket(region string) error {
	resource_name := "ComputeBackendBucket"
	...
	list_template := strings.Split("https://www.googleapis.com/compute/v1/projects/{{project}}/global/backendBuckets", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://www.googleapis.com/compute/v1/projects/{{project}}/global/backendBuckets/{{name}}"

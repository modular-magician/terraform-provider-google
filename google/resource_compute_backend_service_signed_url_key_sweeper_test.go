...
func init() {
	resource.AddTestSweepers("ComputeBackendServiceSignedUrlKey", &resource.Sweeper{
		Name: "ComputeBackendServiceSignedUrlKey",
		F:    testSweepComputeBackendServiceSignedUrlKey,
	})
}

func testSweepComputeBackendServiceSignedUrlKey(region string) error {
	resource_name := "ComputeBackendServiceSignedUrlKey"
	...
	list_template := strings.Split("https://www.googleapis.com/compute/v1/projects/{{project}}/global/backendServices/{{backend_service}}", "?")[0]
	...
	resource_list, ok := res["cdnPolicy"]
	if !ok {
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://www.googleapis.com/compute/v1/projects/{{project}}/global/backendServices/{{backend_service}}/deleteSignedUrlKey?keyName={{name}}"

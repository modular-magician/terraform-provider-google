...
func resourceComputeForwardingRuleCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return err
	} else if v := d.Get("ip_address"); !isEmptyValue(reflect.ValueOf(IPAddressProp)) {
		obj["IPAddress"] = IPAddressProp
	}
	IPProtocolProp, err := expandComputeForwardingRuleIPProtocol(d.Get("ip_protocol"), d, config)
	if err != nil {
		return err
	} else if v := d.Get("ip_protocol"); !isEmptyValue(reflect.ValueOf(IPProtocolProp)) {
	...
	if err != nil {
		return err
	} else if v := d.Get("backend_service"); !isEmptyValue(reflect.ValueOf(backendServiceProp)) {
	...
	if err != nil {
		return err
	} else if v := d.Get("load_balancing_scheme"); !isEmptyValue(reflect.ValueOf(loadBalancingSchemeProp)) {
...
func resourceComputeForwardingRuleImport(d *schema.ResourceData, meta interface{}) ([]*schema.ResourceData, error) {
	config := meta.(*Config)
	if err := parseImportId([]string{"projects/(?P<project>[^/]+)/regions/(?P<region>[^/]+)/forwardingRules/(?P<name>[^/]+)", "(?P<project>[^/]+)/(?P<region>[^/]+)/(?P<name>[^/]+)", "(?P<name>[^/]+)"}, d, config); err != nil {
		return nil, err
	}

...
func TestAccComputeGlobalForwardingRule_updateTarget(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccComputeGlobalForwardingRule_httpProxyUpdated(fr, "proxy2", proxy, proxyUpdated, backend, hc, urlmap),
				Check: resource.ComposeTestCheckFunc(
					resource.TestMatchResourceAttr(
...
func testAccComputeGlobalForwardingRule_httpProxy(fr, setProxyTfName, proxy, proxy2, backend, hc, urlmap string) string {
	return fmt.Sprintf(`
	...
	}`, fr, proxyTfName, proxy, proxy2, backend, hc, urlmap)
}

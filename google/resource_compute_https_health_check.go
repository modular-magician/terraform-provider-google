...
func resourceComputeHttpsHealthCheckCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return err
	} else if v, ok := d.GetOkExists("check_interval_sec"); ok || !reflect.DeepEqual(v, checkIntervalSecProp) {
	...
	if err != nil {
		return err
	} else if v, ok := d.GetOkExists("healthy_threshold"); ok || !reflect.DeepEqual(v, healthyThresholdProp) {
		obj["healthyThreshold"] = healthyThresholdProp
	}
	hostProp, err := expandComputeHttpsHealthCheckHost(d.Get("host"), d, config)
	if err != nil {
		return err
	} else if v, ok := d.GetOkExists("host"); ok || !reflect.DeepEqual(v, hostProp) {
		obj["host"] = hostProp
	}

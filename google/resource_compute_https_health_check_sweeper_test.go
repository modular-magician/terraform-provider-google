...
func testSweepComputeHttpsHealthCheck(region string) error {
	resource_name := "ComputeHttpsHealthCheck"
	...
	list_template := strings.Split("https://www.googleapis.com/compute/v1/projects/{{project}}/global/httpsHealthChecks", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://www.googleapis.com/compute/v1/projects/{{project}}/global/httpsHealthChecks/{{name}}"

	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"routing_config": {
				...
				Description: ``,
				MaxItems:    1,
				Elem:        ComputeNetworkRoutingConfigSchema(),
			},
...
func ComputeNetworkRoutingConfigSchema() *schema.Resource {
	return &schema.Resource{
		Schema: map[string]*schema.Schema{
			"routing_mode": {
				Type:        schema.TypeString,
...
func resourceComputeNetworkCreate(d *schema.ResourceData, meta interface{}) error {
	...
	obj := &compute.Network{
		...
		RoutingConfig:         expandComputeNetworkRoutingConfig(d.Get("routing_config")),
	}
...
func resourceComputeNetworkRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err = d.Set("routing_config", flattenComputeNetworkRoutingConfig(res.RoutingConfig)); err != nil {
...
func expandComputeNetworkRoutingConfig(o interface{}) *compute.NetworkRoutingConfig {
	if o == nil {
		return compute.EmptyNetworkRoutingConfig
	}
	objArr := o.([]interface{})
	if len(objArr) == 0 {
		return compute.EmptyNetworkRoutingConfig
	...
	return &compute.NetworkRoutingConfig{
		RoutingMode: compute.NetworkRoutingConfigRoutingModeEnumRef(obj["routing_mode"].(string)),
	}
}

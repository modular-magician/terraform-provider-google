...
func init() {
	resource.AddTestSweepers("ComputeRegionAutoscaler", &resource.Sweeper{
		Name: "ComputeRegionAutoscaler",
		F:    testSweepComputeRegionAutoscaler,
	})
}

func testSweepComputeRegionAutoscaler(region string) error {
	resource_name := "ComputeRegionAutoscaler"
	...
	list_template := strings.Split("https://www.googleapis.com/compute/v1/projects/{{project}}/regions/{{region}}/autoscalers", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://www.googleapis.com/compute/v1/projects/{{project}}/regions/{{region}}/autoscalers/{{name}}"

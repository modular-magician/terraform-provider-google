...
func testSweepComputeRegionHealthCheck(region string) error {
	resource_name := "ComputeRegionHealthCheck"
	...
	list_template := strings.Split("https://www.googleapis.com/compute/v1/projects/{{project}}/regions/{{region}}/healthChecks", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://www.googleapis.com/compute/v1/projects/{{project}}/regions/{{region}}/healthChecks/{{name}}"

...
func testSweepComputeResourcePolicy(region string) error {
	resource_name := "ComputeResourcePolicy"
	...
	list_template := strings.Split("https://www.googleapis.com/compute/v1/projects/{{project}}/regions/{{region}}/resourcePolicies", "?")[0]
	...
	resource_list, ok := res["resourcePolicies"]
	if !ok {
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://www.googleapis.com/compute/v1/projects/{{project}}/regions/{{region}}/resourcePolicies/{{name}}"

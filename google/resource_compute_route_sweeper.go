...
func init() {
	resource.AddTestSweepers("ComputeRoute", &resource.Sweeper{
		Name: "ComputeRoute",
		F:    testSweepComputeRoute,
	})
}

func testSweepComputeRoute(region string) error {
	resource_name := "ComputeRoute"
	...
	list_template := strings.Split("https://www.googleapis.com/compute/v1/projects/{{project}}/global/routes", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://www.googleapis.com/compute/v1/projects/{{project}}/global/routes/{{name}}"

...
func init() {
	resource.AddTestSweepers("ComputeRouter", &resource.Sweeper{
		Name: "ComputeRouter",
		F:    testSweepComputeRouter,
	})
}

func testSweepComputeRouter(region string) error {
	resource_name := "ComputeRouter"
	...
	list_template := strings.Split("https://www.googleapis.com/compute/v1/projects/{{project}}/regions/{{region}}/routers", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://www.googleapis.com/compute/v1/projects/{{project}}/regions/{{region}}/routers/{{name}}"

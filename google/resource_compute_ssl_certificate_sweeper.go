...
func init() {
	resource.AddTestSweepers("ComputeSslCertificate", &resource.Sweeper{
		Name: "ComputeSslCertificate",
		F:    testSweepComputeSslCertificate,
	})
}

func testSweepComputeSslCertificate(region string) error {
	resource_name := "ComputeSslCertificate"
	...
	list_template := strings.Split("https://www.googleapis.com/compute/v1/projects/{{project}}/global/sslCertificates", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://www.googleapis.com/compute/v1/projects/{{project}}/global/sslCertificates/{{name}}"

...
func resourceComputeSslPolicyCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return err
	} else if v := d.Get("profile"); !isEmptyValue(reflect.ValueOf(profileProp)) {
		obj["profile"] = profileProp
	}
	minTlsVersionProp, err := expandComputeSslPolicyMinTlsVersion(d.Get("min_tls_version"), d, config)
	if err != nil {
		return err
	} else if v := d.Get("min_tls_version"); !isEmptyValue(reflect.ValueOf(minTlsVersionProp)) {
	...
	if err != nil {
		return err
	} else if v := d.Get("custom_features"); !isEmptyValue(reflect.ValueOf(customFeaturesProp)) {

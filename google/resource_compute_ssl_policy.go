...
func resourceComputeSslPolicyCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return err
	} else if v, ok := d.GetOkExists("profile"); ok || !reflect.DeepEqual(v, profileProp) {
		obj["profile"] = profileProp
	}
	minTlsVersionProp, err := expandComputeSslPolicyMinTlsVersion(d.Get("min_tls_version"), d, config)
	if err != nil {
		return err
	} else if v, ok := d.GetOkExists("min_tls_version"); ok || !reflect.DeepEqual(v, minTlsVersionProp) {

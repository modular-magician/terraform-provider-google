...
func init() {
	resource.AddTestSweepers("ComputeSslPolicy", &resource.Sweeper{
		Name: "ComputeSslPolicy",
		F:    testSweepComputeSslPolicy,
	})
}

func testSweepComputeSslPolicy(region string) error {
	resource_name := "ComputeSslPolicy"
	...
	list_template := strings.Split("https://www.googleapis.com/compute/v1/projects/{{project}}/global/sslPolicies", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://www.googleapis.com/compute/v1/projects/{{project}}/global/sslPolicies/{{name}}"

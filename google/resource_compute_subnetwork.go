...
func resourceComputeSubnetworkUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("ip_cidr_range") {
		...
		if err != nil {
			return err
		} else if v, ok := d.GetOkExists("ip_cidr_range"); ok || !reflect.DeepEqual(v, ipCidrRangeProp) || d.HasChange("ip_cidr_range") {
			obj["ipCidrRange"] = ipCidrRangeProp
		}
	...
	if d.HasChange("enable_flow_logs") || d.HasChange("fingerprint") || d.HasChange("secondary_ip_range") {
		...
		if err != nil {
			return err
		} else if v, ok := d.GetOkExists("enable_flow_logs"); ok || !reflect.DeepEqual(v, enableFlowLogsProp) || d.HasChange("enable_flow_logs") {
			obj["enableFlowLogs"] = enableFlowLogsProp
		}
		...
		if err != nil {
			return err
		} else if v, ok := d.GetOkExists("secondary_ip_range"); ok || !reflect.DeepEqual(v, secondaryIpRangesProp) || d.HasChange("secondary_ip_range") {
			obj["secondaryIpRanges"] = secondaryIpRangesProp
		}
	...
	if d.HasChange("private_ip_google_access") {
		...
		if err != nil {
			return err
		} else if v, ok := d.GetOkExists("private_ip_google_access"); ok || !reflect.DeepEqual(v, privateIpGoogleAccessProp) || d.HasChange("private_ip_google_access") {
			obj["privateIpGoogleAccess"] = privateIpGoogleAccessProp
		}

...
func init() {
	resource.AddTestSweepers("ComputeSubnetwork", &resource.Sweeper{
		Name: "ComputeSubnetwork",
		F:    testSweepComputeSubnetwork,
	})
}

func testSweepComputeSubnetwork(region string) error {
	resource_name := "ComputeSubnetwork"
	...
	list_template := strings.Split("https://www.googleapis.com/compute/v1/projects/{{project}}/regions/{{region}}/subnetworks", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://www.googleapis.com/compute/v1/projects/{{project}}/regions/{{region}}/subnetworks/{{name}}"

...
func testSweepComputeTargetHttpProxy(region string) error {
	resource_name := "ComputeTargetHttpProxy"
	...
	list_template := strings.Split("https://www.googleapis.com/compute/v1/projects/{{project}}/global/targetHttpProxies", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://www.googleapis.com/compute/v1/projects/{{project}}/global/targetHttpProxies/{{name}}"

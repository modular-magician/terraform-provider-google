...
func init() {
	resource.AddTestSweepers("ComputeTargetInstance", &resource.Sweeper{
		Name: "ComputeTargetInstance",
		F:    testSweepComputeTargetInstance,
	})
}
...
func testSweepComputeTargetInstance(region string) error {
	...
	listTemplate := strings.Split("https://www.googleapis.com/compute/v1/projects/{{project}}/aggregated/targetInstances", "?")[0]
	...
	for _, ri := range rl {
		...
		deleteTemplate := "https://www.googleapis.com/compute/v1/projects/{{project}}/zones/{{zone}}/targetInstances/{{name}}"

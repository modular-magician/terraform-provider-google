...
func init() {
	resource.AddTestSweepers("ComputeVpnTunnel", &resource.Sweeper{
		Name: "ComputeVpnTunnel",
		F:    testSweepComputeVpnTunnel,
	})
}

func testSweepComputeVpnTunnel(region string) error {
	resource_name := "ComputeVpnTunnel"
	...
	list_template := strings.Split("https://www.googleapis.com/compute/v1/projects/{{project}}/regions/{{region}}/vpnTunnels", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://www.googleapis.com/compute/v1/projects/{{project}}/regions/{{region}}/vpnTunnels/{{name}}"

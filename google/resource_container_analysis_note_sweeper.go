...
func testSweepContainerAnalysisNote(region string) error {
	resource_name := "ContainerAnalysisNote"
	...
	list_template := strings.Split("https://containeranalysis.googleapis.com/v1/projects/{{project}}/notes?noteId={{name}}", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://containeranalysis.googleapis.com/v1/projects/{{project}}/notes/{{name}}"

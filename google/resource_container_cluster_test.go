...
func TestAccContainerCluster_withMonitoringConfigAdvancedDatapathObservabilityConfig(t *testing.T) {
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccContainerCluster_withMonitoringConfigAdvancedDatapathObservabilityConfigUpdated(clusterName),
			},
			{
				ResourceName:            "google_container_cluster.primary",
				ImportState:             true,
...
func testAccContainerCluster_withMonitoringConfigAdvancedDatapathObservabilityConfigUpdated(name string) string {
	return fmt.Sprintf(`
resource "google_container_cluster" "primary" {
  name               = "%s"
  location           = "us-central1-a"

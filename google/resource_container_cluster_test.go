...
func TestAccContainerCluster_withEnableK8sBetaAPIs(t *testing.T) {
	t.Parallel()
	...
	VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccContainerCluster_withEnableK8sBetaAPIs(clusterName, npName),
			},
			{
				ResourceName:      "google_container_cluster.primary",
				ImportState:       true,
...
func testAccContainerCluster_withEnableK8sBetaAPIs(cluster, np string) string {
	return fmt.Sprintf(`
resource "google_container_cluster" "primary" {
  name               = "%s"
  location           = "us-central1-a"
  enable_k8s_beta_apis {
	enabled_apis = ["autoscaling/v2beta1/horizontalpodautoscalers"]
  }

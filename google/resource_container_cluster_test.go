...
func TestAccContainerCluster_withMeshCertificates(t *testing.T) {
	t.Parallel()
	...
	vcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccContainerCluster_withMeshCertificates(pid, clusterName, npName),
			},
			{
...
func testAccContainerCluster_withMeshCertificates(pid string, clusterName string, npName string) string {
	return fmt.Sprintf(`

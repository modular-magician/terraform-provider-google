...
func TestAccContainerCluster_errorCleanDanglingCluster(t *testing.T) {
	...
	config := testAccContainerCluster_withCIDROverlap(clusterName, clusterNameError)
...
func TestAccContainerCluster_errorNoClusterCreated(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config:      testAccContainerCluster_withInvalidName(),
				ExpectError: regexp.MustCompile("Error waiting for creating GKE cluster"),
			},
		},
	})
...
func testAccContainerCluster_withCIDROverlap(firstCluster, secondCluster string) string {
	return fmt.Sprintf(`
resource "google_container_cluster" "cidr_error_test_preempt" {
  name = "%s"
  zone = "us-west1-a"
...
resource "google_container_cluster" "cidr_error_test" {
  name = "%s"
  zone = "us-west1-a"
  ...
  depends_on = ["google_container_cluster.cidr_error_test_preempt"]
}
`, firstCluster, secondCluster)
}

func testAccContainerCluster_withInvalidName() string {
	return `
resource "google_container_cluster" "with_resource_labels" {
	name = "0invalidname"
	zone = "us-central1-a"

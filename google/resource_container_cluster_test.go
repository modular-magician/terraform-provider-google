...
func TestAccContainerCluster_withMultiNetworking(t *testing.T) {
	...
	VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccContainerCluster_enableMultiNetworking(clusterName, npName),
			},
			{
				ResourceName:      "google_container_cluster.multi_network_cluster",
				ImportState:       true,
				ImportStateVerify: true,
			},
		},
...
func testAccContainerCluster_enableMultiNetworking(clusterName string, npName string) string {
	return fmt.Sprintf(`
resource "google_container_cluster" "multi_network_cluster" {
  name               = "%s"
  location           = "us-central1-a"
  release_channel {
    channel = "RAPID"

...
func resourceDataplexAssetCreate(d *schema.ResourceData, meta interface{}) error {
	...
	obj := &dataplex.Asset{
		...
		Zone:          dcl.String(zone),
	}

	id, err := replaceVarsForId(d, config, "projects/{{project}}/locations/{{location}}/lakes/{{lake}}/zones/{{zone}}/assets/{{name}}")
	if err != nil {
	...
	res, err := client.ApplyAsset(context.Background(), obj, createDirective...)
...
func resourceDataplexAssetRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err = d.Set("zone", res.Zone); err != nil {
		return fmt.Errorf("error setting zone in state: %s", err)
	}
...
func resourceDataplexAssetImport(d *schema.ResourceData, meta interface{}) ([]*schema.ResourceData, error) {
	...
	if err := parseImportId([]string{
		"projects/(?P<project>[^/]+)/locations/(?P<location>[^/]+)/lakes/(?P<lake>[^/]+)/zones/(?P<zone>[^/]+)/assets/(?P<name>[^/]+)",
		"(?P<project>[^/]+)/(?P<location>[^/]+)/(?P<lake>[^/]+)/(?P<zone>[^/]+)/(?P<name>[^/]+)",
		"(?P<location>[^/]+)/(?P<lake>[^/]+)/(?P<zone>[^/]+)/(?P<name>[^/]+)",

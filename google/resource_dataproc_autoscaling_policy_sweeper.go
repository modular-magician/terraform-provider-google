...
func testSweepDataprocAutoscalingPolicy(region string) error {
	resource_name := "DataprocAutoscalingPolicy"
	...
	list_template := strings.Split("https://dataproc.googleapis.com/v1/projects/{{project}}/locations/{{location}}/autoscalingPolicies", "?")[0]
	...
	resource_list, ok := res["autoscalingPolicies"]
	if !ok {
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://dataproc.googleapis.com/v1/projects/{{project}}/locations/{{location}}/autoscalingPolicies/{{policy_id}}"

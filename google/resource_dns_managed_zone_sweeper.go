...
func init() {
	resource.AddTestSweepers("DNSManagedZone", &resource.Sweeper{
		Name: "DNSManagedZone",
		F:    testSweepDNSManagedZone,
	})
}

func testSweepDNSManagedZone(region string) error {
	resource_name := "DNSManagedZone"
	...
	list_template := strings.Split("https://www.googleapis.com/dns/v1/projects/{{project}}/managedZones", "?")[0]
	...
	resource_list, ok := res["managedZones"]
	if !ok {
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://www.googleapis.com/dns/v1/projects/{{project}}/managedZones/{{name}}"

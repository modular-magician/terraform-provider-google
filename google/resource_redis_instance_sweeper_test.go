...
func testSweepRedisInstance(region string) error {
	...
	list_template := strings.Split("https://redis.googleapis.com/v1/projects/{{project}}/locations/{{region}}/instances", "?")[0]
	...
	resource_list, ok := res["instances"]
	if !ok {
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://redis.googleapis.com/v1/projects/{{project}}/locations/{{region}}/instances/{{name}}"

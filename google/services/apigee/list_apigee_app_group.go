...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Apigee",
		Func:        NewApigeeAppGroupListResource,
	}.Register()
}

var _ list.ListResource = &ApigeeAppGroupListResource{}

type ApigeeAppGroupListResource struct {
	tpgresource.ListResourceMetadata
}

func NewApigeeAppGroupListResource() list.ListResource {
	listR := &ApigeeAppGroupListResource{}
	listR.TypeName = "google_apigee_app_group"
	listR.SDKv2Resource = ResourceApigeeAppGroup()
...
// ApigeeAppGroupListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ApigeeAppGroupListModel struct {
	OrgId types.String `tfsdk:"org_id"`
}

func (listR *ApigeeAppGroupListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ApigeeAppGroupListModel
...
func ListApigeeAppGroups(config *transport_tpg.Config,
	orgId string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceApigeeAppGroup().Data(&terraform.InstanceState{})
	if orgId != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ApigeeBasePath}}{{org_id}}/appgroups")
	if err != nil {
		return err
	}
	billingProject := ""
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceApigeeAppGroup(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceApigeeAppGroupFlatten(d, config, res, config, userAgent, billingProject, url, headers); err != nil {
				return err
			}

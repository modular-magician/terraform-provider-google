...
func TestAccApigeeAppGroupListQuery_generated(t *testing.T) {
	t.Parallel()
	acctest.SkipIfVcr(t)
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_apigee_app_group.apigee_app_group",
						[]string{
							"name",
						},
					),
					listScope.Capture(map[string]string{
						"org_id": "google_apigee_app_group.apigee_app_group",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccApigeeAppGroup_apigeeAppGroupBasicTestExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_apigee_app_group.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_apigee_app_group.list_query",
...
func testAccApigeeAppGroup_apigeeAppGroupBasicTestExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "org_id" { type = string }
list "google_apigee_app_group" "list_query" {
    provider = google
	limit = 10000
    config {
        org_id = var.org_id

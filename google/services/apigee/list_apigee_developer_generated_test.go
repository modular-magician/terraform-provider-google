...
func TestAccApigeeDeveloperListQuery_generated(t *testing.T) {
	t.Parallel()
	acctest.SkipIfVcr(t)
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_apigee_developer.apigee_developer",
						[]string{
							"email",
						},
					),
					listScope.Capture(map[string]string{
						"org_id": "google_apigee_developer.apigee_developer",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccApigeeDeveloper_apigeeDeveloperBasicTestExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_apigee_developer.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_apigee_developer.list_query",
...
func testAccApigeeDeveloper_apigeeDeveloperBasicTestExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "org_id" { type = string }
list "google_apigee_developer" "list_query" {
    provider = google
	limit = 10000
    config {
        org_id = var.org_id

...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Apigee",
		Func:        NewApigeeEnvKeystoreListResource,
	}.Register()
}

var _ list.ListResource = &ApigeeEnvKeystoreListResource{}

type ApigeeEnvKeystoreListResource struct {
	tpgresource.ListResourceMetadata
}

func NewApigeeEnvKeystoreListResource() list.ListResource {
	listR := &ApigeeEnvKeystoreListResource{}
	listR.TypeName = "google_apigee_env_keystore"
	listR.SDKv2Resource = ResourceApigeeEnvKeystore()
...
// ApigeeEnvKeystoreListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ApigeeEnvKeystoreListModel struct {
	EnvId types.String `tfsdk:"env_id"`
}

func (listR *ApigeeEnvKeystoreListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ApigeeEnvKeystoreListModel
...
func ListApigeeEnvKeystores(config *transport_tpg.Config,
	envId string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceApigeeEnvKeystore().Data(&terraform.InstanceState{})
	if envId != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ApigeeBasePath}}{{env_id}}/keystores")
	if err != nil {
		return err
	}
	billingProject := ""
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceApigeeEnvKeystore(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceApigeeEnvKeystoreFlatten(d, config, res, config, userAgent, billingProject, url, headers); err != nil {
				return err
			}

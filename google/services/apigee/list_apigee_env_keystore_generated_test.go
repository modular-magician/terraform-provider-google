...
func TestAccApigeeEnvKeystoreListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					listScope.Capture(map[string]string{
						"env_id": "google_apigee_env_keystore.apigee_environment_keystore",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccApigeeEnvKeystore_apigeeEnvironmentKeystoreTestExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_apigee_env_keystore.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_apigee_env_keystore.list_query",
...
func testAccApigeeEnvKeystore_apigeeEnvironmentKeystoreTestExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "env_id" { type = string }
list "google_apigee_env_keystore" "list_query" {
    provider = google
	limit = 10000
    config {
        env_id = var.env_id

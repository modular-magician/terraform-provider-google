...
func TestAccApigeeEnvReferencesListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_apigee_env_references.apigee_environment_reference",
						[]string{
							"name",
						},
					),
					listScope.Capture(map[string]string{
						"env_id": "google_apigee_env_references.apigee_environment_reference",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccApigeeEnvReferences_apigeeEnvironmentReferenceTestExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_apigee_env_references.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_apigee_env_references.list_query",
...
func testAccApigeeEnvReferences_apigeeEnvironmentReferenceTestExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "env_id" { type = string }
list "google_apigee_env_references" "list_query" {
    provider = google
	limit = 10000
    config {
        env_id = var.env_id

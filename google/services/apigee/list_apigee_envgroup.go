...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Apigee",
		Func:        NewApigeeEnvgroupListResource,
	}.Register()
}

var _ list.ListResource = &ApigeeEnvgroupListResource{}

type ApigeeEnvgroupListResource struct {
	tpgresource.ListResourceMetadata
}

func NewApigeeEnvgroupListResource() list.ListResource {
	listR := &ApigeeEnvgroupListResource{}
	listR.TypeName = "google_apigee_envgroup"
	listR.SDKv2Resource = ResourceApigeeEnvgroup()
...
// ApigeeEnvgroupListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ApigeeEnvgroupListModel struct {
	OrgId types.String `tfsdk:"org_id"`
}

func (listR *ApigeeEnvgroupListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ApigeeEnvgroupListModel
...
func ListApigeeEnvgroups(config *transport_tpg.Config,
	orgId string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceApigeeEnvgroup().Data(&terraform.InstanceState{})
	if orgId != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ApigeeBasePath}}{{org_id}}/envgroups")
	if err != nil {
		return err
	}
	billingProject := ""
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceApigeeEnvgroup(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "environmentGroups",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceApigeeEnvgroupFlatten(d, config, res, config, userAgent, billingProject, url, headers); err != nil {
				return err
			}

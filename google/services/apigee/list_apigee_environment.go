...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Apigee",
		Func:        NewApigeeEnvironmentListResource,
	}.Register()
}

var _ list.ListResource = &ApigeeEnvironmentListResource{}

type ApigeeEnvironmentListResource struct {
	tpgresource.ListResourceMetadata
}

func NewApigeeEnvironmentListResource() list.ListResource {
	listR := &ApigeeEnvironmentListResource{}
	listR.TypeName = "google_apigee_environment"
	listR.SDKv2Resource = ResourceApigeeEnvironment()
...
// ApigeeEnvironmentListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ApigeeEnvironmentListModel struct {
	OrgId types.String `tfsdk:"org_id"`
}

func (listR *ApigeeEnvironmentListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ApigeeEnvironmentListModel
...
func ListApigeeEnvironments(config *transport_tpg.Config,
	orgId string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceApigeeEnvironment().Data(&terraform.InstanceState{})
	if orgId != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ApigeeBasePath}}{{org_id}}/environments")
	if err != nil {
		return err
	}
	billingProject := ""
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceApigeeEnvironment(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceApigeeEnvironmentFlatten(d, config, res, config, userAgent, billingProject, url, headers); err != nil {
				return err
			}

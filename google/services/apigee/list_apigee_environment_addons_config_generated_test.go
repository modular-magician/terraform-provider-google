...
func TestAccApigeeEnvironmentAddonsConfigListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_apigee_environment_addons_config.apigee_org_addons",
						[]string{
							"env_id",
						},
					),
					listScope.Capture(map[string]string{
						"env_id": "google_apigee_environment_addons_config.apigee_org_addons",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccApigeeEnvironmentAddonsConfig_apigeeEnvAddonsAnalyticsTestExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_apigee_environment_addons_config.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_apigee_environment_addons_config.list_query",
...
func testAccApigeeEnvironmentAddonsConfig_apigeeEnvAddonsAnalyticsTestExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "env_id" { type = string }
list "google_apigee_environment_addons_config" "list_query" {
    provider = google
	limit = 10000
    config {
        env_id = var.env_id

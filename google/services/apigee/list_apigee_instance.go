...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Apigee",
		Func:        NewApigeeInstanceListResource,
	}.Register()
}

var _ list.ListResource = &ApigeeInstanceListResource{}

type ApigeeInstanceListResource struct {
	tpgresource.ListResourceMetadata
}

func NewApigeeInstanceListResource() list.ListResource {
	listR := &ApigeeInstanceListResource{}
	listR.TypeName = "google_apigee_instance"
	listR.SDKv2Resource = ResourceApigeeInstance()
...
// ApigeeInstanceListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ApigeeInstanceListModel struct {
	OrgId types.String `tfsdk:"org_id"`
}

func (listR *ApigeeInstanceListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ApigeeInstanceListModel
...
func ListApigeeInstances(config *transport_tpg.Config,
	orgId string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceApigeeInstance().Data(&terraform.InstanceState{})
	if orgId != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ApigeeBasePath}}{{org_id}}/instances")
	if err != nil {
		return err
	}
	billingProject := ""
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceApigeeInstance(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceApigeeInstanceFlatten(d, config, res, config, userAgent, billingProject, url, headers); err != nil {
				return err
			}

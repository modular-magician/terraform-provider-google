...
func TestAccApigeeInstanceListQuery_generated(t *testing.T) {
	t.Parallel()
	acctest.SkipIfVcr(t)
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					listScope.Capture(map[string]string{
						"org_id": "google_apigee_instance.apigee_instance",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccApigeeInstance_apigeeInstanceBasicTestExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_apigee_instance.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_apigee_instance.list_query",
...
func testAccApigeeInstance_apigeeInstanceBasicTestExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "org_id" { type = string }
list "google_apigee_instance" "list_query" {
    provider = google
	limit = 10000
    config {
        org_id = var.org_id

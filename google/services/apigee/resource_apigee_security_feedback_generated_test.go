...
func TestAccApigeeSecurityFeedback_apigeeSecurityFeedbackBasicTestExample(t *testing.T) {
	t.Parallel()
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		CheckDestroy: testAccCheckApigeeSecurityFeedbackDestroyProducer(t),
		Steps: []resource.TestStep{
			{
				Config: testAccApigeeSecurityFeedback_apigeeSecurityFeedbackBasicTestExample(context),
			},
			{
...
func testAccApigeeSecurityFeedback_apigeeSecurityFeedbackBasicTestExample(context map[string]interface{}) string {
...
func testAccCheckApigeeSecurityFeedbackDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			if rs.Type != "google_apigee_security_feedback" {
				continue
			}
			...
			url, err := tpgresource.ReplaceVarsForTest(config, rs, transport_tpg.BaseUrl(apigee.Product, config)+"{{org_id}}/securityFeedback/{{feedback_id}}")
			if err != nil {
				return err
			}
			...
			if err == nil {
				return fmt.Errorf("ApigeeSecurityFeedback still exists at %s", url)
			}
		}

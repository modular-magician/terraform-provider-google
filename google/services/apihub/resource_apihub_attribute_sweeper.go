...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_apihub_attribute",
		ListAndAction:  listAndActionApihubAttribute,
		DeleteFunction: testSweepApihubAttribute,
	}
...
func testSweepApihubAttribute(_ string) error {
	return listAndActionApihubAttribute(deleteResourceApihubAttribute)
}

func listAndActionApihubAttribute(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ApihubAttribute"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://apihub.googleapis.com/v1/projects/{{project}}/locations/{{location}}/attributes", "?")[0]
		...
		resourceList, ok := res["attributes"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'attributes'")
		} else {
...
func deleteResourceApihubAttribute(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://apihub.googleapis.com/v1/projects/{{project}}/locations/{{location}}/attributes/{{attribute_id}}"

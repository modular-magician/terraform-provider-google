...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionApihubRuntimeProjectAttachment,
		DeleteFunction: testSweepApihubRuntimeProjectAttachment,
	}
...
func testSweepApihubRuntimeProjectAttachment(_ string) error {
	return listAndActionApihubRuntimeProjectAttachment(deleteResourceApihubRuntimeProjectAttachment)
}

func listAndActionApihubRuntimeProjectAttachment(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ApihubRuntimeProjectAttachment"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://apihub.googleapis.com/v1/projects/{{project}}/locations/{{location}}/runtimeProjectAttachments", "?")[0]
		...
		resourceList, ok := res["runtimeProjectAttachments"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'runtimeProjectAttachments'")
		} else {
...
func deleteResourceApihubRuntimeProjectAttachment(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://apihub.googleapis.com/v1/projects/{{project}}/locations/{{location}}/runtimeProjectAttachments/{{runtime_project_attachment_id}}"

...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionApikeysKey,
		DeleteFunction: testSweepApikeysKey,
	}
...
func testSweepApikeysKey(_ string) error {
	return listAndActionApikeysKey(deleteResourceApikeysKey)
}

func listAndActionApikeysKey(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://apikeys.googleapis.com/v2/projects/{{project}}/locations/global/keys", "?")[0]
		...
		resourceList, ok := res["keys"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'keys'")
		} else {
...
func deleteResourceApikeysKey(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://apikeys.googleapis.com/v2/projects/{{project}}/locations/global/keys/{{name}}"

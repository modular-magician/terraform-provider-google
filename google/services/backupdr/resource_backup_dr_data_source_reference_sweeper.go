...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionBackupDRDataSourceReference,
		DeleteFunction: testSweepBackupDRDataSourceReference,
	}
...
func testSweepBackupDRDataSourceReference(_ string) error {
	return listAndActionBackupDRDataSourceReference(deleteResourceBackupDRDataSourceReference)
}

func listAndActionBackupDRDataSourceReference(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "BackupDRDataSourceReference"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://backupdr.googleapis.com/v1/projects/{{project}}/locations/{{location}}/dataSourceReferences", "?")[0]
		...
		resourceList, ok := res["dataSourceReferences"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'dataSourceReferences'")
		} else {
...
func deleteResourceBackupDRDataSourceReference(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://backupdr.googleapis.com/v1/projects/{{project}}/locations/{{location}}/dataSourceReferences/{{data_source_reference_id}}"

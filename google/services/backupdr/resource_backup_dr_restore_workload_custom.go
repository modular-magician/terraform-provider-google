...
//
//	Source file: https://github.com/GoogleCloudPlatform/magic-modules/tree/main/mmv1/third_party/terraform/services/backupdr/resource_backup_dr_restore_workload_custom.go
//
...
func processRestoreWorkloadEncryptionKey(ekList []interface{}) map[string]interface{} {
...
func processRestoreWorkloadGuestOsFeatures(gofList []interface{}) []map[string]interface{} {
	if len(gofList) == 0 {
		return nil
	}
...
func processRestoreWorkloadKeyValueEntries(val interface{}) map[string]string {
...
func resourceBackupDRRestoreWorkloadCustomCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOkExists("compute_instance_restore_properties"); ok {
		...
		if len(list) > 0 {
			...
			if val, ok := m["disks"]; ok && len(val.([]interface{})) > 0 {
				...
				for _, disk := range disksList {
					...
					if v, ok := diskMap["guest_os_feature"]; ok && len(v.([]interface{})) > 0 {
						if gofArr := processRestoreWorkloadGuestOsFeatures(v.([]interface{})); gofArr != nil {
					...
					if v, ok := diskMap["disk_encryption_key"]; ok && len(v.([]interface{})) > 0 {
						if dekObj := processRestoreWorkloadEncryptionKey(v.([]interface{})); dekObj != nil {
			...
			if val, ok := m["instance_encryption_key"]; ok && len(val.([]interface{})) > 0 {
				if iekObj := processRestoreWorkloadEncryptionKey(val.([]interface{})); iekObj != nil {
			...
			if val, ok := m["labels"]; ok {
				if labels := processRestoreWorkloadKeyValueEntries(val); labels != nil {
					props["labels"] = labels
				}
			}
			...
			if val, ok := m["params"]; ok && len(val.([]interface{})) > 0 {
				...
				if pList[0] != nil {
					...
					if rmtVal, ok := p["resource_manager_tags"]; ok {
						if rmtMap := processRestoreWorkloadKeyValueEntries(rmtVal); rmtMap != nil {
	...
	if v, ok := d.GetOkExists("disk_restore_properties"); ok {
		...
		if len(list) > 0 {
			...
			if val, ok := m["guest_os_feature"]; ok && len(val.([]interface{})) > 0 {
				if gofArr := processRestoreWorkloadGuestOsFeatures(val.([]interface{})); gofArr != nil {
			...
			if val, ok := m["disk_encryption_key"]; ok && len(val.([]interface{})) > 0 {
				if dekObj := processRestoreWorkloadEncryptionKey(val.([]interface{})); dekObj != nil {
			...
			if val, ok := m["resource_manager_tags"]; ok {
				if rmtMap := processRestoreWorkloadKeyValueEntries(val); rmtMap != nil {

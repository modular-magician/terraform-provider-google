...
func testSweepBeyondcorpAppConnector(_ string) error {
	return listAndActionBeyondcorpAppConnector(deleteResourceBeyondcorpAppConnector)
}

func listAndActionBeyondcorpAppConnector(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'appConnectors'")
		} else {
...
func deleteResourceBeyondcorpAppConnector(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {

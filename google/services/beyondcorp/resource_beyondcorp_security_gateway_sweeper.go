...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionBeyondcorpSecurityGateway,
		DeleteFunction: testSweepBeyondcorpSecurityGateway,
	}
...
func testSweepBeyondcorpSecurityGateway(_ string) error {
	return listAndActionBeyondcorpSecurityGateway(deleteResourceBeyondcorpSecurityGateway)
}

func listAndActionBeyondcorpSecurityGateway(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "BeyondcorpSecurityGateway"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://beyondcorp.googleapis.com/v1/projects/{{project}}/locations/{{location}}/securityGateways", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'securityGateways'")
		} else {
...
func deleteResourceBeyondcorpSecurityGateway(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://beyondcorp.googleapis.com/v1/projects/{{project}}/locations/{{location}}/securityGateways/{{security_gateway_id}}"

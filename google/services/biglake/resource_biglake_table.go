	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"location": {
				...
				Required:    true,
				ForceNew:    true,
				Description: `The geographic location where the Table should reside.`,
			},
			"name": {
				Type:     schema.TypeString,
				Required: true,
...
func resourceBiglakeTableCreate(d *schema.ResourceData, meta interface{}) error {
	...
	url, err := tpgresource.ReplaceVars(d, config, "{{BiglakeBasePath}}projects/{{project}}/locations/{{location}}/catalogs/{{catalog_id}}/databases/{{database_id}}/tables?tableId={{name}}")
	if err != nil {
		return err
	}
	...
	if err != nil {
		return fmt.Errorf("Error fetching project for Table: %s", err)
	}
	...
	// Store the ID now
	id, err := tpgresource.ReplaceVars(d, config, "projects/{{project}}/locations/{{location}}/catalogs/{{catalog_id}}/databases/{{database_id}}/tables/{{name}}")
	if err != nil {
...
func resourceBiglakeTableRead(d *schema.ResourceData, meta interface{}) error {
	...
	url, err := tpgresource.ReplaceVars(d, config, "{{BiglakeBasePath}}projects/{{project}}/locations/{{location}}/catalogs/{{catalog_id}}/databases/{{database_id}}/tables/{{name}}")
	if err != nil {
		return err
	}
...
func resourceBiglakeTableImport(d *schema.ResourceData, meta interface{}) ([]*schema.ResourceData, error) {
	...
	if err := tpgresource.ParseImportId([]string{
		"projects/(?P<project>[^/]+)/locations/(?P<location>[^/]+)/catalogs/(?P<catalog_id>[^/]+)/databases/(?P<database_id>[^/]+)/tables/(?P<name>[^/]+)",
		"(?P<project>[^/]+)/(?P<location>[^/]+)/(?P<catalog_id>[^/]+)/(?P<database_id>[^/]+)/(?P<name>[^/]+)",
		"(?P<location>[^/]+)/(?P<catalog_id>[^/]+)/(?P<database_id>[^/]+)/(?P<name>[^/]+)",
	}, d, config); err != nil {
		return nil, err
	}

...
func testSweepBigqueryReservation(region string) error {
	...
	servicesUrl := config.BasePaths["bigqueryreservation"] + "projects/" + config.Project + "/locations/" + region + "/reservations"
...
func deleteAllAssignments(config *transport_tpg.Config, reservationName string) {
	assignmentListUrl := config.BasePaths["bigqueryreservation"] + reservationName + "/assignments"
	...
	for _, ri := range al {
		...
		deleteUrl := config.BasePaths["bigqueryreservation"] + name
		_, err = transport_tpg.SendRequest(transport_tpg.SendRequestOptions{
			Config:    config,
			Method:    "DELETE",
			Project:   config.Project,

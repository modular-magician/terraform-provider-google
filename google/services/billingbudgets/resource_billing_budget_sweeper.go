...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionBillingBudgetsBudget,
		DeleteFunction: testSweepBillingBudgetsBudget,
	}
...
func testSweepBillingBudgetsBudget(_ string) error {
	return listAndActionBillingBudgetsBudget(deleteResourceBillingBudgetsBudget)
}

func listAndActionBillingBudgetsBudget(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "BillingBudgetsBudget"
	...
	intermediateValues[0]["billing_account"] = envvar.GetTestMasterBillingAccountFromEnv(t)
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'budgets'")
		} else {
...
func deleteResourceBillingBudgetsBudget(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {

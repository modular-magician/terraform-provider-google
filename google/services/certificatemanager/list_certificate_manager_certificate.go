...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewCertificateManagerCertificateListResource,
	}.Register()
}

var _ list.ListResource = &CertificateManagerCertificateListResource{}

type CertificateManagerCertificateListResource struct {
	tpgresource.ListResourceMetadata
}

func NewCertificateManagerCertificateListResource() list.ListResource {
	listR := &CertificateManagerCertificateListResource{}
	listR.TypeName = "google_certificate_manager_certificate"
	listR.SDKv2Resource = ResourceCertificateManagerCertificate()
...
// CertificateManagerCertificateListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type CertificateManagerCertificateListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *CertificateManagerCertificateListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data CertificateManagerCertificateListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListCertificateManagerCertificates(
			listR.Client,
			location,
			project,
...
func ListCertificateManagerCertificates(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceCertificateManagerCertificate().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{CertificateManagerBasePath}}projects/{{project}}/locations/{{location}}/certificates")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceCertificateManagerCertificate(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if res == nil {
				return fmt.Errorf("error decoding CertificateManagerCertificate from list response")
			}
			...
			if err = ResourceCertificateManagerCertificateFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

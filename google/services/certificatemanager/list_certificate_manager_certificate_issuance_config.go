...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewCertificateManagerCertificateIssuanceConfigListResource,
	}.Register()
}

var _ list.ListResource = &CertificateManagerCertificateIssuanceConfigListResource{}

type CertificateManagerCertificateIssuanceConfigListResource struct {
	tpgresource.ListResourceMetadata
}

func NewCertificateManagerCertificateIssuanceConfigListResource() list.ListResource {
	listR := &CertificateManagerCertificateIssuanceConfigListResource{}
	listR.TypeName = "google_certificate_manager_certificate_issuance_config"
	listR.SDKv2Resource = ResourceCertificateManagerCertificateIssuanceConfig()
...
// CertificateManagerCertificateIssuanceConfigListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type CertificateManagerCertificateIssuanceConfigListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *CertificateManagerCertificateIssuanceConfigListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data CertificateManagerCertificateIssuanceConfigListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListCertificateManagerCertificateIssuanceConfigs(
			listR.Client,
			project,
...
func ListCertificateManagerCertificateIssuanceConfigs(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceCertificateManagerCertificateIssuanceConfig().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{CertificateManagerBasePath}}projects/{{project}}/locations/global/certificateIssuanceConfigs")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceCertificateManagerCertificateIssuanceConfig(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "certificateIssuanceConfigs",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if res == nil {
				return fmt.Errorf("error decoding CertificateManagerCertificateIssuanceConfig from list response")
			}
			...
			if err = ResourceCertificateManagerCertificateIssuanceConfigFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

...
func TestAccCertificateManagerCertificateIssuanceConfigListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_certificate_manager_certificate_issuance_config.default",
						[]string{
							"name",
						},
					),
			...
			{
				Query:           true,
				Config:          testAccCertificateManagerCertificateIssuanceConfig_certificateManagerCertificateIssuanceConfigExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_certificate_manager_certificate_issuance_config.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_certificate_manager_certificate_issuance_config.list_query",
...
func testAccCertificateManagerCertificateIssuanceConfig_certificateManagerCertificateIssuanceConfigExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
list "google_certificate_manager_certificate_issuance_config" "list_query" {
    provider = google
	limit = 10000
    config {
    }

...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewCertificateManagerCertificateMapListResource,
	}.Register()
}

var _ list.ListResource = &CertificateManagerCertificateMapListResource{}

type CertificateManagerCertificateMapListResource struct {
	tpgresource.ListResourceMetadata
}

func NewCertificateManagerCertificateMapListResource() list.ListResource {
	listR := &CertificateManagerCertificateMapListResource{}
	listR.TypeName = "google_certificate_manager_certificate_map"
	listR.SDKv2Resource = ResourceCertificateManagerCertificateMap()
...
// CertificateManagerCertificateMapListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type CertificateManagerCertificateMapListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *CertificateManagerCertificateMapListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data CertificateManagerCertificateMapListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListCertificateManagerCertificateMaps(
			listR.Client,
			project,
...
func ListCertificateManagerCertificateMaps(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceCertificateManagerCertificateMap().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{CertificateManagerBasePath}}projects/{{project}}/locations/global/certificateMaps")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceCertificateManagerCertificateMap(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "certificateMaps",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if res == nil {
				return fmt.Errorf("error decoding CertificateManagerCertificateMap from list response")
			}
			...
			if err = ResourceCertificateManagerCertificateMapFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewCertificateManagerTrustConfigListResource,
	}.Register()
}

var _ list.ListResource = &CertificateManagerTrustConfigListResource{}

type CertificateManagerTrustConfigListResource struct {
	tpgresource.ListResourceMetadata
}

func NewCertificateManagerTrustConfigListResource() list.ListResource {
	listR := &CertificateManagerTrustConfigListResource{}
	listR.TypeName = "google_certificate_manager_trust_config"
	listR.SDKv2Resource = ResourceCertificateManagerTrustConfig()
...
// CertificateManagerTrustConfigListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type CertificateManagerTrustConfigListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *CertificateManagerTrustConfigListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data CertificateManagerTrustConfigListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListCertificateManagerTrustConfigs(
			listR.Client,
			location,
			project,
...
func ListCertificateManagerTrustConfigs(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceCertificateManagerTrustConfig().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{CertificateManagerBasePath}}projects/{{project}}/locations/{{location}}/trustConfigs")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceCertificateManagerTrustConfig(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "trustConfigs",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if res == nil {
				return fmt.Errorf("error decoding CertificateManagerTrustConfig from list response")
			}
			...
			if err = ResourceCertificateManagerTrustConfigFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

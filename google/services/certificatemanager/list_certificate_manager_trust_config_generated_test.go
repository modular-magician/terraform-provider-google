...
func TestAccCertificateManagerTrustConfigListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_certificate_manager_trust_config.default",
						[]string{
							"name",
						},
					),
					listScope.Capture(map[string]string{
						"location": "google_certificate_manager_trust_config.default",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccCertificateManagerTrustConfig_certificateManagerTrustConfigExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_certificate_manager_trust_config.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_certificate_manager_trust_config.list_query",
...
func testAccCertificateManagerTrustConfig_certificateManagerTrustConfigExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "location" { type = string }
list "google_certificate_manager_trust_config" "list_query" {
    provider = google
	limit = 10000
    config {

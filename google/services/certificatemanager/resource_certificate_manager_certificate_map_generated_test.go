	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				ImportStateVerifyIgnore: []string{"labels", "name", "tags", "terraform_labels"},
			},
			{
				ResourceName:       "google_certificate_manager_certificate_map.default",
				RefreshState:       true,
...
func testAccCheckCertificateManagerCertificateMapDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			...
			url, err := tpgresource.ReplaceVarsForTest(config, rs, transport_tpg.BaseUrl(certificatemanager.Product, config)+"projects/{{project}}/locations/global/certificateMaps/{{name}}")
			if err != nil {
				return err
			}

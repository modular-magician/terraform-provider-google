...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_ces_app",
		ListAndAction:  listAndActionCESApp,
		DeleteFunction: testSweepCESApp,
	}
...
func testSweepCESApp(_ string) error {
	return listAndActionCESApp(deleteResourceCESApp)
}

func listAndActionCESApp(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "CESApp"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://ces.googleapis.com/v1/projects/{{project}}/locations/{{location}}/apps", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'apps'")
		} else {
...
func deleteResourceCESApp(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://ces.googleapis.com/v1/projects/{{project}}/locations/{{location}}/apps/{{name}}"

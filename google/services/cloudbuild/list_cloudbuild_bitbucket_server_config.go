...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewCloudBuildBitbucketServerConfigListResource,
	}.Register()
}

var _ list.ListResource = &CloudBuildBitbucketServerConfigListResource{}

type CloudBuildBitbucketServerConfigListResource struct {
	tpgresource.ListResourceMetadata
}

func NewCloudBuildBitbucketServerConfigListResource() list.ListResource {
	listR := &CloudBuildBitbucketServerConfigListResource{}
	listR.TypeName = "google_cloudbuild_bitbucket_server_config"
	listR.SDKv2Resource = ResourceCloudBuildBitbucketServerConfig()
...
// CloudBuildBitbucketServerConfigListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type CloudBuildBitbucketServerConfigListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *CloudBuildBitbucketServerConfigListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data CloudBuildBitbucketServerConfigListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListCloudBuildBitbucketServerConfigs(
			listR.Client,
			location,
			project,
			func(rd *schema.ResourceData) error {
				...
				if err := listR.SetResult(ctx, listReq.IncludeResource, &result, rd, "name", "config_id"); err != nil {
					return err
				}
...
func ListCloudBuildBitbucketServerConfigs(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceCloudBuildBitbucketServerConfig().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{CloudBuildBasePath}}projects/{{project}}/locations/{{location}}/bitbucketServerConfigs")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceCloudBuildBitbucketServerConfig(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "bitbucketServerConfigs",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			headers := make(http.Header)
			var err error
			if v, ok := res["config_id"]; ok && v != nil {
				if err := d.Set("config_id", v); err != nil {
					return fmt.Errorf("error setting config_id: %w", err)
				}
			}
			...
			if err = ResourceCloudBuildBitbucketServerConfigFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

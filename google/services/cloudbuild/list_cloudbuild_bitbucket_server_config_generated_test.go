...
func TestAccCloudBuildBitbucketServerConfigListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_cloudbuild_bitbucket_server_config.bbs-config",
						[]string{
							"name",
							"config_id",
						},
					...
					listScope.Capture(map[string]string{
						"location": "google_cloudbuild_bitbucket_server_config.bbs-config",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccCloudBuildBitbucketServerConfig_cloudbuildBitbucketServerConfigExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_cloudbuild_bitbucket_server_config.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_cloudbuild_bitbucket_server_config.list_query",
...
func testAccCloudBuildBitbucketServerConfig_cloudbuildBitbucketServerConfigExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "location" { type = string }
list "google_cloudbuild_bitbucket_server_config" "list_query" {
    provider = google
	limit = 10000
    config {

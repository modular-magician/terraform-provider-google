...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewCloudBuildTriggerListResource,
	}.Register()
}

var _ list.ListResource = &CloudBuildTriggerListResource{}

type CloudBuildTriggerListResource struct {
	tpgresource.ListResourceMetadata
}

func NewCloudBuildTriggerListResource() list.ListResource {
	listR := &CloudBuildTriggerListResource{}
	listR.TypeName = "google_cloudbuild_trigger"
	listR.SDKv2Resource = ResourceCloudBuildTrigger()
...
// CloudBuildTriggerListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type CloudBuildTriggerListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *CloudBuildTriggerListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data CloudBuildTriggerListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListCloudBuildTriggers(
			...
			func(rd *schema.ResourceData) error {
				...
				if err := listR.SetResult(ctx, listReq.IncludeResource, &result, rd, "name", "trigger_id"); err != nil {
					return err
				}
...
func ListCloudBuildTriggers(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceCloudBuildTrigger().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{CloudBuildBasePath}}projects/{{project}}/locations/{{location}}/triggers")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceCloudBuildTrigger(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if v, ok := res["id"]; ok && v != nil {
				if err := d.Set("trigger_id", v); err != nil {
					return fmt.Errorf("error setting trigger_id: %w", err)
				}
			}
			...
			if err = ResourceCloudBuildTriggerFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

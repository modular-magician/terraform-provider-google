...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionCloudBuildGitLabConfig,
		DeleteFunction: testSweepCloudBuildGitLabConfig,
	}
...
func testSweepCloudBuildGitLabConfig(_ string) error {
	return listAndActionCloudBuildGitLabConfig(deleteResourceCloudBuildGitLabConfig)
}

func listAndActionCloudBuildGitLabConfig(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "CloudBuildGitLabConfig"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://cloudbuild.googleapis.com/v1/projects/{{project}}/locations/{{location}}/gitLabConfigs", "?")[0]
		...
		resourceList, ok := res["gitLabConfigs"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'gitLabConfigs'")
		} else {
...
func deleteResourceCloudBuildGitLabConfig(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://cloudbuild.googleapis.com/v1/projects/{{project}}/locations/{{location}}/gitLabConfigs/{{config_id}}"

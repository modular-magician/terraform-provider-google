...
type Cloudbuildv2ConnectionIamUpdater struct {
	project  string
	location string
	name     string
...
func Cloudbuildv2ConnectionIamUpdaterProducer(d tpgresource.TerraformResourceData, config *transport_tpg.Config) (tpgiamresource.ResourceIamUpdater, error) {
	...
	m, err := tpgresource.GetImportIdQualifiers([]string{"projects/(?P<project>[^/]+)/locations/(?P<location>[^/]+)/connections/(?P<name>[^/]+)", "(?P<project>[^/]+)/(?P<location>[^/]+)/(?P<name>[^/]+)", "(?P<location>[^/]+)/(?P<name>[^/]+)", "(?P<name>[^/]+)"}, d, config, d.Get("name").(string))
	if err != nil {
		return nil, err
	}
	...
	u := &Cloudbuildv2ConnectionIamUpdater{
...
func Cloudbuildv2ConnectionIdParseFunc(d *schema.ResourceData, config *transport_tpg.Config) error {
	...
	m, err := tpgresource.GetImportIdQualifiers([]string{"projects/(?P<project>[^/]+)/locations/(?P<location>[^/]+)/connections/(?P<name>[^/]+)", "(?P<project>[^/]+)/(?P<location>[^/]+)/(?P<name>[^/]+)", "(?P<location>[^/]+)/(?P<name>[^/]+)", "(?P<name>[^/]+)"}, d, config, d.Id())
	if err != nil {
		return err
	}
...
func (u *Cloudbuildv2ConnectionIamUpdater) GetResourceIamPolicy() (*cloudresourcemanager.Policy, error) {
...
func (u *Cloudbuildv2ConnectionIamUpdater) GetResourceId() string {
	return fmt.Sprintf("projects/%s/locations/%s/connections/%s", u.project, u.location, u.name)
}

func (u *Cloudbuildv2ConnectionIamUpdater) GetMutexKey() string {
	return fmt.Sprintf("iam-cloudbuildv2-connection-%s", u.GetResourceId())
}

func (u *Cloudbuildv2ConnectionIamUpdater) DescribeResource() string {
	return fmt.Sprintf("cloudbuildv2 connection %q", u.GetResourceId())
}

...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionCloudIdsEndpoint,
		DeleteFunction: testSweepCloudIdsEndpoint,
	}
...
func testSweepCloudIdsEndpoint(_ string) error {
	return listAndActionCloudIdsEndpoint(deleteResourceCloudIdsEndpoint)
}

func listAndActionCloudIdsEndpoint(action sweeper.ResourceAction) error {
	var lastError error
	...
	intermediateValues[0]["location"] = "us-central1-f"
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'endpoints'")
		} else {
...
func deleteResourceCloudIdsEndpoint(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {

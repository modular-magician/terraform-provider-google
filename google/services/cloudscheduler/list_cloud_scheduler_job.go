...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewCloudSchedulerJobListResource,
	}.Register()
}

var _ list.ListResource = &CloudSchedulerJobListResource{}

type CloudSchedulerJobListResource struct {
	tpgresource.ListResourceMetadata
}

func NewCloudSchedulerJobListResource() list.ListResource {
	listR := &CloudSchedulerJobListResource{}
	listR.TypeName = "google_cloud_scheduler_job"
	listR.SDKv2Resource = ResourceCloudSchedulerJob()
...
// CloudSchedulerJobListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type CloudSchedulerJobListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *CloudSchedulerJobListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data CloudSchedulerJobListModel
...
func ListCloudSchedulerJobs(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceCloudSchedulerJob().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{CloudSchedulerBasePath}}projects/{{project}}/locations/{{region}}/jobs")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceCloudSchedulerJob(),
		ListURL:        url,

...
func testAccComposerEnvironment_composerV2_withDatabaseRetentionEnabled(envName, network, subnetwork string) string {
	return fmt.Sprintf(`
resource "google_composer_environment" "test" {
  name   = "%s"
  region = "us-east1"
    ...
    config {
      ...
      data_retention_config {
		...
		// This will work only in projects with experiment enabled.
...
func testAccComposerEnvironment_composerV2_withDatabaseRetentionDisabled(envName, network, subnetwork string) string {
	return fmt.Sprintf(`
resource "google_composer_environment" "test" {
  name   = "%s"
  region = "us-east1"
...
func TestAccComposerEnvironment_composerV2_databaseRetention(t *testing.T) {
	t.Parallel()
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccComposerEnvironment_composerV2_withDatabaseRetentionEnabled(envName, network, subnetwork),
			},
			{
				ResourceName:      "google_composer_environment.test",
				ImportState:       true,
...
func TestAccComposerEnvironment_composerV2_update_databaseRetention(t *testing.T) {
	t.Parallel()
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccComposerEnvironment_composerV2_withDatabaseRetentionDisabled(envName, network, subnetwork),
			},
			{
				ResourceName:      "google_composer_environment.test",
				ImportState:       true,

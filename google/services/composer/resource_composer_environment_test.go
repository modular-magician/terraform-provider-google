...
func TestAccComposerEnvironmentComposer2_privateWithRangeNames(t *testing.T) {
	t.Parallel()
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccComposerEnvironmentComposer2_privateWithRangeNames(envName, network, subnetwork),
			},
			{
				ResourceName:      "google_composer_environment.test",
				ImportState:       true,
...
func testAccComposerEnvironmentComposer2_privateWithRangeNames(name, network, subnetwork string) string {
	return fmt.Sprintf(`
resource "google_composer_environment" "test" {
  name   = "%s"
  region = "us-central1"

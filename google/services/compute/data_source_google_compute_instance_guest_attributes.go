...
func dataSourceGoogleComputeInstanceGuestAttributesRead(d *schema.ResourceData, meta interface{}) error {
	...
	instanceUrl := fmt.Sprintf("%sprojects/%s/zones/%s/instances/%s", config.BasePaths["compute"], project, zone, name)
	...
	guestAttrsUrl := fmt.Sprintf("%sprojects/%s/zones/%s/instances/%s/getGuestAttributes", config.BasePaths["compute"], project, zone, name)

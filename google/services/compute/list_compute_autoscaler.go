...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeAutoscalerListResource,
	}.Register()
}

var _ list.ListResource = &ComputeAutoscalerListResource{}

type ComputeAutoscalerListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeAutoscalerListResource() list.ListResource {
	listR := &ComputeAutoscalerListResource{}
	listR.TypeName = "google_compute_autoscaler"
	listR.SDKv2Resource = ResourceComputeAutoscaler()
...
// ComputeAutoscalerListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeAutoscalerListModel struct {
	Zone    types.String `tfsdk:"zone"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeAutoscalerListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeAutoscalerListModel
...
func ListComputeAutoscalers(config *transport_tpg.Config,
	zone string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeAutoscaler().Data(&terraform.InstanceState{})
	if zone != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/zones/{{zone}}/autoscalers")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeAutoscaler(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeAutoscalerFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

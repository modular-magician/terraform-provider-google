...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeBackendBucketListResource,
	}.Register()
}

var _ list.ListResource = &ComputeBackendBucketListResource{}

type ComputeBackendBucketListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeBackendBucketListResource() list.ListResource {
	listR := &ComputeBackendBucketListResource{}
	listR.TypeName = "google_compute_backend_bucket"
	listR.SDKv2Resource = ResourceComputeBackendBucket()
...
// ComputeBackendBucketListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeBackendBucketListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeBackendBucketListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeBackendBucketListModel
...
func ListComputeBackendBuckets(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeBackendBucket().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/global/backendBuckets")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeBackendBucket(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeBackendBucketFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeBackendServiceListResource,
	}.Register()
}

var _ list.ListResource = &ComputeBackendServiceListResource{}

type ComputeBackendServiceListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeBackendServiceListResource() list.ListResource {
	listR := &ComputeBackendServiceListResource{}
	listR.TypeName = "google_compute_backend_service"
	listR.SDKv2Resource = ResourceComputeBackendService()
...
// ComputeBackendServiceListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeBackendServiceListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeBackendServiceListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeBackendServiceListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeBackendServices(
			listR.Client,
			project,
...
func ListComputeBackendServices(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeBackendService().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/global/backendServices")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeBackendService(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if res == nil {
				return fmt.Errorf("error decoding ComputeBackendService from list response")
			}
			...
			if err = ResourceComputeBackendServiceFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeCrossSiteNetworkListResource,
	}.Register()
}

var _ list.ListResource = &ComputeCrossSiteNetworkListResource{}

type ComputeCrossSiteNetworkListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeCrossSiteNetworkListResource() list.ListResource {
	listR := &ComputeCrossSiteNetworkListResource{}
	listR.TypeName = "google_compute_cross_site_network"
	listR.SDKv2Resource = ResourceComputeCrossSiteNetwork()
...
// ComputeCrossSiteNetworkListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeCrossSiteNetworkListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeCrossSiteNetworkListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeCrossSiteNetworkListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeCrossSiteNetworks(
			listR.Client,
			project,
...
func ListComputeCrossSiteNetworks(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeCrossSiteNetwork().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/global/crossSiteNetworks")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeCrossSiteNetwork(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "crossSiteNetworks",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeCrossSiteNetworkFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

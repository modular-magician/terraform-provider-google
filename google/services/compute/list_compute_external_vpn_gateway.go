...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeExternalVpnGatewayListResource,
	}.Register()
}

var _ list.ListResource = &ComputeExternalVpnGatewayListResource{}

type ComputeExternalVpnGatewayListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeExternalVpnGatewayListResource() list.ListResource {
	listR := &ComputeExternalVpnGatewayListResource{}
	listR.TypeName = "google_compute_external_vpn_gateway"
	listR.SDKv2Resource = ResourceComputeExternalVpnGateway()
...
// ComputeExternalVpnGatewayListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeExternalVpnGatewayListModel struct {
...
func (listR *ComputeExternalVpnGatewayListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeExternalVpnGatewayListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeExternalVpnGateways(
			listR.Client,
			project,
...
func ListComputeExternalVpnGateways(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeExternalVpnGateway().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/global/externalVpnGateways")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeExternalVpnGateway(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeExternalVpnGatewayFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeForwardingRuleListResource,
	}.Register()
}

var _ list.ListResource = &ComputeForwardingRuleListResource{}

type ComputeForwardingRuleListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeForwardingRuleListResource() list.ListResource {
	listR := &ComputeForwardingRuleListResource{}
	listR.TypeName = "google_compute_forwarding_rule"
	listR.SDKv2Resource = ResourceComputeForwardingRule()
...
// ComputeForwardingRuleListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeForwardingRuleListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeForwardingRuleListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeForwardingRuleListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeForwardingRules(
			listR.Client,
			region,
			project,
...
func ListComputeForwardingRules(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeForwardingRule().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVarsForId(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/regions/{{region}}/forwardingRules")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeForwardingRule(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeForwardingRuleFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

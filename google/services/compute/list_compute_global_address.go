...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeGlobalAddressListResource,
	}.Register()
}

var _ list.ListResource = &ComputeGlobalAddressListResource{}

type ComputeGlobalAddressListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeGlobalAddressListResource() list.ListResource {
	listR := &ComputeGlobalAddressListResource{}
	listR.TypeName = "google_compute_global_address"
	listR.SDKv2Resource = ResourceComputeGlobalAddress()
...
// ComputeGlobalAddressListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeGlobalAddressListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeGlobalAddressListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeGlobalAddressListModel
...
func ListComputeGlobalAddresss(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeGlobalAddress().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/global/addresses")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeGlobalAddress(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeGlobalAddressFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

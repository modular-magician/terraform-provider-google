...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeHealthCheckListResource,
	}.Register()
}

var _ list.ListResource = &ComputeHealthCheckListResource{}

type ComputeHealthCheckListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeHealthCheckListResource() list.ListResource {
	listR := &ComputeHealthCheckListResource{}
	listR.TypeName = "google_compute_health_check"
	listR.SDKv2Resource = ResourceComputeHealthCheck()
...
// ComputeHealthCheckListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeHealthCheckListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeHealthCheckListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeHealthCheckListModel
...
func ListComputeHealthChecks(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeHealthCheck().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/global/healthChecks")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeHealthCheck(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeHealthCheckFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeHttpHealthCheckListResource,
	}.Register()
}

var _ list.ListResource = &ComputeHttpHealthCheckListResource{}

type ComputeHttpHealthCheckListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeHttpHealthCheckListResource() list.ListResource {
	listR := &ComputeHttpHealthCheckListResource{}
	listR.TypeName = "google_compute_http_health_check"
	listR.SDKv2Resource = ResourceComputeHttpHealthCheck()
...
// ComputeHttpHealthCheckListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeHttpHealthCheckListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeHttpHealthCheckListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeHttpHealthCheckListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeHttpHealthChecks(
			listR.Client,
			project,
...
func ListComputeHttpHealthChecks(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeHttpHealthCheck().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/global/httpHealthChecks")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeHttpHealthCheck(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeHttpHealthCheckFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeHttpsHealthCheckListResource,
	}.Register()
}

var _ list.ListResource = &ComputeHttpsHealthCheckListResource{}

type ComputeHttpsHealthCheckListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeHttpsHealthCheckListResource() list.ListResource {
	listR := &ComputeHttpsHealthCheckListResource{}
	listR.TypeName = "google_compute_https_health_check"
	listR.SDKv2Resource = ResourceComputeHttpsHealthCheck()
...
// ComputeHttpsHealthCheckListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeHttpsHealthCheckListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeHttpsHealthCheckListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeHttpsHealthCheckListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeHttpsHealthChecks(
			listR.Client,
			project,
...
func ListComputeHttpsHealthChecks(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeHttpsHealthCheck().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/global/httpsHealthChecks")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeHttpsHealthCheck(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeHttpsHealthCheckFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

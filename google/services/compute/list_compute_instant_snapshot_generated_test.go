...
func TestAccComputeInstantSnapshotListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					listScope.Capture(map[string]string{
						"zone":    "google_compute_instant_snapshot.default",
						"project": "google_compute_instant_snapshot.default",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccComputeInstantSnapshot_instantSnapshotBasicExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_compute_instant_snapshot.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_compute_instant_snapshot.list_query",
...
func testAccComputeInstantSnapshot_instantSnapshotBasicExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "zone" { type = string }
variable "project" { type = string }
list "google_compute_instant_snapshot" "list_query" {
    provider = google
	limit = 10000
    config {
        zone = var.zone

...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeInterconnectGroupListResource,
	}.Register()
}

var _ list.ListResource = &ComputeInterconnectGroupListResource{}

type ComputeInterconnectGroupListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeInterconnectGroupListResource() list.ListResource {
	listR := &ComputeInterconnectGroupListResource{}
	listR.TypeName = "google_compute_interconnect_group"
	listR.SDKv2Resource = ResourceComputeInterconnectGroup()
...
// ComputeInterconnectGroupListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeInterconnectGroupListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeInterconnectGroupListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeInterconnectGroupListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeInterconnectGroups(
			listR.Client,
			project,
...
func ListComputeInterconnectGroups(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeInterconnectGroup().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/global/interconnectGroups")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeInterconnectGroup(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "interconnectGroups",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeInterconnectGroupFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

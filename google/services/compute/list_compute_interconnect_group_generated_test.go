...
func TestAccComputeInterconnectGroupListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_compute_interconnect_group.example-interconnect-group",
						[]string{
							"name",
						},
					),
					listScope.Capture(map[string]string{
						"project": "google_compute_interconnect_group.example-interconnect-group",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccComputeInterconnectGroup_interconnectGroupBasicExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_compute_interconnect_group.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_compute_interconnect_group.list_query",
...
func testAccComputeInterconnectGroup_interconnectGroupBasicExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "project" { type = string }
list "google_compute_interconnect_group" "list_query" {
    provider = google
	limit = 10000
    config {
        project = var.project

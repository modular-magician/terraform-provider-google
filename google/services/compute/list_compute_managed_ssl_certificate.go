...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeManagedSslCertificateListResource,
	}.Register()
}

var _ list.ListResource = &ComputeManagedSslCertificateListResource{}

type ComputeManagedSslCertificateListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeManagedSslCertificateListResource() list.ListResource {
	listR := &ComputeManagedSslCertificateListResource{}
	listR.TypeName = "google_compute_managed_ssl_certificate"
	listR.SDKv2Resource = ResourceComputeManagedSslCertificate()
...
// ComputeManagedSslCertificateListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeManagedSslCertificateListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeManagedSslCertificateListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeManagedSslCertificateListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeManagedSslCertificates(
			listR.Client,
			project,
...
func ListComputeManagedSslCertificates(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeManagedSslCertificate().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/global/sslCertificates")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeManagedSslCertificate(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeManagedSslCertificateFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

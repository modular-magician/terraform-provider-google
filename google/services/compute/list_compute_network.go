...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeNetworkListResource,
	}.Register()
}

var _ list.ListResource = &ComputeNetworkListResource{}

type ComputeNetworkListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeNetworkListResource() list.ListResource {
	listR := &ComputeNetworkListResource{}
	listR.TypeName = "google_compute_network"
	listR.SDKv2Resource = ResourceComputeNetwork()
...
// ComputeNetworkListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeNetworkListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeNetworkListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeNetworkListModel
...
func ListComputeNetworks(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeNetwork().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/global/networks")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeNetwork(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if res == nil {
				return fmt.Errorf("error decoding ComputeNetwork from list response")
			}
			...
			if err = ResourceComputeNetworkFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

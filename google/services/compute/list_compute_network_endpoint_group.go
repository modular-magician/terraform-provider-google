...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeNetworkEndpointGroupListResource,
	}.Register()
}

var _ list.ListResource = &ComputeNetworkEndpointGroupListResource{}

type ComputeNetworkEndpointGroupListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeNetworkEndpointGroupListResource() list.ListResource {
	listR := &ComputeNetworkEndpointGroupListResource{}
	listR.TypeName = "google_compute_network_endpoint_group"
	listR.SDKv2Resource = ResourceComputeNetworkEndpointGroup()
...
// ComputeNetworkEndpointGroupListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeNetworkEndpointGroupListModel struct {
	Zone    types.String `tfsdk:"zone"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeNetworkEndpointGroupListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeNetworkEndpointGroupListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeNetworkEndpointGroups(
			listR.Client,
			zone,
			project,
...
func ListComputeNetworkEndpointGroups(config *transport_tpg.Config,
	zone string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeNetworkEndpointGroup().Data(&terraform.InstanceState{})
	if zone != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/zones/{{zone}}/networkEndpointGroups")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeNetworkEndpointGroup(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeNetworkEndpointGroupFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeNodeGroupListResource,
	}.Register()
}

var _ list.ListResource = &ComputeNodeGroupListResource{}

type ComputeNodeGroupListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeNodeGroupListResource() list.ListResource {
	listR := &ComputeNodeGroupListResource{}
	listR.TypeName = "google_compute_node_group"
	listR.SDKv2Resource = ResourceComputeNodeGroup()
...
// ComputeNodeGroupListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeNodeGroupListModel struct {
	Zone    types.String `tfsdk:"zone"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeNodeGroupListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeNodeGroupListModel
...
func ListComputeNodeGroups(config *transport_tpg.Config,
	zone string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeNodeGroup().Data(&terraform.InstanceState{})
	if zone != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/zones/{{zone}}/nodeGroups")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeNodeGroup(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeNodeGroupFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

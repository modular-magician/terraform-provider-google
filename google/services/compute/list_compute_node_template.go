...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeNodeTemplateListResource,
	}.Register()
}

var _ list.ListResource = &ComputeNodeTemplateListResource{}

type ComputeNodeTemplateListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeNodeTemplateListResource() list.ListResource {
	listR := &ComputeNodeTemplateListResource{}
	listR.TypeName = "google_compute_node_template"
	listR.SDKv2Resource = ResourceComputeNodeTemplate()
...
// ComputeNodeTemplateListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeNodeTemplateListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeNodeTemplateListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeNodeTemplateListModel
...
func ListComputeNodeTemplates(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeNodeTemplate().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/regions/{{region}}/nodeTemplates")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeNodeTemplate(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeNodeTemplateFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

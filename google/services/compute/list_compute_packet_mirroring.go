...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputePacketMirroringListResource,
	}.Register()
}

var _ list.ListResource = &ComputePacketMirroringListResource{}

type ComputePacketMirroringListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputePacketMirroringListResource() list.ListResource {
	listR := &ComputePacketMirroringListResource{}
	listR.TypeName = "google_compute_packet_mirroring"
	listR.SDKv2Resource = ResourceComputePacketMirroring()
...
// ComputePacketMirroringListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputePacketMirroringListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputePacketMirroringListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputePacketMirroringListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputePacketMirrorings(
			listR.Client,
			region,
			project,
...
func ListComputePacketMirrorings(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputePacketMirroring().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/regions/{{region}}/packetMirrorings")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputePacketMirroring(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "packetMirrorings",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputePacketMirroringFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

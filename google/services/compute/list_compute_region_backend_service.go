...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeRegionBackendServiceListResource,
	}.Register()
}

var _ list.ListResource = &ComputeRegionBackendServiceListResource{}

type ComputeRegionBackendServiceListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeRegionBackendServiceListResource() list.ListResource {
	listR := &ComputeRegionBackendServiceListResource{}
	listR.TypeName = "google_compute_region_backend_service"
	listR.SDKv2Resource = ResourceComputeRegionBackendService()
...
// ComputeRegionBackendServiceListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeRegionBackendServiceListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeRegionBackendServiceListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeRegionBackendServiceListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeRegionBackendServices(
			listR.Client,
			region,
			project,
...
func ListComputeRegionBackendServices(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeRegionBackendService().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/regions/{{region}}/backendServices")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeRegionBackendService(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if res == nil {
				return fmt.Errorf("error decoding ComputeRegionBackendService from list response")
			}
			...
			if err = ResourceComputeRegionBackendServiceFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

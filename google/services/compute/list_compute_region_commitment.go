...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeRegionCommitmentListResource,
	}.Register()
}

var _ list.ListResource = &ComputeRegionCommitmentListResource{}

type ComputeRegionCommitmentListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeRegionCommitmentListResource() list.ListResource {
	listR := &ComputeRegionCommitmentListResource{}
	listR.TypeName = "google_compute_region_commitment"
	listR.SDKv2Resource = ResourceComputeRegionCommitment()
...
// ComputeRegionCommitmentListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeRegionCommitmentListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeRegionCommitmentListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeRegionCommitmentListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeRegionCommitments(
			listR.Client,
			region,
			project,
...
func ListComputeRegionCommitments(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeRegionCommitment().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/regions/{{region}}/commitments")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeRegionCommitment(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeRegionCommitmentFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

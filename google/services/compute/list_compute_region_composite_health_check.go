...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeRegionCompositeHealthCheckListResource,
	}.Register()
}

var _ list.ListResource = &ComputeRegionCompositeHealthCheckListResource{}

type ComputeRegionCompositeHealthCheckListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeRegionCompositeHealthCheckListResource() list.ListResource {
	listR := &ComputeRegionCompositeHealthCheckListResource{}
	listR.TypeName = "google_compute_region_composite_health_check"
	listR.SDKv2Resource = ResourceComputeRegionCompositeHealthCheck()
...
// ComputeRegionCompositeHealthCheckListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeRegionCompositeHealthCheckListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeRegionCompositeHealthCheckListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeRegionCompositeHealthCheckListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeRegionCompositeHealthChecks(
			listR.Client,
			region,
			project,
...
func ListComputeRegionCompositeHealthChecks(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeRegionCompositeHealthCheck().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/regions/{{region}}/compositeHealthChecks")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeRegionCompositeHealthCheck(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "compositeHealthChecks",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeRegionCompositeHealthCheckFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

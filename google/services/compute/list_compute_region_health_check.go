...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeRegionHealthCheckListResource,
	}.Register()
}

var _ list.ListResource = &ComputeRegionHealthCheckListResource{}

type ComputeRegionHealthCheckListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeRegionHealthCheckListResource() list.ListResource {
	listR := &ComputeRegionHealthCheckListResource{}
	listR.TypeName = "google_compute_region_health_check"
	listR.SDKv2Resource = ResourceComputeRegionHealthCheck()
...
// ComputeRegionHealthCheckListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeRegionHealthCheckListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeRegionHealthCheckListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeRegionHealthCheckListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeRegionHealthChecks(
			listR.Client,
			region,
			project,
...
func ListComputeRegionHealthChecks(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeRegionHealthCheck().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/regions/{{region}}/healthChecks")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeRegionHealthCheck(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeRegionHealthCheckFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

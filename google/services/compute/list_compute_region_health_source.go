...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeRegionHealthSourceListResource,
	}.Register()
}

var _ list.ListResource = &ComputeRegionHealthSourceListResource{}

type ComputeRegionHealthSourceListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeRegionHealthSourceListResource() list.ListResource {
	listR := &ComputeRegionHealthSourceListResource{}
	listR.TypeName = "google_compute_region_health_source"
	listR.SDKv2Resource = ResourceComputeRegionHealthSource()
...
// ComputeRegionHealthSourceListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeRegionHealthSourceListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeRegionHealthSourceListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeRegionHealthSourceListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeRegionHealthSources(
			listR.Client,
			region,
			project,
...
func ListComputeRegionHealthSources(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeRegionHealthSource().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/regions/{{region}}/healthSources")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeRegionHealthSource(),
		ListURL:        url,

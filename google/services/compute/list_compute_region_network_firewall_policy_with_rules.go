...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeRegionNetworkFirewallPolicyWithRulesListResource,
	}.Register()
}

var _ list.ListResource = &ComputeRegionNetworkFirewallPolicyWithRulesListResource{}

type ComputeRegionNetworkFirewallPolicyWithRulesListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeRegionNetworkFirewallPolicyWithRulesListResource() list.ListResource {
	listR := &ComputeRegionNetworkFirewallPolicyWithRulesListResource{}
	listR.TypeName = "google_compute_region_network_firewall_policy_with_rules"
	listR.SDKv2Resource = ResourceComputeRegionNetworkFirewallPolicyWithRules()
...
// ComputeRegionNetworkFirewallPolicyWithRulesListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeRegionNetworkFirewallPolicyWithRulesListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeRegionNetworkFirewallPolicyWithRulesListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeRegionNetworkFirewallPolicyWithRulesListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeRegionNetworkFirewallPolicyWithRuless(
			listR.Client,
			region,
			project,
...
func ListComputeRegionNetworkFirewallPolicyWithRuless(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeRegionNetworkFirewallPolicyWithRules().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVarsForId(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/regions/{{region}}/firewallPolicies")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeRegionNetworkFirewallPolicyWithRules(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "firewallPolicies",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			headers := make(http.Header)
			var err error
			res, err = resourceComputeRegionNetworkFirewallPolicyWithRulesDecoder(d, config, res)
			if err != nil {
				return err
			}
			if res == nil {
				return fmt.Errorf("error decoding ComputeRegionNetworkFirewallPolicyWithRules from list response")
			}
			...
			if err = ResourceComputeRegionNetworkFirewallPolicyWithRulesFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeRegionSslCertificateListResource,
	}.Register()
}

var _ list.ListResource = &ComputeRegionSslCertificateListResource{}

type ComputeRegionSslCertificateListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeRegionSslCertificateListResource() list.ListResource {
	listR := &ComputeRegionSslCertificateListResource{}
	listR.TypeName = "google_compute_region_ssl_certificate"
	listR.SDKv2Resource = ResourceComputeRegionSslCertificate()
...
// ComputeRegionSslCertificateListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeRegionSslCertificateListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeRegionSslCertificateListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeRegionSslCertificateListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeRegionSslCertificates(
			listR.Client,
			region,
			project,
...
func ListComputeRegionSslCertificates(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeRegionSslCertificate().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/regions/{{region}}/sslCertificates")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeRegionSslCertificate(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeRegionSslCertificateFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

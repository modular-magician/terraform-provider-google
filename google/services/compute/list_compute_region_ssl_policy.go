...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeRegionSslPolicyListResource,
	}.Register()
}

var _ list.ListResource = &ComputeRegionSslPolicyListResource{}

type ComputeRegionSslPolicyListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeRegionSslPolicyListResource() list.ListResource {
	listR := &ComputeRegionSslPolicyListResource{}
	listR.TypeName = "google_compute_region_ssl_policy"
	listR.SDKv2Resource = ResourceComputeRegionSslPolicy()
...
// ComputeRegionSslPolicyListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeRegionSslPolicyListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeRegionSslPolicyListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeRegionSslPolicyListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeRegionSslPolicys(
			listR.Client,
			region,
			project,
...
func ListComputeRegionSslPolicys(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeRegionSslPolicy().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/regions/{{region}}/sslPolicies")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeRegionSslPolicy(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeRegionSslPolicyFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeRegionTargetHttpProxyListResource,
	}.Register()
}

var _ list.ListResource = &ComputeRegionTargetHttpProxyListResource{}

type ComputeRegionTargetHttpProxyListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeRegionTargetHttpProxyListResource() list.ListResource {
	listR := &ComputeRegionTargetHttpProxyListResource{}
	listR.TypeName = "google_compute_region_target_http_proxy"
	listR.SDKv2Resource = ResourceComputeRegionTargetHttpProxy()
...
// ComputeRegionTargetHttpProxyListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeRegionTargetHttpProxyListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeRegionTargetHttpProxyListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeRegionTargetHttpProxyListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeRegionTargetHttpProxys(
			listR.Client,
			region,
			project,
...
func ListComputeRegionTargetHttpProxys(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeRegionTargetHttpProxy().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/regions/{{region}}/targetHttpProxies")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeRegionTargetHttpProxy(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "targetHttpProxies",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeRegionTargetHttpProxyFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeRegionTargetHttpsProxyListResource,
	}.Register()
}

var _ list.ListResource = &ComputeRegionTargetHttpsProxyListResource{}

type ComputeRegionTargetHttpsProxyListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeRegionTargetHttpsProxyListResource() list.ListResource {
	listR := &ComputeRegionTargetHttpsProxyListResource{}
	listR.TypeName = "google_compute_region_target_https_proxy"
	listR.SDKv2Resource = ResourceComputeRegionTargetHttpsProxy()
...
// ComputeRegionTargetHttpsProxyListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeRegionTargetHttpsProxyListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeRegionTargetHttpsProxyListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeRegionTargetHttpsProxyListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeRegionTargetHttpsProxys(
			listR.Client,
			region,
			project,
...
func ListComputeRegionTargetHttpsProxys(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeRegionTargetHttpsProxy().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/regions/{{region}}/targetHttpsProxies")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeRegionTargetHttpsProxy(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "targetHttpsProxies",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if res == nil {
				return fmt.Errorf("error decoding ComputeRegionTargetHttpsProxy from list response")
			}
			...
			if err = ResourceComputeRegionTargetHttpsProxyFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

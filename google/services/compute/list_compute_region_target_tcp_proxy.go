...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeRegionTargetTcpProxyListResource,
	}.Register()
}

var _ list.ListResource = &ComputeRegionTargetTcpProxyListResource{}

type ComputeRegionTargetTcpProxyListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeRegionTargetTcpProxyListResource() list.ListResource {
	listR := &ComputeRegionTargetTcpProxyListResource{}
	listR.TypeName = "google_compute_region_target_tcp_proxy"
	listR.SDKv2Resource = ResourceComputeRegionTargetTcpProxy()
...
// ComputeRegionTargetTcpProxyListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeRegionTargetTcpProxyListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeRegionTargetTcpProxyListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeRegionTargetTcpProxyListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeRegionTargetTcpProxys(
			listR.Client,
			region,
			project,
...
func ListComputeRegionTargetTcpProxys(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeRegionTargetTcpProxy().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/regions/{{region}}/targetTcpProxies")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeRegionTargetTcpProxy(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "targetTcpProxies",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeRegionTargetTcpProxyFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

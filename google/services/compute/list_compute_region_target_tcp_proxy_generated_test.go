...
func TestAccComputeRegionTargetTcpProxyListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					listScope.Capture(map[string]string{
						"region":  "google_compute_region_target_tcp_proxy.default",
						"project": "google_compute_region_target_tcp_proxy.default",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccComputeRegionTargetTcpProxy_regionTargetTcpProxyBasicExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_compute_region_target_tcp_proxy.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_compute_region_target_tcp_proxy.list_query",
...
func testAccComputeRegionTargetTcpProxy_regionTargetTcpProxyBasicExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "region" { type = string }
variable "project" { type = string }
list "google_compute_region_target_tcp_proxy" "list_query" {
    provider = google
	limit = 10000
    config {
        region = var.region

...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeResourcePolicyListResource,
	}.Register()
}

var _ list.ListResource = &ComputeResourcePolicyListResource{}

type ComputeResourcePolicyListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeResourcePolicyListResource() list.ListResource {
	listR := &ComputeResourcePolicyListResource{}
	listR.TypeName = "google_compute_resource_policy"
	listR.SDKv2Resource = ResourceComputeResourcePolicy()
...
// ComputeResourcePolicyListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeResourcePolicyListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeResourcePolicyListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeResourcePolicyListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeResourcePolicys(
			listR.Client,
			region,
			project,
...
func ListComputeResourcePolicys(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeResourcePolicy().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/regions/{{region}}/resourcePolicies")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeResourcePolicy(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeResourcePolicyFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

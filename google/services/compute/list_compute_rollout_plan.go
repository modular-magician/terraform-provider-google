...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeRolloutPlanListResource,
	}.Register()
}

var _ list.ListResource = &ComputeRolloutPlanListResource{}

type ComputeRolloutPlanListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeRolloutPlanListResource() list.ListResource {
	listR := &ComputeRolloutPlanListResource{}
	listR.TypeName = "google_compute_rollout_plan"
	listR.SDKv2Resource = ResourceComputeRolloutPlan()
...
// ComputeRolloutPlanListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeRolloutPlanListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeRolloutPlanListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeRolloutPlanListModel
...
func ListComputeRolloutPlans(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeRolloutPlan().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/global/rolloutPlans")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeRolloutPlan(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "rolloutPlans",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeRolloutPlanFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

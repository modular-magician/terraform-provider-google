...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeRouteListResource,
	}.Register()
}

var _ list.ListResource = &ComputeRouteListResource{}

type ComputeRouteListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeRouteListResource() list.ListResource {
	listR := &ComputeRouteListResource{}
	listR.TypeName = "google_compute_route"
	listR.SDKv2Resource = ResourceComputeRoute()
...
// ComputeRouteListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeRouteListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeRouteListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeRouteListModel
...
func ListComputeRoutes(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeRoute().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/global/routes")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if res == nil {
				return fmt.Errorf("error decoding ComputeRoute from list response")
			}
			...
			if err = ResourceComputeRouteFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeRouterListResource,
	}.Register()
}

var _ list.ListResource = &ComputeRouterListResource{}

type ComputeRouterListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeRouterListResource() list.ListResource {
	listR := &ComputeRouterListResource{}
	listR.TypeName = "google_compute_router"
	listR.SDKv2Resource = ResourceComputeRouter()
...
// ComputeRouterListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeRouterListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeRouterListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeRouterListModel
...
func ListComputeRouters(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeRouter().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/regions/{{region}}/routers")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeRouter(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeRouterFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeServiceAttachmentListResource,
	}.Register()
}

var _ list.ListResource = &ComputeServiceAttachmentListResource{}

type ComputeServiceAttachmentListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeServiceAttachmentListResource() list.ListResource {
	listR := &ComputeServiceAttachmentListResource{}
	listR.TypeName = "google_compute_service_attachment"
	listR.SDKv2Resource = ResourceComputeServiceAttachment()
...
// ComputeServiceAttachmentListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeServiceAttachmentListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeServiceAttachmentListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeServiceAttachmentListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeServiceAttachments(
			listR.Client,
			region,
			project,
...
func ListComputeServiceAttachments(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeServiceAttachment().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/regions/{{region}}/serviceAttachments")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeServiceAttachment(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "serviceAttachments",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeServiceAttachmentFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

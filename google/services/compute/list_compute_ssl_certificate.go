...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeSslCertificateListResource,
	}.Register()
}

var _ list.ListResource = &ComputeSslCertificateListResource{}

type ComputeSslCertificateListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeSslCertificateListResource() list.ListResource {
	listR := &ComputeSslCertificateListResource{}
	listR.TypeName = "google_compute_ssl_certificate"
	listR.SDKv2Resource = ResourceComputeSslCertificate()
...
// ComputeSslCertificateListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeSslCertificateListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeSslCertificateListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeSslCertificateListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeSslCertificates(
			listR.Client,
			project,
...
func ListComputeSslCertificates(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeSslCertificate().Data(&terraform.InstanceState{})
	if project != "" {
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeSslCertificate(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeSslCertificateFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

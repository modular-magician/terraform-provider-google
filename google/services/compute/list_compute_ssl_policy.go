...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeSslPolicyListResource,
	}.Register()
}

var _ list.ListResource = &ComputeSslPolicyListResource{}

type ComputeSslPolicyListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeSslPolicyListResource() list.ListResource {
	listR := &ComputeSslPolicyListResource{}
	listR.TypeName = "google_compute_ssl_policy"
	listR.SDKv2Resource = ResourceComputeSslPolicy()
...
// ComputeSslPolicyListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeSslPolicyListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeSslPolicyListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeSslPolicyListModel
...
func ListComputeSslPolicys(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeSslPolicy().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/global/sslPolicies")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeSslPolicy(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeSslPolicyFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

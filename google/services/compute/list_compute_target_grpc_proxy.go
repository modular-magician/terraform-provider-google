...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeTargetGrpcProxyListResource,
	}.Register()
}

var _ list.ListResource = &ComputeTargetGrpcProxyListResource{}

type ComputeTargetGrpcProxyListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeTargetGrpcProxyListResource() list.ListResource {
	listR := &ComputeTargetGrpcProxyListResource{}
	listR.TypeName = "google_compute_target_grpc_proxy"
	listR.SDKv2Resource = ResourceComputeTargetGrpcProxy()
...
// ComputeTargetGrpcProxyListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeTargetGrpcProxyListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeTargetGrpcProxyListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeTargetGrpcProxyListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeTargetGrpcProxys(
			listR.Client,
			project,
...
func ListComputeTargetGrpcProxys(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeTargetGrpcProxy().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/global/targetGrpcProxies")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeTargetGrpcProxy(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeTargetGrpcProxyFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

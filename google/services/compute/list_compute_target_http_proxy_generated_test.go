...
func TestAccComputeTargetHttpProxyListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					listScope.Capture(map[string]string{
						"project": "google_compute_target_http_proxy.default",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccComputeTargetHttpProxy_targetHttpProxyBasicExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_compute_target_http_proxy.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_compute_target_http_proxy.list_query",
...
func testAccComputeTargetHttpProxy_targetHttpProxyBasicExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "project" { type = string }
list "google_compute_target_http_proxy" "list_query" {
    provider = google
	limit = 10000
    config {
        project = var.project

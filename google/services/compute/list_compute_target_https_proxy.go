...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeTargetHttpsProxyListResource,
	}.Register()
}

var _ list.ListResource = &ComputeTargetHttpsProxyListResource{}

type ComputeTargetHttpsProxyListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeTargetHttpsProxyListResource() list.ListResource {
	listR := &ComputeTargetHttpsProxyListResource{}
	listR.TypeName = "google_compute_target_https_proxy"
	listR.SDKv2Resource = ResourceComputeTargetHttpsProxy()
...
// ComputeTargetHttpsProxyListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeTargetHttpsProxyListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeTargetHttpsProxyListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeTargetHttpsProxyListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeTargetHttpsProxys(
			listR.Client,
			project,
...
func ListComputeTargetHttpsProxys(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeTargetHttpsProxy().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/global/targetHttpsProxies")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeTargetHttpsProxy(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if res == nil {
				return fmt.Errorf("error decoding ComputeTargetHttpsProxy from list response")
			}
			...
			if err = ResourceComputeTargetHttpsProxyFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeUrlMapListResource,
	}.Register()
}

var _ list.ListResource = &ComputeUrlMapListResource{}

type ComputeUrlMapListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeUrlMapListResource() list.ListResource {
	listR := &ComputeUrlMapListResource{}
	listR.TypeName = "google_compute_url_map"
	listR.SDKv2Resource = ResourceComputeUrlMap()
...
// ComputeUrlMapListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeUrlMapListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeUrlMapListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeUrlMapListModel
...
func ListComputeUrlMaps(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeUrlMap().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/global/urlMaps")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeUrlMap(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeUrlMapFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeVpnTunnelListResource,
	}.Register()
}

var _ list.ListResource = &ComputeVpnTunnelListResource{}

type ComputeVpnTunnelListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeVpnTunnelListResource() list.ListResource {
	listR := &ComputeVpnTunnelListResource{}
	listR.TypeName = "google_compute_vpn_tunnel"
	listR.SDKv2Resource = ResourceComputeVpnTunnel()
...
// ComputeVpnTunnelListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeVpnTunnelListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeVpnTunnelListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeVpnTunnelListModel
...
func ListComputeVpnTunnels(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeVpnTunnel().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/regions/{{region}}/vpnTunnels")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeVpnTunnel(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeVpnTunnelFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

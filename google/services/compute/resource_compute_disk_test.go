...
func TestDiskKmsKeyDiffSuppress(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			if tpgcompute.DiskKmsKeyDiffSuppress("disk_encryption_key.0.kms_key_self_link", tc.Old, tc.New, nil) != tc.ExpectDiffSuppress {
				t.Fatalf("DiskKmsKeyDiffSuppress %q failed: old=%s, new=%s, expected=%v, got=%v",
...
func TestAccComputeDisk_kmsKeyVersionBehaviors(t *testing.T) {
	t.Parallel()

	diskName := fmt.Sprintf("tf-test-%s", acctest.RandString(t, 10))
	kms1 := kms.BootstrapKMSKey(t)
	kms2 := kms.BootstrapKMSKeyWithPurposeInLocationAndName(t, "ENCRYPT_DECRYPT", "global", "tftest-shared-key-2")
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		CheckDestroy:             testAccCheckComputeDiskDestroyProducer(t),
		Steps: []resource.TestStep{
			// Step 1: Create disk with base key
			{
				Config: testAccComputeDisk_encryptionKMS(diskName, kms1.CryptoKey.Name),
			},
			// Step 2: Change config to version 1 of the SAME key. Diff should be suppressed.
			{
				Config:             testAccComputeDisk_encryptionKMS(diskName, kms1.CryptoKey.Name+"/cryptoKeyVersions/1"),
				PlanOnly:           true,
				ExpectNonEmptyPlan: false,
			},
			// Step 3: Change to a DIFFERENT key WITH a version. Diff is not suppressed. API fails.
			{
				Config:      testAccComputeDisk_encryptionKMS(diskName, kms2.CryptoKey.Name+"/cryptoKeyVersions/1"),
				ExpectError: regexp.MustCompile("Changing the KMS key to a specific version is not supported\\."),
			},
		},
	})
}
func TestAccComputeDisk_updateKmsKey(t *testing.T) {
	t.Parallel()

	bootstrapped1 := kms.BootstrapKMSKey(t)
	bootstrapped2 := kms.BootstrapKMSKeyWithPurposeInLocationAndName(t, "ENCRYPT_DECRYPT", "global", "tftest-shared-key-2")
	pid := envvar.GetTestProjectFromEnv()
	diskName := fmt.Sprintf("tf-test-%s", acctest.RandString(t, 10))
	importID := fmt.Sprintf("%s/%s/%s", pid, "us-central1-a", diskName)
	var disk map[string]interface{}
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		CheckDestroy:             testAccCheckComputeDiskDestroyProducer(t),
		Steps: []resource.TestStep{
			// Step 1: Create disk with CMEK Key 1
			{
				Config: testAccComputeDisk_encryptionKMS(diskName, bootstrapped1.CryptoKey.Name),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeDiskExists(
						t, "google_compute_disk.foobar", pid, &disk),
					resource.TestCheckResourceAttr(
						"google_compute_disk.foobar", "disk_encryption_key.0.kms_key_self_link", bootstrapped1.CryptoKey.Name),
				),
			},
			// Step 2: Update in-place to change CMEK to Key 2
			{
				Config: testAccComputeDisk_encryptionKMS(diskName, bootstrapped2.CryptoKey.Name),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeDiskExists(
						t, "google_compute_disk.foobar", pid, &disk),
					resource.TestCheckResourceAttr(
						"google_compute_disk.foobar", "disk_encryption_key.0.kms_key_self_link", bootstrapped2.CryptoKey.Name),
				),
			},
			{
				ResourceName:      "google_compute_disk.foobar",
...
func TestAccComputeDisk_disallowCmekToGmek(t *testing.T) {
	t.Parallel()
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		CheckDestroy:             testAccCheckComputeDiskDestroyProducer(t),
		Steps: []resource.TestStep{
			// Step 1: Create disk with CMEK
			{
				Config: testAccComputeDisk_encryptionKMS(diskName, bootstrapped.CryptoKey.Name),
			},
			// Step 2: Attempt removing CMEK (downgrade to GMEK) - expect validation error
			{
				Config:      testAccComputeDisk_noEncryption(diskName),
				ExpectError: regexp.MustCompile("Removing 'disk_encryption_key' is not supported in-place. To remove the KMS key, please destroy and recreate the disk without a KMS key."),
			},
		},
	})
}
...
func testAccComputeDisk_noEncryption(diskName string) string {
	return fmt.Sprintf(`
data "google_compute_image" "my_image" {
  family  = "debian-11"
  project = "debian-cloud"

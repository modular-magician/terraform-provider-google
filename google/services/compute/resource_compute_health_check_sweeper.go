...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionComputeHealthCheck,
		DeleteFunction: testSweepComputeHealthCheck,
	}
	// Add dependencies
...
func testSweepComputeHealthCheck(_ string) error {
	return listAndActionComputeHealthCheck(deleteResourceComputeHealthCheck)
}

func listAndActionComputeHealthCheck(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://compute.googleapis.com/compute/v1/projects/{{project}}/global/healthChecks", "?")[0]
...
func deleteResourceComputeHealthCheck(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://compute.googleapis.com/compute/v1/projects/{{project}}/global/healthChecks/{{name}}"

...
//
//	Source file: https://github.com/GoogleCloudPlatform/magic-modules/tree/main/mmv1/third_party/terraform/services/compute/resource_compute_instance_template_test.go.tmpl
//
...
func TestAccComputeInstance_IPv6ExternalRecreate(t *testing.T) {
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccComputeInstanceIPv6External(acctest.RandString(t, 10), false),
				Check: resource.ComposeTestCheckFunc(
...
func testAccComputeInstanceIPv6External(suffix string, useNewIPv6 bool) string {
	ipv6Ref := "google_compute_address.old_static_external_ipv6.address"
	if useNewIPv6 {
		ipv6Ref = "google_compute_address.new_static_external_ipv6.address"
	}
...
resource "google_compute_network" "test_vpc" {
  name                     = "tf-test-vpc-dual-stack-%s"
  auto_create_subnetworks  = false
  enable_ula_internal_ipv6 = true
}

resource "google_compute_subnetwork" "dual_stack" {
  name          = "tf-test-subnet-dual-stack-%s"
  region        = "us-central1"
...
resource "google_compute_address" "old_static_external_ipv6" {
  name               = "tf-test-ipv6-old-%s"
  region             = "us-central1"
  subnetwork         = google_compute_subnetwork.dual_stack.id
  ip_version         = "IPV6"
  ipv6_endpoint_type = "VM"
}

resource "google_compute_address" "new_static_external_ipv6" {
  name               = "tf-test-ipv6-new-%s"
  region             = "us-central1"
  subnetwork         = google_compute_subnetwork.dual_stack.id
  ip_version         = "IPV6"
  ipv6_endpoint_type = "VM"
...
`, suffix, suffix, suffix, suffix, suffix, ipv6Ref)
}

...
func TestAccComputeInstance_ServiceClassNetworkAttachment(t *testing.T) {
	t.Parallel()
	suffix := fmt.Sprintf("%s", acctest.RandString(t, 10))
	envRegion := "us-central1"
	var instance map[string]interface{}
	...
	// Need to have the full network attachment name in the format project/{project_id}/regions/{region_id}/networkAttachments/{testNetworkAttachmentName}
	fullFormNetworkAttachmentName := fmt.Sprintf("projects/%s/regions/%s/networkAttachments/%s", envvar.GetTestProjectFromEnv(), envRegion, testNetworkAttachmentName)
	testServiceClassId := "gcp-tf-persistent-service-class-ga"

	// we use different ServiceClasses between Vcr and nightly
	if acctest.IsVcrEnabled() {
		testServiceClassId = "gcp-tf-persistent-service-class-dev"
	}
	...
	context := map[string]interface{}{
		...
		"service_class_id":        testServiceClassId,
	}
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccComputeInstance_serviceClassNetworkAttachment(context),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeInstanceExists(
						t, "google_compute_instance.foobar", &instance),
					testAccCheckComputeInstanceHasNetworkAttachment(&instance, fmt.Sprintf("https://www.googleapis.com/compute/%s/%s", providerVersion, fullFormNetworkAttachmentName)),
...
func testAccComputeInstance_serviceClassNetworkAttachment(context map[string]interface{}) string {
	return acctest.Nprintf(`
resource "google_project" "accepted_consumer_project1" {
  project_id      = "tf-test-prj-accept1-%{suffix}"
  name            = "tf-test-prj-accept1-%{suffix}"
  org_id          = "%{org_id}"
...
resource "google_project_service" "compute_service" {
  project = google_project.accepted_consumer_project1.project_id
  service = "compute.googleapis.com"
}
...
resource "google_compute_address" "producer_address" {
  name = "tf-test-basic-address%{suffix}"
  network_attachment = google_compute_network_attachment.test_network_attachment.id

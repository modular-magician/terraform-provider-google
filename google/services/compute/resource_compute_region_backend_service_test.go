...
func TestAccComputeRegionBackendService_zonalILB(t *testing.T) {
	...
	// The name for the network endpoint group was missing.
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccComputeRegionBackendService_zonalILB(serviceName, checkName, negName),
			},
			{
				ResourceName:      "google_compute_region_backend_service.test",
				ImportState:       true,
				ImportStateVerify: true,
			},
			{
				Config: testAccComputeRegionBackendService_zonalILBModified(serviceName, checkName, checkName2, negName),
			},
			{
				ResourceName:      "google_compute_region_backend_service.test",
				ImportState:       true,
...
func testAccComputeRegionBackendService_zonalILB(serviceName, checkName, negName string) string {
	return fmt.Sprintf(`
...
// FIX: Changed resource type to google_compute_region_health_check
...
`, checkName, negName, serviceName)
}

func testAccComputeRegionBackendService_zonalILBModified(serviceName, checkName, _ string, negName string) string {
	return fmt.Sprintf(`
resource "google_compute_network" "default" {
  name = "tf-test-net"
  auto_create_subnetworks = false

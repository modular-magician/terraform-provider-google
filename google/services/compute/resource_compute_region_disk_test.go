...
func TestAccComputeRegionDisk_kmsKeyVersionBehaviors(t *testing.T) {
	t.Parallel()
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccComputeRegionDisk_updateKmsKey_step2(diskName, kms1.CryptoKey.Name),
			},
			{
				Config:             testAccComputeRegionDisk_updateKmsKey_step2(diskName, kms1.CryptoKey.Name+"/cryptoKeyVersions/1"),
				PlanOnly:           true,
				ExpectNonEmptyPlan: false,
			},
			{
				Config:      testAccComputeRegionDisk_updateKmsKey_step2(diskName, kms2.CryptoKey.Name+"/cryptoKeyVersions/1"),
				ExpectError: regexp.MustCompile("Changing the KMS key to a specific version is not supported\\."),
			},
		},
	})
}
func TestAccComputeRegionDisk_updateKmsKey(t *testing.T) {
	t.Parallel()
	...
	suffix := acctest.RandString(t, 10)
	diskName := fmt.Sprintf("tf-test-kms-rdisk-%s", suffix)
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		CheckDestroy:             testAccCheckComputeRegionDiskDestroyProducer(t),
		Steps: []resource.TestStep{
			// Step 1: Create regional disk with CMEK Key 1
			{
				Config: testAccComputeRegionDisk_updateKmsKey_step2(diskName, bootstrapped1.CryptoKey.Name),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeRegionDiskExists(
						t, "google_compute_region_disk.foobar", &disk),
					resource.TestCheckResourceAttr(
						"google_compute_region_disk.foobar", "disk_encryption_key.0.kms_key_name", bootstrapped1.CryptoKey.Name),
				),
			},
			// Step 2: Update in-place to change CMEK to Key 2
			{
				Config: testAccComputeRegionDisk_updateKmsKey_step2(diskName, bootstrapped2.CryptoKey.Name),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeRegionDiskExists(
						t, "google_compute_region_disk.foobar", &disk),
					resource.TestCheckResourceAttr(
						"google_compute_region_disk.foobar", "disk_encryption_key.0.kms_key_name", bootstrapped2.CryptoKey.Name),
				),
			},
			{
...
func TestAccComputeRegionDisk_disallowCmekToGmek(t *testing.T) {
	t.Parallel()
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccComputeRegionDisk_updateKmsKey_step2(diskName, bootstrapped.CryptoKey.Name),
			},
			{
				Config:      testAccComputeRegionDisk_updateKmsKey_step1(diskName),
				ExpectError: regexp.MustCompile("Removing 'disk_encryption_key' is not supported in-place. To remove the KMS key, please destroy and recreate the regional disk without a KMS key."),
			},
		},
	})
}

func testAccComputeRegionDisk_updateKmsKey_step1(diskName string) string {
	return fmt.Sprintf(`
resource "google_compute_region_disk" "foobar" {
  name          = "%s"
  type          = "pd-ssd"
...
func testAccComputeRegionDisk_updateKmsKey_step2(diskName, kmsKey string) string {
	return fmt.Sprintf(`
resource "google_compute_region_disk" "foobar" {
  name          = "%s"
  type          = "pd-ssd"

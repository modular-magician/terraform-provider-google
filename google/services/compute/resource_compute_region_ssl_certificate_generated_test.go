...
func TestAccComputeRegionSslCertificate_regionSslCertificateBasicWriteonlyExample(t *testing.T) {
	t.Parallel()

	randomSuffix := acctest.RandString(t, 10)

	context := map[string]interface{}{
		"cert_name":     "tf-test-my-certificate" + randomSuffix,
		"random_suffix": randomSuffix,
	}
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccComputeRegionSslCertificate_regionSslCertificateBasicWriteonlyExample(context),
			},
			{
				ResourceName:            "google_compute_region_ssl_certificate.default",
				ImportState:             true,
...
func testAccCheckComputeRegionSslCertificateDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			...
			url, err := tpgresource.ReplaceVarsForTest(config, rs, transport_tpg.BaseUrl(compute.Product, config)+"projects/{{project}}/regions/{{region}}/sslCertificates/{{name}}")
			if err != nil {
				return err
			}

...
func testAccComputeServiceAttachment_serviceAttachmentBasicExampleUpdateReordered(context map[string]interface{}) string {
	// Swaps the order of consumer_reject_lists to verify that Sets do not track order
	return acctest.Nprintf(`
resource "google_compute_service_attachment" "psc_ilb_service_attachment" {
  name        = "tf-test-my-psc-ilb%{random_suffix}"
  region      = "us-west2"
  ...
  # Order is deliberately swapped compared to the previous step
  consumer_reject_lists = ["482878270665", "673497134629"]

...
func TestAccComputeSslCertificate_sslCertificateBasicWriteonlyExample(t *testing.T) {
	t.Parallel()
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccComputeSslCertificate_sslCertificateBasicWriteonlyExample(context),
			},
			{
				ResourceName:            "google_compute_ssl_certificate.default",
				ImportState:             true,
...
func testAccCheckComputeSslCertificateDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			...
			url, err := tpgresource.ReplaceVarsForTest(config, rs, transport_tpg.BaseUrl(compute.Product, config)+"projects/{{project}}/global/sslCertificates/{{name}}")
			if err != nil {
				return err
			}

...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionConfigDeploymentGroup,
		DeleteFunction: testSweepConfigDeploymentGroup,
	}
...
func testSweepConfigDeploymentGroup(_ string) error {
	return listAndActionConfigDeploymentGroup(deleteResourceConfigDeploymentGroup)
}

func listAndActionConfigDeploymentGroup(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ConfigDeploymentGroup"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://config.googleapis.com/v1/projects/{{project}}/locations/{{location}}/deploymentGroups", "?")[0]
		...
		resourceList, ok := res["deploymentGroups"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'deploymentGroups'")
		} else {
...
func deleteResourceConfigDeploymentGroup(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://config.googleapis.com/v1/projects/{{project}}/locations/{{location}}/deploymentGroups/{{name}}?deploymentReferencePolicy={{deployment_reference_policy}}&force={{force_destroy}}"

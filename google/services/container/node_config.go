					...
					Elem: &schema.Resource{
						Schema: map[string]*schema.Schema{
							...
							"disk_io_scheduler": {
								...
								Optional:    true,
								MaxItems:    1,
								Description: `Controls the configuration for the disk IO scheduler.`,
								Elem: &schema.Resource{
									Schema: map[string]*schema.Schema{
										"node_system_io_scheduler": {
											Type:        schema.TypeString,
											Optional:    true,
											Description: `Configures the IO scheduler for the boot disk or ephemeral lssd that runs node system workloads.`,
										},
										"node_attached_disk_io_scheduler": {
											Type:        schema.TypeString,
											Optional:    true,
											Description: `Configures the IO scheduler for the attached disks.`,
										},
									},
								},
							},
							"node_vfio_config": {
								...
								Optional:    true,
								MaxItems:    1,
								Description: `Contains VFIO-related configurations for this node.`,
								Elem: &schema.Resource{
									Schema: map[string]*schema.Schema{
										"dma_entry_limit": {
											Type:         schema.TypeInt,
											Optional:     true,
											ValidateFunc: validation.IntBetween(65535, 4194304),
											Description:  `Specifies the maximum number of DMA entries (pages) that can be mapped by the VFIO IOMMU type 1 driver for a container.`,
										},
									},
								},
							},
							...
							"swap_config": {
								...
								Optional:    true,
								MaxItems:    1,
								Description: `Swap configuration for the node.`,
								Elem: &schema.Resource{
									Schema: map[string]*schema.Schema{
										"boot_disk_profile": {
											Type:        schema.TypeList,
										...
										"enabled": {
											Type:        schema.TypeBool,
											Optional:    true,
											Description: `Enables or disables swap for the node pool.`,
										},
										"encryption_config": {
											Type:        schema.TypeList,
											Optional:    true,
											MaxItems:    1,
											Description: `If omitted, swap space is encrypted by default.`,
											Elem: &schema.Resource{
												Schema: map[string]*schema.Schema{
													"disabled": {
														Type:        schema.TypeBool,
														Optional:    true,
														Description: `If true, swap space will not be encrypted. Defaults to false (encrypted).`,
													},
												},
											},
										},
...
func expandLinuxNodeConfig(v interface{}) *container.LinuxNodeConfig {
	...
	if v, ok := cfg["disk_io_scheduler"]; ok {
		linuxNodeConfig.DiskIoScheduler = expandDiskIoScheduler(v)
	}

	if v, ok := cfg["node_vfio_config"]; ok {
		linuxNodeConfig.NodeVfioConfig = expandNodeVfioConfig(v)
	}
...
func expandDiskIoScheduler(v interface{}) *container.DiskIoScheduler {
	if v == nil {
		return nil
	}
	ls := v.([]interface{})
	...
	if ls[0] == nil {
		return &container.DiskIoScheduler{}
	}
	cfg := ls[0].(map[string]interface{})

	diskIoScheduler := &container.DiskIoScheduler{}

	if v, ok := cfg["node_system_io_scheduler"]; ok {
		diskIoScheduler.NodeSystemIoScheduler = v.(string)
	}

	if v, ok := cfg["node_attached_disk_io_scheduler"]; ok {
		diskIoScheduler.NodeAttachedDiskIoScheduler = v.(string)
	}

	return diskIoScheduler
}

func expandNodeVfioConfig(v interface{}) *container.NodeVfioConfig {
	if v == nil {
		return nil
	}
	ls := v.([]interface{})
	...
	if ls[0] == nil {
		return &container.NodeVfioConfig{}
	}
	cfg := ls[0].(map[string]interface{})

	nodeVfioConfig := &container.NodeVfioConfig{}

	if v, ok := cfg["dma_entry_limit"]; ok {
		nodeVfioConfig.DmaEntryLimit = int64(v.(int))
	}
...
func expandSwapConfig(v interface{}) *container.SwapConfig {
	...
	if v, ok := cfg["boot_disk_profile"]; ok && len(v.([]interface{})) > 0 {
		swapConfig.BootDiskProfile = expandBootDiskProfile(v)
	}

	if v, ok := cfg["dedicated_local_ssd_profile"]; ok && len(v.([]interface{})) > 0 {
	...
	if v, ok := cfg["encryption_config"]; ok && len(v.([]interface{})) > 0 {
		swapConfig.EncryptionConfig = expandSwapEncryptionConfig(v)
	}

	if v, ok := cfg["ephemeral_local_ssd_profile"]; ok && len(v.([]interface{})) > 0 {
...
func expandBootDiskProfile(v interface{}) *container.BootDiskProfile {
	...
	profile := &container.BootDiskProfile{}

	if v, ok := cfg["swap_size_gib"]; ok {
		profile.SwapSizeGib = int64(v.(int))
	}

	if v, ok := cfg["swap_size_percent"]; ok {
		profile.SwapSizePercent = int64(v.(int))
	}
...
func expandSwapEncryptionConfig(v interface{}) *container.EncryptionConfig {
	if v == nil {
		return nil
	}
	ls := v.([]interface{})
	...
	if ls[0] == nil {
		return &container.EncryptionConfig{}
	}
	cfg := ls[0].(map[string]interface{})

	config := &container.EncryptionConfig{}

	if v, ok := cfg["disabled"]; ok {
		config.Disabled = v.(bool)
		if !config.Disabled {
			config.ForceSendFields = append(config.ForceSendFields, "Disabled")
		}
	}
...
func flattenLinuxNodeConfig(c *container.LinuxNodeConfig) []map[string]interface{} {
	...
	if c != nil {
		result = append(result, map[string]interface{}{
			...
			"disk_io_scheduler":            flattenDiskIoScheduler(c.DiskIoScheduler),
			"node_vfio_config":             flattenNodeVfioConfig(c.NodeVfioConfig),
		})
	}
	return result
}

func flattenDiskIoScheduler(c *container.DiskIoScheduler) []map[string]interface{} {
	result := []map[string]interface{}{}
	if c != nil {
		result = append(result, map[string]interface{}{
			"node_system_io_scheduler":        c.NodeSystemIoScheduler,
			"node_attached_disk_io_scheduler": c.NodeAttachedDiskIoScheduler,
		})
	}
	return result
}

func flattenNodeVfioConfig(c *container.NodeVfioConfig) []map[string]interface{} {
	result := []map[string]interface{}{}
	if c != nil {
		result = append(result, map[string]interface{}{
			"dma_entry_limit": c.DmaEntryLimit,
		})
	}
	return result
}
...
func flattenSwapConfig(c *container.SwapConfig) []map[string]interface{} {
	...
	if c != nil {
		result = append(result, map[string]interface{}{
			"enabled":                     c.Enabled,
			"boot_disk_profile":           flattenBootDiskProfile(c.BootDiskProfile),
			"dedicated_local_ssd_profile": flattenDedicatedLocalSsdProfile(c.DedicatedLocalSsdProfile),
			"ephemeral_local_ssd_profile": flattenEphemeralLocalSsdProfile(c.EphemeralLocalSsdProfile),
			"encryption_config":           flattenSwapEncryptionConfig(c.EncryptionConfig),
		})
	}
	return result
}

func flattenBootDiskProfile(c *container.BootDiskProfile) []map[string]interface{} {
	result := []map[string]interface{}{}
	if c != nil {
		result = append(result, map[string]interface{}{
			"swap_size_gib":     c.SwapSizeGib,
			"swap_size_percent": c.SwapSizePercent,
		})
	}
	return result
}

func flattenDedicatedLocalSsdProfile(c *container.DedicatedLocalSsdProfile) []map[string]interface{} {
	result := []map[string]interface{}{}
	if c != nil {
		result = append(result, map[string]interface{}{
			"disk_count": c.DiskCount,
		})
	}
	return result
}

func flattenEphemeralLocalSsdProfile(c *container.EphemeralLocalSsdProfile) []map[string]interface{} {
	result := []map[string]interface{}{}
	if c != nil {
		result = append(result, map[string]interface{}{
			"swap_size_gib":     c.SwapSizeGib,
...
func flattenSwapEncryptionConfig(c *container.EncryptionConfig) []map[string]interface{} {
	result := []map[string]interface{}{}
	if c != nil {
		result = append(result, map[string]interface{}{
			"disabled": c.Disabled,

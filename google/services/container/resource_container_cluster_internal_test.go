...
func TestContainerCluster_expandControlPlaneEgress(t *testing.T) {
	t.Parallel()

	cases := []struct {
		name  string
		input []interface{}
		want  *container.ControlPlaneEgress
	}{
		{
			name:  "nil input",
			input: nil,
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := expandControlPlaneEgress(tc.input)
			if diff := cmp.Diff(tc.want, got); diff != "" {
				t.Errorf("expandControlPlaneEgress() mismatch (-want +got):\n%s", diff)
			}
		})
	}
}

func TestContainerCluster_flattenControlPlaneEgress(t *testing.T) {
	t.Parallel()

	cases := []struct {
		name  string
		input *container.ControlPlaneEgress
		want  []map[string]interface{}
	}{
		{
			name:  "nil",
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := flattenControlPlaneEgress(tc.input)
			if diff := cmp.Diff(tc.want, got); diff != "" {
				t.Errorf("flattenControlPlaneEgress() mismatch (-want +got):\n%s", diff)
			}
		})
	}
}

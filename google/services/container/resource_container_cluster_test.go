...
func TestAccContainerCluster_with_PodCIDROverprovision(t *testing.T) {
	t.Parallel()
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccContainerCluster_with_PodCIDROverprovision(containerNetName, clusterName, true),
				Check: resource.ComposeTestCheckFunc(
			...
			{
				Config: testAccContainerCluster_with_PodCIDROverprovision(containerNetName, clusterName, false),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "ip_allocation_policy.0.pod_cidr_overprovision_config.0.disabled", "false"),
				),
			},
		},
	})
...
func testAccContainerCluster_with_PodCIDROverprovision(containerNetName string, clusterName string, disabled bool) string {
	return fmt.Sprintf(`
resource "google_compute_network" "container_network" {
  name                    = "%s"
  auto_create_subnetworks = false
...
`, containerNetName, clusterName, disabled)
}

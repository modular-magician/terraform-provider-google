...
func TestAccContainerCluster_WithCPAFeatures(t *testing.T) {
	...
	diskKey := acctest.BootstrapKMSKeyInLocation(t, "us-central1")
	signingKey := acctest.BootstrapKMSKeyInLocation(t, "us-central1")
	verificationKey := acctest.BootstrapKMSKeyInLocation(t, "us-central1")
	backupKey := acctest.BootstrapKMSKeyInLocation(t, "us-central1")
	if acctest.BootstrapPSARoles(t, "service-", "container-engine-robot", []string{"roles/container.cloudKmsKeyUser", "roles/privateca.certificateManager"}) {
		t.Fatal("Stopping the test because a role was added to the policy.")
	}
	...
	context := map[string]interface{}{
		...
		"signing_key":      signingKey.CryptoKey.Primary.Name,
		"verification_key": verificationKey.CryptoKey.Primary.Name,
		...
		Steps: []resource.TestStep{
			{
				...
				// TODO(b/372305432): check that cluster resource is created successfully.
				ExpectError: regexp.MustCompile(`certificate authority pool location .* is not in the same region as cluster location .*`),
			},
		},
	})
}
...
resource "google_privateca_ca_pool" "cluster_ca" {
  ...
  // TODO(b/372305432): use the same region as the cluster.
  location = "us-west1"
  tier = "ENTERPRISE"
  publishing_options {

...
func TestAccContainerCluster_withShutdownGracePeriodConfig(t *testing.T) {
	t.Parallel()
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccContainerCluster_withShutdownGracePeriodConfig(clusterName, nodePoolName, networkName, subnetworkName, 120, 30),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("google_container_cluster.with_shutdown_grace_period",
			...
			{
				ResourceName:            "google_container_cluster.with_shutdown_grace_period",
				ImportState:             true,
				ImportStateVerify:       true,
				ImportStateVerifyIgnore: []string{"deletion_protection"},
			},
			{
				Config: testAccContainerCluster_withShutdownGracePeriodConfig(clusterName, nodePoolName, networkName, subnetworkName, 30, 15),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("google_container_cluster.with_shutdown_grace_period",
						"node_pool.0.node_config.0.kubelet_config.0.shutdown_grace_period_seconds", "30"),
					resource.TestCheckResourceAttr("google_container_cluster.with_shutdown_grace_period",
						"node_pool.0.node_config.0.kubelet_config.0.shutdown_grace_period_critical_pods_seconds", "15"),
				),
			},
			{
				ResourceName:            "google_container_cluster.with_shutdown_grace_period",
...
func testAccContainerCluster_withShutdownGracePeriodConfig(clusterName, nodePoolName, networkName, subnetworkName string, gracePeriod, criticalGracePeriod int) string {
	return fmt.Sprintf(`
resource "google_container_cluster" "with_shutdown_grace_period" {
  name                = %q
  location            = "us-central1-a"
  deletion_protection = false
  network             = %q
...
`, clusterName, networkName, subnetworkName, nodePoolName, gracePeriod, criticalGracePeriod)
}

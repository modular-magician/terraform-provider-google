...
func TestAccContainerNodePool_withShutdownGracePeriodConfig(t *testing.T) {
	t.Parallel()
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccContainerNodePool_withShutdownGracePeriodConfig(cluster, np, networkName, subnetworkName, 120, 30),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("google_container_node_pool.with_shutdown_grace_period",
			...
			{
				ResourceName:      "google_container_node_pool.with_shutdown_grace_period",
				ImportState:       true,
				ImportStateVerify: true,
			},
			{
				Config: testAccContainerNodePool_withShutdownGracePeriodConfig(cluster, np, networkName, subnetworkName, 30, 15),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("google_container_node_pool.with_shutdown_grace_period",
						"node_config.0.kubelet_config.0.shutdown_grace_period_seconds", "30"),
					resource.TestCheckResourceAttr("google_container_node_pool.with_shutdown_grace_period",
...
func testAccContainerNodePool_withShutdownGracePeriodConfig(cluster, np, networkName, subnetworkName string, gracePeriod, criticalGracePeriod int) string {
	return fmt.Sprintf(`
resource "google_container_cluster" "cluster" {
  name                = "%s"
  location            = "us-central1-a"
...
# shutdown_grace_period_seconds and shutdown_grace_period_critical_pods_seconds
# are only valid for Spot VMs.
resource "google_container_node_pool" "with_shutdown_grace_period" {
  name               = "%s"
  location           = "us-central1-a"
  cluster            = google_container_cluster.cluster.name
  initial_node_count = 1
...
`, cluster, networkName, subnetworkName, np, gracePeriod, criticalGracePeriod)
}

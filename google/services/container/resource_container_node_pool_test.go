...
func TestAccContainerNodePool_withoutCpuCfsQuota(t *testing.T) {
	t.Parallel()
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccContainerNodePool_withoutCpuCfsQuota(cluster, networkName, subnetworkName),
				Check: resource.ComposeTestCheckFunc(
					// resource.TestCheckResourceAttr("google_container_node_pool.with_kubelet_config",
					// 	"node_config.0.kubelet_config.0.cpu_cfs_quota", "true"),
...
func testAccContainerNodePool_withoutCpuCfsQuota(cluster, networkName, subnetworkName string) string {
	return fmt.Sprintf(`
resource "google_container_cluster" "primary" {
  name               = "%s"
  location           = "us-central1-a"

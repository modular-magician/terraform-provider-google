...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_database_migration_service_conversion_workspace",
		ListAndAction:  listAndActionDatabaseMigrationServiceConversionWorkspace,
		DeleteFunction: testSweepDatabaseMigrationServiceConversionWorkspace,
	}
...
func testSweepDatabaseMigrationServiceConversionWorkspace(_ string) error {
	return listAndActionDatabaseMigrationServiceConversionWorkspace(deleteResourceDatabaseMigrationServiceConversionWorkspace)
}

func listAndActionDatabaseMigrationServiceConversionWorkspace(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "DatabaseMigrationServiceConversionWorkspace"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://datamigration.googleapis.com/v1/projects/{{project}}/locations/{{location}}/conversionWorkspaces", "?")[0]
		...
		resourceList, ok := res["conversionWorkspaces"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'conversionWorkspaces'")
		} else {
...
func deleteResourceDatabaseMigrationServiceConversionWorkspace(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://datamigration.googleapis.com/v1/projects/{{project}}/locations/{{location}}/conversionWorkspaces/{{conversion_workspace_id}}"

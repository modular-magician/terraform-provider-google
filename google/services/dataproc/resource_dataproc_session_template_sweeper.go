...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_dataproc_session_template",
		ListAndAction:  listAndActionDataprocSessionTemplate,
		DeleteFunction: testSweepDataprocSessionTemplate,
	}
...
func testSweepDataprocSessionTemplate(_ string) error {
	return listAndActionDataprocSessionTemplate(deleteResourceDataprocSessionTemplate)
}

func listAndActionDataprocSessionTemplate(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "DataprocSessionTemplate"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://dataproc.googleapis.com/v1/projects/{{project}}/locations/{{location}}/sessionTemplates", "?")[0]
		...
		resourceList, ok := res["sessionTemplates"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'sessionTemplates'")
		} else {
...
func deleteResourceDataprocSessionTemplate(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {

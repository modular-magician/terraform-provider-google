...
func TestAccMetastoreService_tags(t *testing.T) {
	...
	resourceName := acctest.Nprintf("google_metastore_service.%{resource_name}", context)
	org := envvar.GetTestOrgFromEnv(t)

	acctest.VcrTest(t, resource.TestCase{
		PreCheck:                 func() { acctest.AccTestPreCheck(t) },
		ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories(t),
		CheckDestroy:             testAccCheckMetastoreServiceDestroyProducer(t),
		Steps: []resource.TestStep{
			{
				Config: testAccMetastoreServiceTags(context, map[string]string{org + "/env": "test"}),
			},
			{
			...
			{
				Config: testAccMetastoreServiceTags_allowDestroy(context, map[string]string{org + "/env": "test"}),
			},
		},
	})
}
...
func testAccMetastoreServiceTags_allowDestroy(context map[string]interface{}, tags map[string]string) string {

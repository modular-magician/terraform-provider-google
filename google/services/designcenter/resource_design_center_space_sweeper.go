...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionDesignCenterSpace,
		DeleteFunction: testSweepDesignCenterSpace,
	}
...
func testSweepDesignCenterSpace(_ string) error {
	return listAndActionDesignCenterSpace(deleteResourceDesignCenterSpace)
}

func listAndActionDesignCenterSpace(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "DesignCenterSpace"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://designcenter.googleapis.com/v1/projects/{{project}}/locations/{{location}}/spaces", "?")[0]
		...
		resourceList, ok := res["spaces"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'spaces'")
		} else {
...
func deleteResourceDesignCenterSpace(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://designcenter.googleapis.com/v1/projects/{{project}}/locations/{{location}}/spaces/{{space_id}}"

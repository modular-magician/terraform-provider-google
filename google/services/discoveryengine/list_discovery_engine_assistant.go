...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewDiscoveryEngineAssistantListResource,
	}.Register()
}

var _ list.ListResource = &DiscoveryEngineAssistantListResource{}

type DiscoveryEngineAssistantListResource struct {
	tpgresource.ListResourceMetadata
}

func NewDiscoveryEngineAssistantListResource() list.ListResource {
	listR := &DiscoveryEngineAssistantListResource{}
	listR.TypeName = "google_discovery_engine_assistant"
	listR.SDKv2Resource = ResourceDiscoveryEngineAssistant()
...
// DiscoveryEngineAssistantListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type DiscoveryEngineAssistantListModel struct {
...
func (listR *DiscoveryEngineAssistantListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data DiscoveryEngineAssistantListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListDiscoveryEngineAssistants(
			listR.Client,
			location,
			collectionId,
			engineId,
			...
			func(rd *schema.ResourceData) error {
				...
				if err := listR.SetResult(ctx, listReq.IncludeResource, &result, rd, "name", "assistant_id"); err != nil {
					return err
				}
...
func ListDiscoveryEngineAssistants(config *transport_tpg.Config,
	location string,
	collectionId string,
	engineId string,
	project string,
	...
	resourceData := ResourceDiscoveryEngineAssistant().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{DiscoveryEngineBasePath}}projects/{{project}}/locations/{{location}}/collections/{{collection_id}}/engines/{{engine_id}}/assistants")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceDiscoveryEngineAssistant(),
		ListURL:        url,

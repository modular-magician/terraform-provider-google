...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewDiscoveryEngineDataStoreListResource,
	}.Register()
}

var _ list.ListResource = &DiscoveryEngineDataStoreListResource{}

type DiscoveryEngineDataStoreListResource struct {
	tpgresource.ListResourceMetadata
}

func NewDiscoveryEngineDataStoreListResource() list.ListResource {
	listR := &DiscoveryEngineDataStoreListResource{}
	listR.TypeName = "google_discovery_engine_data_store"
	listR.SDKv2Resource = ResourceDiscoveryEngineDataStore()
...
// DiscoveryEngineDataStoreListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type DiscoveryEngineDataStoreListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *DiscoveryEngineDataStoreListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data DiscoveryEngineDataStoreListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListDiscoveryEngineDataStores(
			listR.Client,
			location,
			project,
			func(rd *schema.ResourceData) error {
				...
				if err := listR.SetResult(ctx, listReq.IncludeResource, &result, rd, "name", "data_store_id"); err != nil {
					return err
				}
...
func ListDiscoveryEngineDataStores(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceDiscoveryEngineDataStore().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{DiscoveryEngineBasePath}}projects/{{project}}/locations/{{location}}/collections/default_collection/dataStores")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceDiscoveryEngineDataStore(),
		ListURL:        url,

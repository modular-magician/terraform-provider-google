...
func init() {
	registry.FrameworkListResource{
		Name:        "google_discovery_engine_schema",
		ProductName: "DiscoveryEngine",
		Func:        NewDiscoveryEngineSchemaListResource,
	}.Register()
}

var _ list.ListResource = &DiscoveryEngineSchemaListResource{}

type DiscoveryEngineSchemaListResource struct {
	tpgresource.ListResourceMetadata
}

func NewDiscoveryEngineSchemaListResource() list.ListResource {
	listR := &DiscoveryEngineSchemaListResource{}
	listR.TypeName = "google_discovery_engine_schema"
	listR.SDKv2Resource = ResourceDiscoveryEngineSchema()
...
// DiscoveryEngineSchemaListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type DiscoveryEngineSchemaListModel struct {
	Location    types.String `tfsdk:"location"`
	DataStoreId types.String `tfsdk:"data_store_id"`
	Project     types.String `tfsdk:"project"`
}

func (listR *DiscoveryEngineSchemaListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data DiscoveryEngineSchemaListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListDiscoveryEngineSchemas(
			listR.Client,
			location,
			dataStoreId,
			project,
			func(rd *schema.ResourceData) error {
				...
				if err := listR.SetResult(ctx, listReq.IncludeResource, &result, rd, "name", "schema_id"); err != nil {
					return err
				}
...
func ListDiscoveryEngineSchemas(config *transport_tpg.Config,
	location string,
	dataStoreId string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceDiscoveryEngineSchema().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{DiscoveryEngineBasePath}}projects/{{project}}/locations/{{location}}/collections/default_collection/dataStores/{{data_store_id}}/schemas")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceDiscoveryEngineSchema(),
		ListURL:        url,

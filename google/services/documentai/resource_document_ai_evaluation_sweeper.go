...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_document_ai_evaluation",
		ListAndAction:  listAndActionDocumentAIEvaluation,
		DeleteFunction: testSweepDocumentAIEvaluation,
	}
...
func testSweepDocumentAIEvaluation(_ string) error {
	return listAndActionDocumentAIEvaluation(deleteResourceDocumentAIEvaluation)
}

func listAndActionDocumentAIEvaluation(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "DocumentAIEvaluation"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://{{location}}-documentai.googleapis.com/v1/projects/{{project}}/locations/{{location}}/processors/{{processor}}/processorVersions/{{processorVersion}}/evaluations", "?")[0]
		...
		resourceList, ok := res["evaluations"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'evaluations'")
		} else {
...
func deleteResourceDocumentAIEvaluation(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://{{location}}-documentai.googleapis.com/v1/projects/{{project}}/locations/{{location}}/processors/{{processor}}/processorVersions/{{processorVersion}}/evaluations/{{name}}"

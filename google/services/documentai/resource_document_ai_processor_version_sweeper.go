...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionDocumentAIProcessorVersion,
		DeleteFunction: testSweepDocumentAIProcessorVersion,
	}
...
func testSweepDocumentAIProcessorVersion(_ string) error {
	return listAndActionDocumentAIProcessorVersion(deleteResourceDocumentAIProcessorVersion)
}

func listAndActionDocumentAIProcessorVersion(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "DocumentAIProcessorVersion"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://{{location}}-documentai.googleapis.com/v1/projects/{{project}}/locations/{{location}}/processors/{{processor}}/processorVersions", "?")[0]
		...
		resourceList, ok := res["processorVersions"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'processorVersions'")
		} else {
...
func deleteResourceDocumentAIProcessorVersion(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://{{location}}-documentai.googleapis.com/v1/projects/{{project}}/locations/{{location}}/processors/{{processor}}/processorVersions/{{name}}"

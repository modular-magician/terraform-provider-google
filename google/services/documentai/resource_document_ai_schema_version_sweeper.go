...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionDocumentAISchemaVersion,
		DeleteFunction: testSweepDocumentAISchemaVersion,
	}
...
func testSweepDocumentAISchemaVersion(_ string) error {
	return listAndActionDocumentAISchemaVersion(deleteResourceDocumentAISchemaVersion)
}

func listAndActionDocumentAISchemaVersion(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "DocumentAISchemaVersion"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://{{location}}-documentai.googleapis.com/v1/projects/{{project}}/locations/{{location}}/schemas/{{schema_id}}/schemaVersions", "?")[0]
		...
		resourceList, ok := res["schemaVersions"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'schemaVersions'")
		} else {
...
func deleteResourceDocumentAISchemaVersion(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://{{location}}-documentai.googleapis.com/v1/projects/{{project}}/locations/{{location}}/schemas/{{schema_id}}/schemaVersions/{{name}}"

...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewGKEBackupBackupPlanListResource,
	}.Register()
}

var _ list.ListResource = &GKEBackupBackupPlanListResource{}

type GKEBackupBackupPlanListResource struct {
	tpgresource.ListResourceMetadata
}

func NewGKEBackupBackupPlanListResource() list.ListResource {
	listR := &GKEBackupBackupPlanListResource{}
	listR.TypeName = "google_gke_backup_backup_plan"
	listR.SDKv2Resource = ResourceGKEBackupBackupPlan()
...
// GKEBackupBackupPlanListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type GKEBackupBackupPlanListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *GKEBackupBackupPlanListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data GKEBackupBackupPlanListModel
...
func ListGKEBackupBackupPlans(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceGKEBackupBackupPlan().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{GKEBackupBasePath}}projects/{{project}}/locations/{{location}}/backupPlans")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceGKEBackupBackupPlan(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "backupPlans",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceGKEBackupBackupPlanFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

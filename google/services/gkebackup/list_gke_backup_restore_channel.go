...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewGKEBackupRestoreChannelListResource,
	}.Register()
}

var _ list.ListResource = &GKEBackupRestoreChannelListResource{}

type GKEBackupRestoreChannelListResource struct {
	tpgresource.ListResourceMetadata
}

func NewGKEBackupRestoreChannelListResource() list.ListResource {
	listR := &GKEBackupRestoreChannelListResource{}
	listR.TypeName = "google_gke_backup_restore_channel"
	listR.SDKv2Resource = ResourceGKEBackupRestoreChannel()
...
// GKEBackupRestoreChannelListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type GKEBackupRestoreChannelListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *GKEBackupRestoreChannelListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data GKEBackupRestoreChannelListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListGKEBackupRestoreChannels(
			listR.Client,
			location,
			project,
...
func ListGKEBackupRestoreChannels(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceGKEBackupRestoreChannel().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{GKEBackupBasePath}}projects/{{project}}/locations/{{location}}/restoreChannels")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceGKEBackupRestoreChannel(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "restoreChannels",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceGKEBackupRestoreChannelFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

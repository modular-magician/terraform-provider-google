...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewGKEBackupRestorePlanListResource,
	}.Register()
}

var _ list.ListResource = &GKEBackupRestorePlanListResource{}

type GKEBackupRestorePlanListResource struct {
	tpgresource.ListResourceMetadata
}

func NewGKEBackupRestorePlanListResource() list.ListResource {
	listR := &GKEBackupRestorePlanListResource{}
	listR.TypeName = "google_gke_backup_restore_plan"
	listR.SDKv2Resource = ResourceGKEBackupRestorePlan()
...
// GKEBackupRestorePlanListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type GKEBackupRestorePlanListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *GKEBackupRestorePlanListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data GKEBackupRestorePlanListModel
...
func ListGKEBackupRestorePlans(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceGKEBackupRestorePlan().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{GKEBackupBasePath}}projects/{{project}}/locations/{{location}}/restorePlans")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceGKEBackupRestorePlan(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "restorePlans",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceGKEBackupRestorePlanFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

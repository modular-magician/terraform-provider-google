...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionGKEHubMembership,
		DeleteFunction: testSweepGKEHubMembership,
	}
...
func testSweepGKEHubMembership(_ string) error {
	return listAndActionGKEHubMembership(deleteResourceGKEHubMembership)
}

func listAndActionGKEHubMembership(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "GKEHubMembership"
	...
	intermediateValues[1]["location"] = envvar.GetTestRegionFromEnv()
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://gkehub.googleapis.com/v1/projects/{{project}}/locations/{{location}}/memberships", "?")[0]
		...
		resourceList, ok := res["resources"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'resources'")
		} else {
...
func deleteResourceGKEHubMembership(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://gkehub.googleapis.com/v1/projects/{{project}}/locations/{{location}}/memberships/{{membership_id}}"

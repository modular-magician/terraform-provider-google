	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"fleet": {
				...
				Optional: true,
				Description: `Whether the membershipbinding is Fleet-wide; true means that this
Membership should be bound to all Namespaces in this entire Fleet.`,
				ExactlyOneOf: []string{"scope", "fleet"},
			},
			"scope": {
				Type:     schema.TypeString,
				Optional: true,
...
func resourceGKEHub2MembershipBindingRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err := d.Set("fleet", flattenGKEHub2MembershipBindingFleet(res["fleet"], d, config)); err != nil {
		return fmt.Errorf("Error reading MembershipBinding: %s", err)
	}
...
func flattenGKEHub2MembershipBindingFleet(v interface{}, d *schema.ResourceData, config *transport_tpg.Config) interface{} {
	return v
}

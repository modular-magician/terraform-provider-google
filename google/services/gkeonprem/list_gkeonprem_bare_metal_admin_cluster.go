...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewGkeonpremBareMetalAdminClusterListResource,
	}.Register()
}

var _ list.ListResource = &GkeonpremBareMetalAdminClusterListResource{}

type GkeonpremBareMetalAdminClusterListResource struct {
	tpgresource.ListResourceMetadata
}

func NewGkeonpremBareMetalAdminClusterListResource() list.ListResource {
	listR := &GkeonpremBareMetalAdminClusterListResource{}
	listR.TypeName = "google_gkeonprem_bare_metal_admin_cluster"
	listR.SDKv2Resource = ResourceGkeonpremBareMetalAdminCluster()
...
// GkeonpremBareMetalAdminClusterListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type GkeonpremBareMetalAdminClusterListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *GkeonpremBareMetalAdminClusterListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data GkeonpremBareMetalAdminClusterListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListGkeonpremBareMetalAdminClusters(
			listR.Client,
			location,
			project,
...
func ListGkeonpremBareMetalAdminClusters(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceGkeonpremBareMetalAdminCluster().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{GkeonpremBasePath}}projects/{{project}}/locations/{{location}}/bareMetalAdminClusters")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceGkeonpremBareMetalAdminCluster(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "bareMetalAdminClusters",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if res == nil {
				return fmt.Errorf("error decoding GkeonpremBareMetalAdminCluster from list response")
			}
			...
			if err = ResourceGkeonpremBareMetalAdminClusterFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

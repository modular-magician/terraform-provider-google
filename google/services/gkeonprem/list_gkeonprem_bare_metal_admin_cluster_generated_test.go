...
func TestAccGkeonpremBareMetalAdminClusterListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_gkeonprem_bare_metal_admin_cluster.admin-cluster-basic",
						[]string{
							"name",
						},
					),
					listScope.Capture(map[string]string{
						"location": "google_gkeonprem_bare_metal_admin_cluster.admin-cluster-basic",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccGkeonpremBareMetalAdminCluster_gkeonpremBareMetalAdminClusterBasicExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_gkeonprem_bare_metal_admin_cluster.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_gkeonprem_bare_metal_admin_cluster.list_query",
...
func testAccGkeonpremBareMetalAdminCluster_gkeonpremBareMetalAdminClusterBasicExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "location" { type = string }
list "google_gkeonprem_bare_metal_admin_cluster" "list_query" {
    provider = google
	limit = 10000
    config {

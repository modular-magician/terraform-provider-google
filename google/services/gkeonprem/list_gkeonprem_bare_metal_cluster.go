...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewGkeonpremBareMetalClusterListResource,
	}.Register()
}

var _ list.ListResource = &GkeonpremBareMetalClusterListResource{}

type GkeonpremBareMetalClusterListResource struct {
	tpgresource.ListResourceMetadata
}

func NewGkeonpremBareMetalClusterListResource() list.ListResource {
	listR := &GkeonpremBareMetalClusterListResource{}
	listR.TypeName = "google_gkeonprem_bare_metal_cluster"
	listR.SDKv2Resource = ResourceGkeonpremBareMetalCluster()
...
// GkeonpremBareMetalClusterListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type GkeonpremBareMetalClusterListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *GkeonpremBareMetalClusterListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data GkeonpremBareMetalClusterListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListGkeonpremBareMetalClusters(
			listR.Client,
			location,
			project,
...
func ListGkeonpremBareMetalClusters(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceGkeonpremBareMetalCluster().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{GkeonpremBasePath}}projects/{{project}}/locations/{{location}}/bareMetalClusters")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceGkeonpremBareMetalCluster(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "bareMetalClusters",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if res == nil {
				return fmt.Errorf("error decoding GkeonpremBareMetalCluster from list response")
			}
			...
			if err = ResourceGkeonpremBareMetalClusterFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

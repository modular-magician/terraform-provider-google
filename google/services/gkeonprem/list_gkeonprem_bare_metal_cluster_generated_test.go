...
func TestAccGkeonpremBareMetalClusterListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_gkeonprem_bare_metal_cluster.cluster-basic",
						[]string{
							"name",
						},
					),
					listScope.Capture(map[string]string{
						"location": "google_gkeonprem_bare_metal_cluster.cluster-basic",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccGkeonpremBareMetalCluster_gkeonpremBareMetalClusterBasicExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_gkeonprem_bare_metal_cluster.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_gkeonprem_bare_metal_cluster.list_query",
...
func testAccGkeonpremBareMetalCluster_gkeonpremBareMetalClusterBasicExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "location" { type = string }
list "google_gkeonprem_bare_metal_cluster" "list_query" {
    provider = google
	limit = 10000
    config {

...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewGkeonpremVmwareAdminClusterListResource,
	}.Register()
}

var _ list.ListResource = &GkeonpremVmwareAdminClusterListResource{}

type GkeonpremVmwareAdminClusterListResource struct {
	tpgresource.ListResourceMetadata
}

func NewGkeonpremVmwareAdminClusterListResource() list.ListResource {
	listR := &GkeonpremVmwareAdminClusterListResource{}
	listR.TypeName = "google_gkeonprem_vmware_admin_cluster"
	listR.SDKv2Resource = ResourceGkeonpremVmwareAdminCluster()
...
// GkeonpremVmwareAdminClusterListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type GkeonpremVmwareAdminClusterListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *GkeonpremVmwareAdminClusterListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data GkeonpremVmwareAdminClusterListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListGkeonpremVmwareAdminClusters(
			listR.Client,
			location,
			project,
...
func ListGkeonpremVmwareAdminClusters(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceGkeonpremVmwareAdminCluster().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{GkeonpremBasePath}}projects/{{project}}/locations/{{location}}/vmwareAdminClusters")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceGkeonpremVmwareAdminCluster(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "vmwareAdminClusters",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			headers := make(http.Header)
			var err error
			res, err = resourceGkeonpremVmwareAdminClusterDecoder(d, config, res)
			if err != nil {
				return err
			}
			if res == nil {
				return fmt.Errorf("error decoding GkeonpremVmwareAdminCluster from list response")
			}
			...
			if err = ResourceGkeonpremVmwareAdminClusterFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

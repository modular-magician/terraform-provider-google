...
func TestAccGkeonpremVmwareAdminClusterListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_gkeonprem_vmware_admin_cluster.admin-cluster-basic",
						[]string{
							"name",
						},
					),
					listScope.Capture(map[string]string{
						"location": "google_gkeonprem_vmware_admin_cluster.admin-cluster-basic",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccGkeonpremVmwareAdminCluster_gkeonpremVmwareAdminClusterBasicExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_gkeonprem_vmware_admin_cluster.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_gkeonprem_vmware_admin_cluster.list_query",
...
func testAccGkeonpremVmwareAdminCluster_gkeonpremVmwareAdminClusterBasicExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "location" { type = string }
list "google_gkeonprem_vmware_admin_cluster" "list_query" {
    provider = google
	limit = 10000
    config {

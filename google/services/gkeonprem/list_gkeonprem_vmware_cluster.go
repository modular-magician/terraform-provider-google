...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewGkeonpremVmwareClusterListResource,
	}.Register()
}

var _ list.ListResource = &GkeonpremVmwareClusterListResource{}

type GkeonpremVmwareClusterListResource struct {
	tpgresource.ListResourceMetadata
}

func NewGkeonpremVmwareClusterListResource() list.ListResource {
	listR := &GkeonpremVmwareClusterListResource{}
	listR.TypeName = "google_gkeonprem_vmware_cluster"
	listR.SDKv2Resource = ResourceGkeonpremVmwareCluster()
...
// GkeonpremVmwareClusterListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type GkeonpremVmwareClusterListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *GkeonpremVmwareClusterListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data GkeonpremVmwareClusterListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListGkeonpremVmwareClusters(
			listR.Client,
			location,
			project,
...
func ListGkeonpremVmwareClusters(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceGkeonpremVmwareCluster().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{GkeonpremBasePath}}projects/{{project}}/locations/{{location}}/vmwareClusters")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceGkeonpremVmwareCluster(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "vmwareClusters",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if res == nil {
				return fmt.Errorf("error decoding GkeonpremVmwareCluster from list response")
			}
			...
			if err = ResourceGkeonpremVmwareClusterFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

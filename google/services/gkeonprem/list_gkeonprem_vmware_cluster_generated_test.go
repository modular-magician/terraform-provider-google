...
func TestAccGkeonpremVmwareClusterListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					listScope.Capture(map[string]string{
						"location": "google_gkeonprem_vmware_cluster.cluster-basic",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccGkeonpremVmwareCluster_gkeonpremVmwareClusterBasicExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_gkeonprem_vmware_cluster.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_gkeonprem_vmware_cluster.list_query",
...
func testAccGkeonpremVmwareCluster_gkeonpremVmwareClusterBasicExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "location" { type = string }
list "google_gkeonprem_vmware_cluster" "list_query" {
    provider = google
	limit = 10000
    config {

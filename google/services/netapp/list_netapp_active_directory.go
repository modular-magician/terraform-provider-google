...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Netapp",
		Func:        NewNetappActiveDirectoryListResource,
	}.Register()
}

var _ list.ListResource = &NetappActiveDirectoryListResource{}

type NetappActiveDirectoryListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetappActiveDirectoryListResource() list.ListResource {
	listR := &NetappActiveDirectoryListResource{}
	listR.TypeName = "google_netapp_active_directory"
	listR.SDKv2Resource = ResourceNetappActiveDirectory()
...
// NetappActiveDirectoryListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetappActiveDirectoryListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *NetappActiveDirectoryListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetappActiveDirectoryListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetappActiveDirectorys(
			listR.Client,
			location,
			project,
...
func ListNetappActiveDirectorys(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetappActiveDirectory().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetappBasePath}}projects/{{project}}/locations/{{location}}/activeDirectories")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetappActiveDirectory(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "activeDirectories",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetappActiveDirectoryFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

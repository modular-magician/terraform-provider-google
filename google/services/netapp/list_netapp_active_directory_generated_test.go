...
func TestAccNetappActiveDirectoryListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_netapp_active_directory.test_active_directory_full",
						[]string{
							"name",
						},
					),
					listScope.Capture(map[string]string{
						"location": "google_netapp_active_directory.test_active_directory_full",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccNetappActiveDirectory_netappActiveDirectoryFullExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_netapp_active_directory.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_netapp_active_directory.list_query",
...
func testAccNetappActiveDirectory_netappActiveDirectoryFullExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "location" { type = string }
list "google_netapp_active_directory" "list_query" {
    provider = google
	limit = 10000
    config {

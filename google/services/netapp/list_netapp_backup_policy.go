...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Netapp",
		Func:        NewNetappBackupPolicyListResource,
	}.Register()
}

var _ list.ListResource = &NetappBackupPolicyListResource{}

type NetappBackupPolicyListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetappBackupPolicyListResource() list.ListResource {
	listR := &NetappBackupPolicyListResource{}
	listR.TypeName = "google_netapp_backup_policy"
	listR.SDKv2Resource = ResourceNetappBackupPolicy()
...
// NetappBackupPolicyListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetappBackupPolicyListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *NetappBackupPolicyListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetappBackupPolicyListModel
...
func ListNetappBackupPolicys(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetappBackupPolicy().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetappBasePath}}projects/{{project}}/locations/{{location}}/backupPolicies")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetappBackupPolicy(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "backupPolicies",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetappBackupPolicyFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

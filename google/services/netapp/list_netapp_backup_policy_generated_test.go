...
func TestAccNetappBackupPolicyListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_netapp_backup_policy.test_backup_policy_full",
						[]string{
							"name",
						},
					),
					listScope.Capture(map[string]string{
						"location": "google_netapp_backup_policy.test_backup_policy_full",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccNetappBackupPolicy_netappBackupPolicyFullExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_netapp_backup_policy.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_netapp_backup_policy.list_query",
...
func testAccNetappBackupPolicy_netappBackupPolicyFullExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "location" { type = string }
list "google_netapp_backup_policy" "list_query" {
    provider = google
	limit = 10000
    config {

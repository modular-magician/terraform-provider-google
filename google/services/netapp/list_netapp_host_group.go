...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Netapp",
		Func:        NewNetappHostGroupListResource,
	}.Register()
}

var _ list.ListResource = &NetappHostGroupListResource{}

type NetappHostGroupListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetappHostGroupListResource() list.ListResource {
	listR := &NetappHostGroupListResource{}
	listR.TypeName = "google_netapp_host_group"
	listR.SDKv2Resource = ResourceNetappHostGroup()
...
// NetappHostGroupListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetappHostGroupListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *NetappHostGroupListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetappHostGroupListModel
...
func ListNetappHostGroups(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetappHostGroup().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetappBasePath}}projects/{{project}}/locations/{{location}}/hostGroups")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetappHostGroup(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetappHostGroupFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

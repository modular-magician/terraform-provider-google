...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Netapp",
		Func:        NewNetappkmsconfigListResource,
	}.Register()
}

var _ list.ListResource = &NetappkmsconfigListResource{}

type NetappkmsconfigListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetappkmsconfigListResource() list.ListResource {
	listR := &NetappkmsconfigListResource{}
	listR.TypeName = "google_netapp_kmsconfig"
	listR.SDKv2Resource = ResourceNetappkmsconfig()
...
// NetappkmsconfigListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetappkmsconfigListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *NetappkmsconfigListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetappkmsconfigListModel
...
func ListNetappkmsconfigs(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetappkmsconfig().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetappBasePath}}projects/{{project}}/locations/{{location}}/kmsConfigs")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetappkmsconfig(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetappkmsconfigFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

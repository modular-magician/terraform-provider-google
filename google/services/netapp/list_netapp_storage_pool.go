...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Netapp",
		Func:        NewNetappStoragePoolListResource,
	}.Register()
}

var _ list.ListResource = &NetappStoragePoolListResource{}

type NetappStoragePoolListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetappStoragePoolListResource() list.ListResource {
	listR := &NetappStoragePoolListResource{}
	listR.TypeName = "google_netapp_storage_pool"
	listR.SDKv2Resource = ResourceNetappStoragePool()
...
// NetappStoragePoolListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetappStoragePoolListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *NetappStoragePoolListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetappStoragePoolListModel
...
func ListNetappStoragePools(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetappStoragePool().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetappBasePath}}projects/{{project}}/locations/{{location}}/storagePools")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetappStoragePool(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "storagePools",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetappStoragePoolFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

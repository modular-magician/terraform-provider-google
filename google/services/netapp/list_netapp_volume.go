...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Netapp",
		Func:        NewNetappVolumeListResource,
	}.Register()
}

var _ list.ListResource = &NetappVolumeListResource{}

type NetappVolumeListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetappVolumeListResource() list.ListResource {
	listR := &NetappVolumeListResource{}
	listR.TypeName = "google_netapp_volume"
	listR.SDKv2Resource = ResourceNetappVolume()
...
// NetappVolumeListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetappVolumeListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *NetappVolumeListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetappVolumeListModel
...
func ListNetappVolumes(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetappVolume().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetappBasePath}}projects/{{project}}/locations/{{location}}/volumes")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetappVolumeFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

...
func TestAccNetappVolumeListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_netapp_volume.test_volume",
						[]string{
							"name",
						},
					),
					listScope.Capture(map[string]string{
						"location": "google_netapp_volume.test_volume",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccNetappVolume_netappVolumeBasicExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_netapp_volume.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_netapp_volume.list_query",
...
func testAccNetappVolume_netappVolumeBasicExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "location" { type = string }
list "google_netapp_volume" "list_query" {
    provider = google
	limit = 10000
    config {

...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetappActiveDirectory,
		DeleteFunction: testSweepNetappActiveDirectory,
	}
	// Add dependencies
...
func testSweepNetappActiveDirectory(_ string) error {
	return listAndActionNetappActiveDirectory(deleteResourceNetappActiveDirectory)
}

func listAndActionNetappActiveDirectory(action sweeper.ResourceAction) error {
	var lastError error
	...
	intermediateValues[2]["location"] = "us-west2"
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'activeDirectories'")
		} else {
...
func deleteResourceNetappActiveDirectory(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {

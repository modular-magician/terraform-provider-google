...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetappBackupPolicy,
		DeleteFunction: testSweepNetappBackupPolicy,
	}
	// Add dependencies
...
func testSweepNetappBackupPolicy(_ string) error {
	return listAndActionNetappBackupPolicy(deleteResourceNetappBackupPolicy)
}

func listAndActionNetappBackupPolicy(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'backupPolicies'")
		} else {
...
func deleteResourceNetappBackupPolicy(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {

...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetappBackupVault,
		DeleteFunction: testSweepNetappBackupVault,
	}
...
func testSweepNetappBackupVault(_ string) error {
	return listAndActionNetappBackupVault(deleteResourceNetappBackupVault)
}

func listAndActionNetappBackupVault(action sweeper.ResourceAction) error {
	var lastError error
	...
	intermediateValues[1]["location"] = "us-east4"
	intermediateValues[2] = map[string]string{}
	intermediateValues[2]["location"] = "us-west1"
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'backupVaults'")
		} else {
...
func deleteResourceNetappBackupVault(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {

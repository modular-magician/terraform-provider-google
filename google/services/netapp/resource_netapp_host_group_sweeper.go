...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetappHostGroup,
		DeleteFunction: testSweepNetappHostGroup,
	}
	// Add dependencies
...
func testSweepNetappHostGroup(_ string) error {
	return listAndActionNetappHostGroup(deleteResourceNetappHostGroup)
}

func listAndActionNetappHostGroup(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetappHostGroup"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://netapp.googleapis.com/v1/projects/{{project}}/locations/{{location}}/hostGroups", "?")[0]
		...
		resourceList, ok := res["hostGroups"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'hostGroups'")
		} else {
...
func deleteResourceNetappHostGroup(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://netapp.googleapis.com/v1/projects/{{project}}/locations/{{location}}/hostGroups/{{name}}"

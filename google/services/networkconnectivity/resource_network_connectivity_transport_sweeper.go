...
func listAndActionNetworkConnectivityTransport(action sweeper.ResourceAction) error {
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networkconnectivity.googleapis.com/v1/projects/{{project}}/locations/{{region}}/transports", "?")[0]
		...
		resourceList, ok := res["transports"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'transports'")
		} else {
...
func deleteResourceNetworkConnectivityTransport(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networkconnectivity.googleapis.com/v1/projects/{{project}}/locations/{{region}}/transports/{{name}}"

...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkConnectivityv1ServiceConnectionMap,
		DeleteFunction: testSweepNetworkConnectivityv1ServiceConnectionMap,
	}
...
func testSweepNetworkConnectivityv1ServiceConnectionMap(_ string) error {
	return listAndActionNetworkConnectivityv1ServiceConnectionMap(deleteResourceNetworkConnectivityv1ServiceConnectionMap)
}

func listAndActionNetworkConnectivityv1ServiceConnectionMap(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkConnectivityv1ServiceConnectionMap"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networkconnectivity.googleapis.com/v1/projects/{{project}}/locations/{{location}}/serviceConnectionMaps", "?")[0]
		...
		resourceList, ok := res["serviceConnectionMaps"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'serviceConnectionMaps'")
		} else {
...
func deleteResourceNetworkConnectivityv1ServiceConnectionMap(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networkconnectivity.googleapis.com/v1/projects/{{project}}/locations/{{location}}/serviceConnectionMaps/{{name}}"

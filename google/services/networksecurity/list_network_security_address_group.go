...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewNetworkSecurityAddressGroupListResource,
	}.Register()
}

var _ list.ListResource = &NetworkSecurityAddressGroupListResource{}

type NetworkSecurityAddressGroupListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkSecurityAddressGroupListResource() list.ListResource {
	listR := &NetworkSecurityAddressGroupListResource{}
	listR.TypeName = "google_network_security_address_group"
	listR.SDKv2Resource = ResourceNetworkSecurityAddressGroup()
...
// NetworkSecurityAddressGroupListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkSecurityAddressGroupListModel struct {
	Parent   types.String `tfsdk:"parent"`
	Location types.String `tfsdk:"location"`
}

func (listR *NetworkSecurityAddressGroupListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkSecurityAddressGroupListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkSecurityAddressGroups(
			listR.Client,
			parent,
			location,
...
func ListNetworkSecurityAddressGroups(config *transport_tpg.Config,
	parent string,
	location string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkSecurityAddressGroup().Data(&terraform.InstanceState{})
	if parent != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkSecurityBasePath}}{{parent}}/locations/{{location}}/addressGroups")
	if err != nil {
		return err
	}
	billingProject := ""
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkSecurityAddressGroup(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "addressGroups",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetworkSecurityAddressGroupFlatten(d, config, res, config, userAgent, billingProject, url, headers); err != nil {
				return err
			}

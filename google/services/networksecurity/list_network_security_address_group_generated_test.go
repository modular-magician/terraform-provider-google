...
func TestAccNetworkSecurityAddressGroupListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_network_security_address_group.default",
						[]string{
							"name",
						},
					),
					listScope.Capture(map[string]string{
						"location": "google_network_security_address_group.default",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccNetworkSecurityAddressGroup_networkSecurityAddressGroupsBasicExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_network_security_address_group.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_network_security_address_group.list_query",
...
func testAccNetworkSecurityAddressGroup_networkSecurityAddressGroupsBasicExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "location" { type = string }
list "google_network_security_address_group" "list_query" {
    provider = google
	limit = 10000
    config {

...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewNetworkSecurityBackendAuthenticationConfigListResource,
	}.Register()
}

var _ list.ListResource = &NetworkSecurityBackendAuthenticationConfigListResource{}

type NetworkSecurityBackendAuthenticationConfigListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkSecurityBackendAuthenticationConfigListResource() list.ListResource {
	listR := &NetworkSecurityBackendAuthenticationConfigListResource{}
	listR.TypeName = "google_network_security_backend_authentication_config"
	listR.SDKv2Resource = ResourceNetworkSecurityBackendAuthenticationConfig()
...
// NetworkSecurityBackendAuthenticationConfigListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkSecurityBackendAuthenticationConfigListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *NetworkSecurityBackendAuthenticationConfigListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkSecurityBackendAuthenticationConfigListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkSecurityBackendAuthenticationConfigs(
			listR.Client,
			project,
...
func ListNetworkSecurityBackendAuthenticationConfigs(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkSecurityBackendAuthenticationConfig().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkSecurityBasePath}}projects/{{project}}/locations/global/backendAuthenticationConfigs")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkSecurityBackendAuthenticationConfig(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "backendAuthenticationConfigs",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetworkSecurityBackendAuthenticationConfigFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

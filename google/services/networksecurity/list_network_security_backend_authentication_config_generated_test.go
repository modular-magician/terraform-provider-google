...
func TestAccNetworkSecurityBackendAuthenticationConfigListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_network_security_backend_authentication_config.default",
						[]string{
							"name",
						},
					),
			...
			{
				Query:           true,
				Config:          testAccNetworkSecurityBackendAuthenticationConfig_networkSecurityBackendAuthenticationConfigBasicExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_network_security_backend_authentication_config.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_network_security_backend_authentication_config.list_query",
...
func testAccNetworkSecurityBackendAuthenticationConfig_networkSecurityBackendAuthenticationConfigBasicExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
list "google_network_security_backend_authentication_config" "list_query" {
    provider = google
	limit = 10000
    config {
    }

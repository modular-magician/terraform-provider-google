...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewNetworkSecurityClientTlsPolicyListResource,
	}.Register()
}

var _ list.ListResource = &NetworkSecurityClientTlsPolicyListResource{}

type NetworkSecurityClientTlsPolicyListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkSecurityClientTlsPolicyListResource() list.ListResource {
	listR := &NetworkSecurityClientTlsPolicyListResource{}
	listR.TypeName = "google_network_security_client_tls_policy"
	listR.SDKv2Resource = ResourceNetworkSecurityClientTlsPolicy()
...
// NetworkSecurityClientTlsPolicyListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkSecurityClientTlsPolicyListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *NetworkSecurityClientTlsPolicyListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkSecurityClientTlsPolicyListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkSecurityClientTlsPolicys(
			listR.Client,
			project,
...
func ListNetworkSecurityClientTlsPolicys(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkSecurityClientTlsPolicy().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkSecurityBasePath}}projects/{{project}}/locations/global/clientTlsPolicies")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkSecurityClientTlsPolicy(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "clientTlsPolicies",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetworkSecurityClientTlsPolicyFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

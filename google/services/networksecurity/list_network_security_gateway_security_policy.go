...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewNetworkSecurityGatewaySecurityPolicyListResource,
	}.Register()
}

var _ list.ListResource = &NetworkSecurityGatewaySecurityPolicyListResource{}

type NetworkSecurityGatewaySecurityPolicyListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkSecurityGatewaySecurityPolicyListResource() list.ListResource {
	listR := &NetworkSecurityGatewaySecurityPolicyListResource{}
	listR.TypeName = "google_network_security_gateway_security_policy"
	listR.SDKv2Resource = ResourceNetworkSecurityGatewaySecurityPolicy()
...
// NetworkSecurityGatewaySecurityPolicyListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkSecurityGatewaySecurityPolicyListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *NetworkSecurityGatewaySecurityPolicyListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkSecurityGatewaySecurityPolicyListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkSecurityGatewaySecurityPolicys(
			listR.Client,
			project,
...
func ListNetworkSecurityGatewaySecurityPolicys(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkSecurityGatewaySecurityPolicy().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkSecurityBasePath}}projects/{{project}}/locations/us-central1/gatewaySecurityPolicies")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkSecurityGatewaySecurityPolicy(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "gatewaySecurityPolicies",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetworkSecurityGatewaySecurityPolicyFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

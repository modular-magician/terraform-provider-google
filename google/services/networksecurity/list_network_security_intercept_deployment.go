...
func init() {
	registry.FrameworkListResource{
		Name:        "google_network_security_intercept_deployment",
		ProductName: "NetworkSecurity",
		Func:        NewNetworkSecurityInterceptDeploymentListResource,
	}.Register()
}

var _ list.ListResource = &NetworkSecurityInterceptDeploymentListResource{}

type NetworkSecurityInterceptDeploymentListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkSecurityInterceptDeploymentListResource() list.ListResource {
	listR := &NetworkSecurityInterceptDeploymentListResource{}
	listR.TypeName = "google_network_security_intercept_deployment"
	listR.SDKv2Resource = ResourceNetworkSecurityInterceptDeployment()
...
// NetworkSecurityInterceptDeploymentListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkSecurityInterceptDeploymentListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *NetworkSecurityInterceptDeploymentListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkSecurityInterceptDeploymentListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkSecurityInterceptDeployments(
			listR.Client,
			location,
			project,
			func(rd *schema.ResourceData) error {
				...
				if err := listR.SetResult(ctx, listReq.IncludeResource, &result, rd, "name", "intercept_deployment_id"); err != nil {
					return err
				}
...
func ListNetworkSecurityInterceptDeployments(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkSecurityInterceptDeployment().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkSecurityBasePath}}projects/{{project}}/locations/{{location}}/interceptDeployments")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkSecurityInterceptDeployment(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "interceptDeployments",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetworkSecurityInterceptDeploymentFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

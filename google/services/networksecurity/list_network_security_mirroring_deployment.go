...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewNetworkSecurityMirroringDeploymentListResource,
	}.Register()
}

var _ list.ListResource = &NetworkSecurityMirroringDeploymentListResource{}

type NetworkSecurityMirroringDeploymentListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkSecurityMirroringDeploymentListResource() list.ListResource {
	listR := &NetworkSecurityMirroringDeploymentListResource{}
	listR.TypeName = "google_network_security_mirroring_deployment"
	listR.SDKv2Resource = ResourceNetworkSecurityMirroringDeployment()
...
// NetworkSecurityMirroringDeploymentListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkSecurityMirroringDeploymentListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *NetworkSecurityMirroringDeploymentListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkSecurityMirroringDeploymentListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkSecurityMirroringDeployments(
			listR.Client,
			location,
			project,
			func(rd *schema.ResourceData) error {
				...
				if err := listR.SetResult(ctx, listReq.IncludeResource, &result, rd, "name", "mirroring_deployment_id"); err != nil {
					return err
				}
...
func ListNetworkSecurityMirroringDeployments(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkSecurityMirroringDeployment().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkSecurityBasePath}}projects/{{project}}/locations/{{location}}/mirroringDeployments")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkSecurityMirroringDeployment(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "mirroringDeployments",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetworkSecurityMirroringDeploymentFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

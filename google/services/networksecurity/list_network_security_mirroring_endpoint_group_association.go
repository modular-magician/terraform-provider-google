...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewNetworkSecurityMirroringEndpointGroupAssociationListResource,
	}.Register()
}

var _ list.ListResource = &NetworkSecurityMirroringEndpointGroupAssociationListResource{}

type NetworkSecurityMirroringEndpointGroupAssociationListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkSecurityMirroringEndpointGroupAssociationListResource() list.ListResource {
	listR := &NetworkSecurityMirroringEndpointGroupAssociationListResource{}
	listR.TypeName = "google_network_security_mirroring_endpoint_group_association"
	listR.SDKv2Resource = ResourceNetworkSecurityMirroringEndpointGroupAssociation()
...
// NetworkSecurityMirroringEndpointGroupAssociationListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkSecurityMirroringEndpointGroupAssociationListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *NetworkSecurityMirroringEndpointGroupAssociationListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkSecurityMirroringEndpointGroupAssociationListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkSecurityMirroringEndpointGroupAssociations(
			listR.Client,
			location,
			project,
			func(rd *schema.ResourceData) error {
				...
				if err := listR.SetResult(ctx, listReq.IncludeResource, &result, rd, "name", "mirroring_endpoint_group_association_id"); err != nil {
					return err
				}
...
func ListNetworkSecurityMirroringEndpointGroupAssociations(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkSecurityMirroringEndpointGroupAssociation().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkSecurityBasePath}}projects/{{project}}/locations/{{location}}/mirroringEndpointGroupAssociations")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkSecurityMirroringEndpointGroupAssociation(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "mirroringEndpointGroupAssociations",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if v, ok := res["mirroringEndpointGroupAssociationId"]; ok && v != nil {
				if err := d.Set("mirroring_endpoint_group_association_id", v); err != nil {
					return fmt.Errorf("error setting mirroring_endpoint_group_association_id: %w", err)
				}
			}

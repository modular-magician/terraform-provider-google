...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewNetworkSecurityServerTlsPolicyListResource,
	}.Register()
}

var _ list.ListResource = &NetworkSecurityServerTlsPolicyListResource{}

type NetworkSecurityServerTlsPolicyListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkSecurityServerTlsPolicyListResource() list.ListResource {
	listR := &NetworkSecurityServerTlsPolicyListResource{}
	listR.TypeName = "google_network_security_server_tls_policy"
	listR.SDKv2Resource = ResourceNetworkSecurityServerTlsPolicy()
...
// NetworkSecurityServerTlsPolicyListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkSecurityServerTlsPolicyListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *NetworkSecurityServerTlsPolicyListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkSecurityServerTlsPolicyListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkSecurityServerTlsPolicys(
			listR.Client,
			project,
...
func ListNetworkSecurityServerTlsPolicys(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkSecurityServerTlsPolicy().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkSecurityBasePath}}projects/{{project}}/locations/global/serverTlsPolicies")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkSecurityServerTlsPolicy(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "serverTlsPolicies",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetworkSecurityServerTlsPolicyFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

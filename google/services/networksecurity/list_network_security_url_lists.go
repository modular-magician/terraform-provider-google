...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewNetworkSecurityUrlListsListResource,
	}.Register()
}

var _ list.ListResource = &NetworkSecurityUrlListsListResource{}

type NetworkSecurityUrlListsListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkSecurityUrlListsListResource() list.ListResource {
	listR := &NetworkSecurityUrlListsListResource{}
	listR.TypeName = "google_network_security_url_lists"
	listR.SDKv2Resource = ResourceNetworkSecurityUrlLists()
...
// NetworkSecurityUrlListsListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkSecurityUrlListsListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *NetworkSecurityUrlListsListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkSecurityUrlListsListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkSecurityUrlListss(
			listR.Client,
			project,
...
func ListNetworkSecurityUrlListss(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkSecurityUrlLists().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkSecurityBasePath}}projects/{{project}}/locations/us-central1/urlLists")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkSecurityUrlLists(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetworkSecurityUrlListsFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

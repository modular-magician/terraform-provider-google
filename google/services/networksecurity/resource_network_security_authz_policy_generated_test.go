...
resource "google_compute_instance_group" "callouts_instance_group" {
  name        = "%{callouts_instance_group_name}"
...
resource "google_compute_region_health_check" "callouts_health_check" {
  name     = "%{callouts_health_check_name}"
  region   = "us-west1"
...
func testAccCheckNetworkSecurityAuthzPolicyDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			...
			url, err := tpgresource.ReplaceVarsForTest(config, rs, transport_tpg.BaseUrl(networksecurity.Product, config)+"projects/{{project}}/locations/{{location}}/authzPolicies/{{name}}")
			if err != nil {
				return err
			}

...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkSecurityGatewaySecurityPolicy,
		DeleteFunction: testSweepNetworkSecurityGatewaySecurityPolicy,
	}
	// Add dependencies
...
func testSweepNetworkSecurityGatewaySecurityPolicy(_ string) error {
	return listAndActionNetworkSecurityGatewaySecurityPolicy(deleteResourceNetworkSecurityGatewaySecurityPolicy)
}

func listAndActionNetworkSecurityGatewaySecurityPolicy(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkSecurityGatewaySecurityPolicy"
	...
	intermediateValues[0]["region"] = "us-east1"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networksecurity.googleapis.com/v1/projects/{{project}}/locations/us-central1/gatewaySecurityPolicies", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'gatewaySecurityPolicies'")
		} else {
...
func deleteResourceNetworkSecurityGatewaySecurityPolicy(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networksecurity.googleapis.com/v1/projects/{{project}}/locations/{{location}}/gatewaySecurityPolicies/{{name}}"

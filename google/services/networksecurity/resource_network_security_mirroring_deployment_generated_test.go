...
resource "google_network_security_mirroring_deployment" "default" {
  mirroring_deployment_id    = "%{deployment_id}"
...
func testAccCheckNetworkSecurityMirroringDeploymentDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			...
			url, err := tpgresource.ReplaceVarsForTest(config, rs, transport_tpg.BaseUrl(networksecurity.Product, config)+"projects/{{project}}/locations/{{location}}/mirroringDeployments/{{mirroring_deployment_id}}")
			if err != nil {
				return err
			}

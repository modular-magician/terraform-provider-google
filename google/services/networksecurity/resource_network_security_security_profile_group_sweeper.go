...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkSecuritySecurityProfileGroup,
		DeleteFunction: testSweepNetworkSecuritySecurityProfileGroup,
	}
...
func testSweepNetworkSecuritySecurityProfileGroup(_ string) error {
	return listAndActionNetworkSecuritySecurityProfileGroup(deleteResourceNetworkSecuritySecurityProfileGroup)
}

func listAndActionNetworkSecuritySecurityProfileGroup(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkSecuritySecurityProfileGroup"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networksecurity.googleapis.com/v1/{{parent}}/locations/{{location}}securityProfileGroups", "?")[0]
		...
		resourceList, ok := res["securityProfileGroups"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'securityProfileGroups'")
		} else {
...
func deleteResourceNetworkSecuritySecurityProfileGroup(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networksecurity.googleapis.com/v1/{{parent}}/locations/{{location}}/securityProfileGroups/{{name}}"

...
func testSweepNetworkSecuritySecurityProfile(_ string) error {
	return listAndActionNetworkSecuritySecurityProfile(deleteResourceNetworkSecuritySecurityProfile)
}

func listAndActionNetworkSecuritySecurityProfile(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkSecuritySecurityProfile"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networksecurity.googleapis.com/v1/{{parent}}/locations/{{location}}/securityProfiles", "?")[0]
		...
		resourceList, ok := res["securityProfiles"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'securityProfiles'")
		} else {
...
func deleteResourceNetworkSecuritySecurityProfile(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networksecurity.googleapis.com/v1/{{parent}}/locations/{{location}}/securityProfiles/{{name}}"

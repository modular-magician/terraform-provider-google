...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewNetworkServicesAgentGatewayListResource,
	}.Register()
}

var _ list.ListResource = &NetworkServicesAgentGatewayListResource{}

type NetworkServicesAgentGatewayListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkServicesAgentGatewayListResource() list.ListResource {
	listR := &NetworkServicesAgentGatewayListResource{}
	listR.TypeName = "google_network_services_agent_gateway"
	listR.SDKv2Resource = ResourceNetworkServicesAgentGateway()
...
// NetworkServicesAgentGatewayListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkServicesAgentGatewayListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *NetworkServicesAgentGatewayListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkServicesAgentGatewayListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkServicesAgentGateways(
			listR.Client,
			location,
			project,
...
func ListNetworkServicesAgentGateways(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkServicesAgentGateway().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkServicesBasePath}}projects/{{project}}/locations/{{location}}/agentGateways")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkServicesAgentGateway(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "agentGateways",

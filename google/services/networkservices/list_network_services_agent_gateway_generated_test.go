...
func TestAccNetworkServicesAgentGatewayListQuery_generated(t *testing.T) {
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					listScope.Capture(map[string]string{
						"location": "google_network_services_agent_gateway.default",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccNetworkServicesAgentGateway_networkServicesAgentGatewayClientToAgentExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_network_services_agent_gateway.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_network_services_agent_gateway.list_query",
...
func testAccNetworkServicesAgentGateway_networkServicesAgentGatewayClientToAgentExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "location" { type = string }
list "google_network_services_agent_gateway" "list_query" {
    provider = google
	limit = 10000
    config {

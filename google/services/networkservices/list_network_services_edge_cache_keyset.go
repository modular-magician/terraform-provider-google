...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewNetworkServicesEdgeCacheKeysetListResource,
	}.Register()
}

var _ list.ListResource = &NetworkServicesEdgeCacheKeysetListResource{}

type NetworkServicesEdgeCacheKeysetListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkServicesEdgeCacheKeysetListResource() list.ListResource {
	listR := &NetworkServicesEdgeCacheKeysetListResource{}
	listR.TypeName = "google_network_services_edge_cache_keyset"
	listR.SDKv2Resource = ResourceNetworkServicesEdgeCacheKeyset()
...
// NetworkServicesEdgeCacheKeysetListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkServicesEdgeCacheKeysetListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *NetworkServicesEdgeCacheKeysetListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkServicesEdgeCacheKeysetListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkServicesEdgeCacheKeysets(
			listR.Client,
			project,
...
func ListNetworkServicesEdgeCacheKeysets(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkServicesEdgeCacheKeyset().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkServicesBasePath}}projects/{{project}}/locations/global/edgeCacheKeysets")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkServicesEdgeCacheKeyset(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "edgeCacheKeysets",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetworkServicesEdgeCacheKeysetFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

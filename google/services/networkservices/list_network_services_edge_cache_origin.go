...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewNetworkServicesEdgeCacheOriginListResource,
	}.Register()
}

var _ list.ListResource = &NetworkServicesEdgeCacheOriginListResource{}

type NetworkServicesEdgeCacheOriginListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkServicesEdgeCacheOriginListResource() list.ListResource {
	listR := &NetworkServicesEdgeCacheOriginListResource{}
	listR.TypeName = "google_network_services_edge_cache_origin"
	listR.SDKv2Resource = ResourceNetworkServicesEdgeCacheOrigin()
...
// NetworkServicesEdgeCacheOriginListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkServicesEdgeCacheOriginListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *NetworkServicesEdgeCacheOriginListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkServicesEdgeCacheOriginListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkServicesEdgeCacheOrigins(
			listR.Client,
			project,
...
func ListNetworkServicesEdgeCacheOrigins(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkServicesEdgeCacheOrigin().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkServicesBasePath}}projects/{{project}}/locations/global/edgeCacheOrigins")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkServicesEdgeCacheOrigin(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "edgeCacheOrigins",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetworkServicesEdgeCacheOriginFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

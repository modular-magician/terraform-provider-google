...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewNetworkServicesEdgeCacheServiceListResource,
	}.Register()
}

var _ list.ListResource = &NetworkServicesEdgeCacheServiceListResource{}

type NetworkServicesEdgeCacheServiceListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkServicesEdgeCacheServiceListResource() list.ListResource {
	listR := &NetworkServicesEdgeCacheServiceListResource{}
	listR.TypeName = "google_network_services_edge_cache_service"
	listR.SDKv2Resource = ResourceNetworkServicesEdgeCacheService()
...
// NetworkServicesEdgeCacheServiceListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkServicesEdgeCacheServiceListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *NetworkServicesEdgeCacheServiceListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkServicesEdgeCacheServiceListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkServicesEdgeCacheServices(
			listR.Client,
			project,
...
func ListNetworkServicesEdgeCacheServices(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkServicesEdgeCacheService().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkServicesBasePath}}projects/{{project}}/locations/global/edgeCacheServices")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkServicesEdgeCacheService(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "edgeCacheServices",

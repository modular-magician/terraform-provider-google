...
func init() {
	registry.FrameworkListResource{
		Name:        "google_network_services_endpoint_policy",
		ProductName: "NetworkServices",
		Func:        NewNetworkServicesEndpointPolicyListResource,
	}.Register()
}

var _ list.ListResource = &NetworkServicesEndpointPolicyListResource{}

type NetworkServicesEndpointPolicyListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkServicesEndpointPolicyListResource() list.ListResource {
	listR := &NetworkServicesEndpointPolicyListResource{}
	listR.TypeName = "google_network_services_endpoint_policy"
	listR.SDKv2Resource = ResourceNetworkServicesEndpointPolicy()
...
// NetworkServicesEndpointPolicyListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkServicesEndpointPolicyListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *NetworkServicesEndpointPolicyListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkServicesEndpointPolicyListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkServicesEndpointPolicys(
			listR.Client,
			project,
...
func ListNetworkServicesEndpointPolicys(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkServicesEndpointPolicy().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkServicesBasePath}}projects/{{project}}/locations/global/endpointPolicies")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkServicesEndpointPolicy(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "endpointPolicies",

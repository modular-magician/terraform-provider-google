...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewNetworkServicesHttpRouteListResource,
	}.Register()
}

var _ list.ListResource = &NetworkServicesHttpRouteListResource{}

type NetworkServicesHttpRouteListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkServicesHttpRouteListResource() list.ListResource {
	listR := &NetworkServicesHttpRouteListResource{}
	listR.TypeName = "google_network_services_http_route"
	listR.SDKv2Resource = ResourceNetworkServicesHttpRoute()
...
// NetworkServicesHttpRouteListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkServicesHttpRouteListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *NetworkServicesHttpRouteListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkServicesHttpRouteListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkServicesHttpRoutes(
			listR.Client,
			project,
...
func ListNetworkServicesHttpRoutes(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkServicesHttpRoute().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkServicesBasePath}}projects/{{project}}/locations/global/httpRoutes")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkServicesHttpRoute(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetworkServicesHttpRouteFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

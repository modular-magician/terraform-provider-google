...
func TestAccNetworkServicesLbRouteExtensionListQuery_generated(t *testing.T) {
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_network_services_lb_route_extension.default",
						[]string{
							"name",
						},
					),
					listScope.Capture(map[string]string{
						"location": "google_network_services_lb_route_extension.default",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccNetworkServicesLbRouteExtension_networkServicesLbRouteExtensionBasicExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_network_services_lb_route_extension.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_network_services_lb_route_extension.list_query",
...
func testAccNetworkServicesLbRouteExtension_networkServicesLbRouteExtensionBasicExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "location" { type = string }
list "google_network_services_lb_route_extension" "list_query" {
    provider = google
	limit = 10000
    config {

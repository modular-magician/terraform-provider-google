...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewNetworkServicesLbTrafficExtensionListResource,
	}.Register()
}

var _ list.ListResource = &NetworkServicesLbTrafficExtensionListResource{}

type NetworkServicesLbTrafficExtensionListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkServicesLbTrafficExtensionListResource() list.ListResource {
	listR := &NetworkServicesLbTrafficExtensionListResource{}
	listR.TypeName = "google_network_services_lb_traffic_extension"
	listR.SDKv2Resource = ResourceNetworkServicesLbTrafficExtension()
...
// NetworkServicesLbTrafficExtensionListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkServicesLbTrafficExtensionListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *NetworkServicesLbTrafficExtensionListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkServicesLbTrafficExtensionListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkServicesLbTrafficExtensions(
			listR.Client,
			location,
			project,
...
func ListNetworkServicesLbTrafficExtensions(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkServicesLbTrafficExtension().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkServicesBasePath}}projects/{{project}}/locations/{{location}}/lbTrafficExtensions")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkServicesLbTrafficExtension(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "lbTrafficExtensions",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetworkServicesLbTrafficExtensionFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewNetworkServicesMeshListResource,
	}.Register()
}

var _ list.ListResource = &NetworkServicesMeshListResource{}

type NetworkServicesMeshListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkServicesMeshListResource() list.ListResource {
	listR := &NetworkServicesMeshListResource{}
	listR.TypeName = "google_network_services_mesh"
	listR.SDKv2Resource = ResourceNetworkServicesMesh()
...
// NetworkServicesMeshListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkServicesMeshListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *NetworkServicesMeshListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkServicesMeshListModel
...
func ListNetworkServicesMeshs(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkServicesMesh().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkServicesBasePath}}projects/{{project}}/locations/{{location}}/meshes")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkServicesMesh(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetworkServicesMeshFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

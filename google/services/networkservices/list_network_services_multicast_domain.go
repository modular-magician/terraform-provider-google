...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewNetworkServicesMulticastDomainListResource,
	}.Register()
}

var _ list.ListResource = &NetworkServicesMulticastDomainListResource{}

type NetworkServicesMulticastDomainListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkServicesMulticastDomainListResource() list.ListResource {
	listR := &NetworkServicesMulticastDomainListResource{}
	listR.TypeName = "google_network_services_multicast_domain"
	listR.SDKv2Resource = ResourceNetworkServicesMulticastDomain()
...
// NetworkServicesMulticastDomainListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkServicesMulticastDomainListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *NetworkServicesMulticastDomainListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkServicesMulticastDomainListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkServicesMulticastDomains(
			listR.Client,
			location,
			project,
			func(rd *schema.ResourceData) error {
				...
				if err := listR.SetResult(ctx, listReq.IncludeResource, &result, rd, "name", "multicast_domain_id"); err != nil {
					return err
				}
...
func ListNetworkServicesMulticastDomains(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkServicesMulticastDomain().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkServicesBasePath}}projects/{{project}}/locations/{{location}}/multicastDomains")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkServicesMulticastDomain(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "multicastDomains",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if v, ok := res["multicastDomainId"]; ok && v != nil {
				if s, ok := v.(string); ok {
					v = tpgresource.GetResourceNameFromSelfLink(s)
				}
				if err := d.Set("multicast_domain_id", v); err != nil {
					return fmt.Errorf("error setting multicast_domain_id: %w", err)
				}
			}
			if err = ResourceNetworkServicesMulticastDomainFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

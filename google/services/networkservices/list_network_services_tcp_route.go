...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewNetworkServicesTcpRouteListResource,
	}.Register()
}

var _ list.ListResource = &NetworkServicesTcpRouteListResource{}

type NetworkServicesTcpRouteListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkServicesTcpRouteListResource() list.ListResource {
	listR := &NetworkServicesTcpRouteListResource{}
	listR.TypeName = "google_network_services_tcp_route"
	listR.SDKv2Resource = ResourceNetworkServicesTcpRoute()
...
// NetworkServicesTcpRouteListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkServicesTcpRouteListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *NetworkServicesTcpRouteListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkServicesTcpRouteListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkServicesTcpRoutes(
			listR.Client,
			project,
...
func ListNetworkServicesTcpRoutes(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkServicesTcpRoute().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkServicesBasePath}}projects/{{project}}/locations/global/tcpRoutes")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkServicesTcpRoute(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetworkServicesTcpRouteFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewNetworkServicesTlsRouteListResource,
	}.Register()
}

var _ list.ListResource = &NetworkServicesTlsRouteListResource{}

type NetworkServicesTlsRouteListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkServicesTlsRouteListResource() list.ListResource {
	listR := &NetworkServicesTlsRouteListResource{}
	listR.TypeName = "google_network_services_tls_route"
	listR.SDKv2Resource = ResourceNetworkServicesTlsRoute()
...
// NetworkServicesTlsRouteListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkServicesTlsRouteListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *NetworkServicesTlsRouteListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkServicesTlsRouteListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkServicesTlsRoutes(
			listR.Client,
			location,
			project,
...
func ListNetworkServicesTlsRoutes(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkServicesTlsRoute().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkServicesBasePath}}projects/{{project}}/locations/{{location}}/tlsRoutes")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkServicesTlsRoute(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetworkServicesTlsRouteFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

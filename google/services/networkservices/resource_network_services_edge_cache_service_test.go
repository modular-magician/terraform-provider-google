...
func TestAccNetworkServicesEdgeCacheService_updateAndImport(t *testing.T) {
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccNetworkServicesEdgeCacheService_update_2(namebkt, nameorigin, nameservice),
			},
			{
				ResourceName:      "google_network_services_edge_cache_service.served",
				ImportState:       true,
...
func testAccNetworkServicesEdgeCacheService_update_2(bktName, originName, serviceName string) string {
	return fmt.Sprintf(`
resource "google_storage_bucket" "dest" {
  name          = "%s"
  location      = "US"
    ...
    path_matcher {
      ...
      route_rule {
        ...
        route_action {
          ...
          wasm_action = "/path/to/wasm_action"
        }
        header_action {
          response_header_to_add {
            header_name  = "x-cache-status"

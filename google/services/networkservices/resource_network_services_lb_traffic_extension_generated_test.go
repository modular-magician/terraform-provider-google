...
func TestAccNetworkServicesLbTrafficExtension_networkServicesLbTrafficExtensionHttpRouteExample(t *testing.T) {
	t.Parallel()
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccNetworkServicesLbTrafficExtension_networkServicesLbTrafficExtensionHttpRouteExample(context),
			},
			{
				ResourceName:            "google_network_services_lb_traffic_extension.default",
				ImportState:             true,
...
func testAccNetworkServicesLbTrafficExtension_networkServicesLbTrafficExtensionHttpRouteExample(context map[string]interface{}) string {
	return acctest.Nprintf(`
# Internal HTTP load balancer with a managed instance group backend
# VPC network
resource "google_compute_network" "ilb_network" {
  name                    = "tf-test-l7-ilb-network-route%{random_suffix}"
  auto_create_subnetworks = false
}

# proxy-only subnet
resource "google_compute_subnetwork" "proxy_subnet" {
  name          = "tf-test-l7-ilb-proxy-subnet-route%{random_suffix}"
  ip_cidr_range = "10.0.0.0/24"
  region        = "us-west1"
  purpose       = "REGIONAL_MANAGED_PROXY"
  role          = "ACTIVE"
...
# backend subnet
resource "google_compute_subnetwork" "ilb_subnet" {
  name          = "tf-test-l7-ilb-subnet-route%{random_suffix}"
  ip_cidr_range = "10.0.1.0/24"
  region        = "us-west1"
  network       = google_compute_network.ilb_network.id
...
# forwarding rule
resource "google_compute_forwarding_rule" "default" {
  name                  = "tf-test-l7-ilb-forwarding-rule-route%{random_suffix}"
  region                = "us-west1"
  ip_protocol           = "TCP"
  load_balancing_scheme = "INTERNAL_MANAGED"
  port_range            = "80"
...
# HTTP target proxy
resource "google_compute_region_target_http_proxy" "default" {
  name     = "tf-test-l7-ilb-target-http-proxy-route%{random_suffix}"
  region   = "us-west1"
  url_map  = google_compute_region_url_map.default.id
}
...
# URL map
resource "google_compute_region_url_map" "default" {
  name            = "tf-test-l7-ilb-regional-url-map-route%{random_suffix}"
  region          = "us-west1"
  default_service = google_compute_region_backend_service.default.id
}
...
# backend service
resource "google_compute_region_backend_service" "default" {
  name                  = "tf-test-l7-ilb-backend-subnet-route%{random_suffix}"
  region                = "us-west1"
  protocol              = "HTTP"
  load_balancing_scheme = "INTERNAL_MANAGED"
  timeout_sec           = 10
...
# instance template
resource "google_compute_instance_template" "instance_template" {
  name         = "tf-test-l7-ilb-mig-template-route%{random_suffix}"
  machine_type = "e2-small"
  tags         = ["http-server"]
...
# health check
resource "google_compute_region_health_check" "default" {
  name     = "tf-test-l7-ilb-hc-route%{random_suffix}"
  region   = "us-west1"
...
# MIG
resource "google_compute_region_instance_group_manager" "mig" {
  name     = "tf-test-l7-ilb-mig1-route%{random_suffix}"
  region   = "us-west1"
...
# allow all access from IAP and health check ranges
resource "google_compute_firewall" "fw_iap" {
  name          = "tf-test-l7-ilb-fw-allow-iap-hc-route%{random_suffix}"
  direction     = "INGRESS"
  network       = google_compute_network.ilb_network.id
  source_ranges = ["130.211.0.0/22", "35.191.0.0/16", "35.235.240.0/20"]
...
# allow http from proxy subnet to backends
resource "google_compute_firewall" "fw_ilb_to_backends" {
  name          = "tf-test-l7-ilb-fw-allow-ilb-to-backends-route%{random_suffix}"
  direction     = "INGRESS"
  network       = google_compute_network.ilb_network.id
  source_ranges = ["10.0.0.0/24"]
  target_tags   = ["http-server"]
...
resource "google_network_services_lb_traffic_extension" "default" {
  name     = "tf-test-l7-ilb-traffic-ext-route%{random_suffix}"
  description = "my traffic extension"
  location = "us-west1"
...
# test instance
resource "google_compute_instance" "vm_test" {
  name         = "tf-test-l7-ilb-test-vm-route%{random_suffix}"
  zone         = "us-west1-b"
  machine_type = "e2-small"
...
# Traffic Extension Backend Instance
resource "google_compute_instance" "callouts_instance" {
  name         = "tf-test-l7-ilb-callouts-ins-route%{random_suffix}"
  zone         = "us-west1-a"
  machine_type = "e2-small"
...
// callouts instance group
resource "google_compute_instance_group" "callouts_instance_group" {
  name        = "tf-test-l7-ilb-callouts-ins-group-route%{random_suffix}"
  description = "Terraform test instance group"
  zone        = "us-west1-a"
...
# callout health check
resource "google_compute_region_health_check" "callouts_health_check" {
  name     = "tf-test-l7-ilb-callouts-hc-route%{random_suffix}"
  region   = "us-west1"
...
# callout backend service
resource "google_compute_region_backend_service" "callouts_backend" {
  name                  = "tf-test-l7-ilb-callouts-backend-route%{random_suffix}"
  region                = "us-west1"
  protocol              = "HTTP2"
  load_balancing_scheme = "INTERNAL_MANAGED"
  timeout_sec           = 10

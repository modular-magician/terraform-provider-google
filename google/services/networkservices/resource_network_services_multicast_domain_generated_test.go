...
func testAccCheckNetworkServicesMulticastDomainDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			if rs.Type != "google_network_services_multicast_domain" {
				continue
			}
			...
			url, err := tpgresource.ReplaceVarsForTest(config, rs, transport_tpg.BaseUrl(networkservices.Product, config)+"projects/{{project}}/locations/{{location}}/multicastDomains/{{multicast_domain_id}}")
			if err != nil {
				return err
			}
			...
			if err == nil {
				return fmt.Errorf("NetworkServicesMulticastDomain still exists at %s", url)
			}
		}

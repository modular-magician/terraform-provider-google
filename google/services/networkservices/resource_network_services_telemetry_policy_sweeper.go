...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkServicesTelemetryPolicy,
		DeleteFunction: testSweepNetworkServicesTelemetryPolicy,
	}
...
func testSweepNetworkServicesTelemetryPolicy(_ string) error {
	return listAndActionNetworkServicesTelemetryPolicy(deleteResourceNetworkServicesTelemetryPolicy)
}

func listAndActionNetworkServicesTelemetryPolicy(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkServicesTelemetryPolicy"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networkservices.googleapis.com/v1/projects/{{project}}/locations/{{location}}/telemetryPolicies", "?")[0]
		...
		resourceList, ok := res["telemetryPolicies"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'telemetryPolicies'")
		} else {
...
func deleteResourceNetworkServicesTelemetryPolicy(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networkservices.googleapis.com/v1/projects/{{project}}/locations/{{location}}/telemetryPolicies/{{name}}"

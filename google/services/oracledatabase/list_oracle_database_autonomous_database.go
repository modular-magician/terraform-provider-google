...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewOracleDatabaseAutonomousDatabaseListResource,
	}.Register()
}

var _ list.ListResource = &OracleDatabaseAutonomousDatabaseListResource{}

type OracleDatabaseAutonomousDatabaseListResource struct {
	tpgresource.ListResourceMetadata
}

func NewOracleDatabaseAutonomousDatabaseListResource() list.ListResource {
	listR := &OracleDatabaseAutonomousDatabaseListResource{}
	listR.TypeName = "google_oracle_database_autonomous_database"
	listR.SDKv2Resource = ResourceOracleDatabaseAutonomousDatabase()
...
// OracleDatabaseAutonomousDatabaseListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type OracleDatabaseAutonomousDatabaseListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *OracleDatabaseAutonomousDatabaseListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data OracleDatabaseAutonomousDatabaseListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListOracleDatabaseAutonomousDatabases(
			listR.Client,
			location,
			project,
			func(rd *schema.ResourceData) error {
				...
				if err := listR.SetResult(ctx, listReq.IncludeResource, &result, rd, "name", "autonomous_database_id"); err != nil {
					return err
				}
...
func ListOracleDatabaseAutonomousDatabases(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceOracleDatabaseAutonomousDatabase().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{OracleDatabaseBasePath}}projects/{{project}}/locations/{{location}}/autonomousDatabases")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceOracleDatabaseAutonomousDatabase(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "autonomousDatabases",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if v, ok := res["autonomousDatabaseId"]; ok && v != nil {
				if err := d.Set("autonomous_database_id", v); err != nil {
					return fmt.Errorf("error setting autonomous_database_id: %w", err)
				}
			}
			if err = ResourceOracleDatabaseAutonomousDatabaseFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

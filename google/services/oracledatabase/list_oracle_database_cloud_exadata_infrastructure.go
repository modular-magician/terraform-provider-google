...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewOracleDatabaseCloudExadataInfrastructureListResource,
	}.Register()
}

var _ list.ListResource = &OracleDatabaseCloudExadataInfrastructureListResource{}

type OracleDatabaseCloudExadataInfrastructureListResource struct {
	tpgresource.ListResourceMetadata
}

func NewOracleDatabaseCloudExadataInfrastructureListResource() list.ListResource {
	listR := &OracleDatabaseCloudExadataInfrastructureListResource{}
	listR.TypeName = "google_oracle_database_cloud_exadata_infrastructure"
...
// OracleDatabaseCloudExadataInfrastructureListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type OracleDatabaseCloudExadataInfrastructureListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *OracleDatabaseCloudExadataInfrastructureListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data OracleDatabaseCloudExadataInfrastructureListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListOracleDatabaseCloudExadataInfrastructures(
			listR.Client,
			location,
			project,
			func(rd *schema.ResourceData) error {
				...
				if err := listR.SetResult(ctx, listReq.IncludeResource, &result, rd, "name", "cloud_exadata_infrastructure_id"); err != nil {
					return err
				}
...
func ListOracleDatabaseCloudExadataInfrastructures(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceOracleDatabaseCloudExadataInfrastructure().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{OracleDatabaseBasePath}}projects/{{project}}/locations/{{location}}/cloudExadataInfrastructures")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceOracleDatabaseCloudExadataInfrastructure(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "cloudExadataInfrastructures",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if v, ok := res["cloudExadataInfrastructureId"]; ok && v != nil {
				if err := d.Set("cloud_exadata_infrastructure_id", v); err != nil {
					return fmt.Errorf("error setting cloud_exadata_infrastructure_id: %w", err)
				}
			}
			if err = ResourceOracleDatabaseCloudExadataInfrastructureFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

...
func init() {
	registry.FrameworkListResource{
		Name:        "google_oracle_database_db_system",
		ProductName: "OracleDatabase",
		Func:        NewOracleDatabaseDbSystemListResource,
	}.Register()
}

var _ list.ListResource = &OracleDatabaseDbSystemListResource{}

type OracleDatabaseDbSystemListResource struct {
	tpgresource.ListResourceMetadata
}

func NewOracleDatabaseDbSystemListResource() list.ListResource {
	listR := &OracleDatabaseDbSystemListResource{}
	listR.TypeName = "google_oracle_database_db_system"
...
// OracleDatabaseDbSystemListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type OracleDatabaseDbSystemListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *OracleDatabaseDbSystemListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data OracleDatabaseDbSystemListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListOracleDatabaseDbSystems(
			listR.Client,
			location,
			project,
			func(rd *schema.ResourceData) error {
				...
				if err := listR.SetResult(ctx, listReq.IncludeResource, &result, rd, "name", "db_system_id"); err != nil {
					return err
				}
...
func ListOracleDatabaseDbSystems(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceOracleDatabaseDbSystem().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{OracleDatabaseBasePath}}projects/{{project}}/locations/{{location}}/dbSystems")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceOracleDatabaseDbSystem(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if v, ok := res["dbSystemId"]; ok && v != nil {
				if err := d.Set("db_system_id", v); err != nil {
					return fmt.Errorf("error setting db_system_id: %w", err)
				}
			}
			if err = ResourceOracleDatabaseDbSystemFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

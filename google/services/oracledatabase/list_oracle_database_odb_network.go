...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewOracleDatabaseOdbNetworkListResource,
	}.Register()
}

var _ list.ListResource = &OracleDatabaseOdbNetworkListResource{}

type OracleDatabaseOdbNetworkListResource struct {
	tpgresource.ListResourceMetadata
}

func NewOracleDatabaseOdbNetworkListResource() list.ListResource {
	listR := &OracleDatabaseOdbNetworkListResource{}
	listR.TypeName = "google_oracle_database_odb_network"
	listR.SDKv2Resource = ResourceOracleDatabaseOdbNetwork()
...
// OracleDatabaseOdbNetworkListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type OracleDatabaseOdbNetworkListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *OracleDatabaseOdbNetworkListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data OracleDatabaseOdbNetworkListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListOracleDatabaseOdbNetworks(
			listR.Client,
			location,
			project,
			func(rd *schema.ResourceData) error {
				...
				if err := listR.SetResult(ctx, listReq.IncludeResource, &result, rd, "name", "odb_network_id"); err != nil {
					return err
				}
...
func ListOracleDatabaseOdbNetworks(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceOracleDatabaseOdbNetwork().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{OracleDatabaseBasePath}}projects/{{project}}/locations/{{location}}/odbNetworks")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceOracleDatabaseOdbNetwork(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "odbNetworks",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if v, ok := res["odbNetworkId"]; ok && v != nil {
				if err := d.Set("odb_network_id", v); err != nil {
					return fmt.Errorf("error setting odb_network_id: %w", err)
				}
			}
			project := ""
			...
			if err = ResourceOracleDatabaseOdbNetworkFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

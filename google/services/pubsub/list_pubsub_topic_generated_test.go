...
func TestAccPubsubTopicListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					listScope.Capture(map[string]string{
						"project": "google_pubsub_topic.example",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccPubsubTopic_pubsubTopicBasicExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_pubsub_topic.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_pubsub_topic.list_query",
...
func testAccPubsubTopic_pubsubTopicBasicExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "project" { type = string }
list "google_pubsub_topic" "list_query" {
    provider = google
	limit = 10000
    config {
        project = var.project

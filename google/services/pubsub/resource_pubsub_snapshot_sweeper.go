...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionPubsubSnapshot,
		DeleteFunction: testSweepPubsubSnapshot,
	}
...
func testSweepPubsubSnapshot(_ string) error {
	return listAndActionPubsubSnapshot(deleteResourcePubsubSnapshot)
}

func listAndActionPubsubSnapshot(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "PubsubSnapshot"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://pubsub.googleapis.com/v1/projects/{{project}}/snapshots", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'snapshots'")
		} else {
...
func deleteResourcePubsubSnapshot(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://pubsub.googleapis.com/v1/projects/{{project}}/snapshots/{{name}}"

...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_random_random_name",
		ListAndAction:  listAndActionRandomRandomName,
		DeleteFunction: testSweepRandomRandomName,
	}
...
func testSweepRandomRandomName(_ string) error {
	return listAndActionRandomRandomName(deleteResourceRandomRandomName)
}

func listAndActionRandomRandomName(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "RandomRandomName"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://random.googleapis.com/v1/projects/{{project}}/locations/{{location}}/resourcenames", "?")[0]
		...
		resourceList, ok := res["randomNames"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'randomNames'")
		} else {
...
func deleteResourceRandomRandomName(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://random.googleapis.com/v1/projects/{{project}}/locations/{{location}}/resourcenames/{{name}}"

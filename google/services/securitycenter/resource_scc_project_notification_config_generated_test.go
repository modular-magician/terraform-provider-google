...
func TestAccSecurityCenterProjectNotificationConfig_sccProjectNotificationConfigBasicExample(t *testing.T) {
	t.Parallel()
...
func testAccCheckSecurityCenterProjectNotificationConfigDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			if rs.Type != "google_scc_project_notification_config" {
				continue
			}
			...
			if err == nil {
				return fmt.Errorf("SecurityCenterProjectNotificationConfig still exists at %s", url)
			}
		}

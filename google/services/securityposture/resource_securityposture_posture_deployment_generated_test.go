	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				ImportStateVerifyIgnore: []string{"parent", "posture_deployment_id", "annotations"},
			},
		},
	})
}
...
func testAccCheckSecurityposturePostureDeploymentDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			...
			url, err := tpgresource.ReplaceVarsForTest(config, rs, "{{SecuritypostureBasePath}}{{parent}}/postureDeployments/{{posture_deployment_id}}")
			if err != nil {
				return err
			}

		...
		Steps: []resource.TestStep{
			{
				...
				Check:  resource.ComposeTestCheckFunc(verifyPscOperation("google_sql_database_instance.instance", true, true, nil, false, false, false, "", "")),
			},
			{
				ResourceName:            "google_sql_database_instance.instance",
				ImportState:             true,
		...
		Steps: []resource.TestStep{
			{
				...
				Check:  resource.ComposeTestCheckFunc(verifyPscOperation("google_sql_database_instance.instance", true, true, []string{}, false, false, false, "", "")),
			},
			{
				ResourceName:            "google_sql_database_instance.instance",
				ImportState:             true,
		...
		Steps: []resource.TestStep{
			{
				...
				Check:  resource.ComposeTestCheckFunc(verifyPscOperation("google_sql_database_instance.instance", true, true, []string{envvar.GetTestProjectFromEnv()}, false, false, false, "", "")),
			},
			{
				ResourceName:            "google_sql_database_instance.instance",
				ImportState:             true,
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					verifyPscOperation("google_sql_database_instance.instance", true, true, []string{projectId}, true, true, false, "", ""),
				),
			},
			{
				ResourceName:            "google_sql_database_instance.instance",
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					verifyPscOperation("google_sql_database_instance.instance", true, true, []string{projectId}, false, false, false, "", ""),
				),
			},
			{
				ResourceName:            "google_sql_database_instance.instance",
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					verifyPscOperation("google_sql_database_instance.instance", true, true, []string{projectId}, true, false, false, "", ""),
				),
			},
			{
				ResourceName:            "google_sql_database_instance.instance",
...
func TestAccSqlDatabaseInstance_withPSCEnabled_updateAllowedConsumerProjects(t *testing.T) {
	t.Parallel()
...
func verifyPscOperation(resourceName string, isPscConfigExpected bool, expectedPscEnabled bool, expectedAllowedConsumerProjects []string, expectedPscAutoDnsEnabled bool, expectedPscWriteEndpointDnsEnabled bool, expectedEnablePscAutoConnectionPolicy bool, expectedServiceConnectionPolicy string, expectedServiceConnectionPolicyCreationResult string) func(*terraform.State) error {
	return func(s *terraform.State) error {
		...
		if isPscConfigExpected {
			...
			enablePscAutoConnectionPolicyStr, ok := resourceAttributes["settings.0.ip_configuration.0.psc_config.0.psc_auto_connection_policy_enabled"]
			enablePscAutoConnectionPolicy, err := strconv.ParseBool(enablePscAutoConnectionPolicyStr)
			if err != nil || enablePscAutoConnectionPolicy != expectedEnablePscAutoConnectionPolicy {
				return fmt.Errorf("settings.0.ip_configuration.0.psc_config.0.psc_auto_connection_policy_enabled property value is not set as expected in state of %s, expected %v, actual %v", resourceName, expectedEnablePscAutoConnectionPolicy, enablePscAutoConnectionPolicy)
			}

			if expectedServiceConnectionPolicy != "" {
				serviceConnectionPolicy, ok := resourceAttributes["settings.0.ip_configuration.0.psc_config.0.psc_auto_connections.0.service_connection_policy"]
				if !ok || serviceConnectionPolicy != expectedServiceConnectionPolicy {
					return fmt.Errorf("settings.0.ip_configuration.0.psc_config.0.psc_auto_connections.0.service_connection_policy property value is not set as expected in state of %s, expected %v, actual %v", resourceName, expectedServiceConnectionPolicy, serviceConnectionPolicy)
				}
			}

			if expectedServiceConnectionPolicyCreationResult != "" {
				serviceConnectionPolicyCreationResult, ok := resourceAttributes["settings.0.ip_configuration.0.psc_config.0.psc_auto_connections.0.service_connection_policy_creation_result"]
				if !ok || serviceConnectionPolicyCreationResult != expectedServiceConnectionPolicyCreationResult {
					return fmt.Errorf("settings.0.ip_configuration.0.psc_config.0.psc_auto_connections.0.service_connection_policy_creation_result property value is not set as expected in state of %s, expected %v, actual %v", resourceName, expectedServiceConnectionPolicyCreationResult, serviceConnectionPolicyCreationResult)
				}
			}
...
func testAccCheckSqlDatabaseInstanceDeleteServiceConnectionPolicyByNetwork(t *testing.T, project string, region string, networkName string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		config := acctest.GoogleProviderConfig(t)
		url := fmt.Sprintf("https://networkconnectivity.googleapis.com/v1/projects/%s/locations/%s/serviceConnectionPolicies", project, region)
		...
		if err != nil {
			log.Printf("[DEBUG] Error listing existing service connection policies: %s", err)
			return nil
		}
		if res == nil || res["serviceConnectionPolicies"] == nil {
			return nil
		}

		policies, ok := res["serviceConnectionPolicies"].([]interface{})
		if !ok {
			return nil
		}
		for _, p := range policies {
			...
			network, ok := policy["network"].(string)
			if !ok || (!strings.HasSuffix(network, "/networks/"+networkName) && !strings.Contains(network, "/"+networkName+"")) {
				continue
			}
			...
			deleteUrl := fmt.Sprintf("https://networkconnectivity.googleapis.com/v1/%s", strings.TrimPrefix(name, "/"))
			log.Printf("[DEBUG] Deleting service connection policy %q before deleting network", name)
			_, err := transport_tpg.SendRequest(transport_tpg.SendRequestOptions{
				Config:    config,
				Method:    "DELETE",
				Project:   billingProject,
			...
			if err != nil && !transport_tpg.IsGoogleApiErrorWithCode(err, 404) {
				log.Printf("[DEBUG] Error deleting service connection policy %s: %s", name, err)
			}
			...
			for i := 0; i < 30; i++ {
				...
				if err != nil && transport_tpg.IsGoogleApiErrorWithCode(err, 404) {
					log.Printf("[DEBUG] Service connection policy %q deleted successfully", name)
					break
				}
			}
		}

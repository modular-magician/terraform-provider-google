...
func ResourceStorageBucketStateUpgradeV3(_ context.Context, rawState map[string]interface{}, meta interface{}) (map[string]interface{}, error) {
	log.Printf("[DEBUG] Attributes before migration: %#v", rawState)
	bucketName, ok := rawState["name"].(string)
	if !ok {
		return nil, fmt.Errorf("failed to upgrade google_storage_bucket state: 'name' not found")
	}

	lifecycleRules, hasLifecycle := rawState["lifecycle_rule"].([]interface{})
	if !hasLifecycle || len(lifecycleRules) == 0 {
		return rawStateAsSingleMap(rawState), nil // No lifecycle rules to migrate
	}

	upgradedState := rawStateAsSingleMap(rawState)
	delete(upgradedState, "lifecycle_rule") // Remove the old lifecycle_rule

	multiState := make(map[string]interface{})
	newState := map[string]interface{}{
		"bucket":         bucketName,
		"lifecycle_rule": []interface{}{lifecycleRules},
	}
	multiState[bucketName] = newState

	return multiState, nil
}

// Helper function to return the raw state as a single-item map
func rawStateAsSingleMap(rawState map[string]interface{}) map[string]interface{} {

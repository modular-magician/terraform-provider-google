...
func TestAccStorageBucket_basic_hns(t *testing.T) {
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccStorageBucket_basic_hns(bucketName, false),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(
						"google_storage_bucket.bucket", "hierarchical_namespace.0.enabled", "false"),
				),
			},
			{
				ResourceName:            "google_storage_bucket.bucket",
...
// testcase to create bucket with HNS disabled and
// forcenew to recreate the bucket if HNS set to true
func TestAccStorageBucket_hns_forcenew(t *testing.T) {
	t.Parallel()
...
// testcase with HNS set to false
func TestAccStorageBucket_hns_disabled(t *testing.T) {
	t.Parallel()

	// test case with hns disabled
	bucketName := acctest.TestBucketName(t)
	acctest.VcrTest(t, resource.TestCase{
		PreCheck:                 func() { acctest.AccTestPreCheck(t) },
		ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories(t),
...
// testcase to force_destroy HNS bucket
func TestAccStorageBucket_hns_force_destroy(t *testing.T) {
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccStorageBucket_basic_hns_with_data(bucketName, true, true),
				Check: resource.ComposeTestCheckFunc(
...
`, bucketName, enabled, forceDestroy)
}

	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"name": {
				...
				Description:  `The name of the managed folder, expressed as a path. For example, example-dir or example-dir/example-dir1.`,
			},
			"create_time": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: `Time the ManagedFolder was created in UTC.`,
			},
			"metageneration": {
				Type:        schema.TypeInt,
				Computed:    true,
			...
			"update_time": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: `Time the ManagedFolder was updated in UTC.`,
			},
		},
		UseJSONNumber: true,
	}
...
func resourceStorageManagedFolderCreate(d *schema.ResourceData, meta interface{}) error {
	...
	// Store the ID now
	id, err := tpgresource.ReplaceVars(d, config, "b/{{bucket}}/managedFolders/{{%name}}")
	if err != nil {
...
func resourceStorageManagedFolderImport(d *schema.ResourceData, meta interface{}) ([]*schema.ResourceData, error) {
	...
	if err := tpgresource.ParseImportId([]string{
		"^b/(?P<bucket>[^/]+)/managedFolders/(?P<name>[^/]+)$",
		"^(?P<bucket>[^/]+)/(?P<name>[^/]+)$",
	}, d, config); err != nil {
		return nil, err
	}

...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewVertexAIEvaluationMetricListResource,
	}.Register()
}

var _ list.ListResource = &VertexAIEvaluationMetricListResource{}

type VertexAIEvaluationMetricListResource struct {
	tpgresource.ListResourceMetadata
}

func NewVertexAIEvaluationMetricListResource() list.ListResource {
	listR := &VertexAIEvaluationMetricListResource{}
	listR.TypeName = "google_vertex_ai_evaluation_metric"
	listR.SDKv2Resource = ResourceVertexAIEvaluationMetric()
...
// VertexAIEvaluationMetricListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type VertexAIEvaluationMetricListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *VertexAIEvaluationMetricListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data VertexAIEvaluationMetricListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListVertexAIEvaluationMetrics(
			listR.Client,
			region,
			project,
...
func ListVertexAIEvaluationMetrics(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceVertexAIEvaluationMetric().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{VertexAIBasePath}}projects/{{project}}/locations/{{region}}/evaluationMetrics")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceVertexAIEvaluationMetric(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "evaluationMetrics",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceVertexAIEvaluationMetricFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

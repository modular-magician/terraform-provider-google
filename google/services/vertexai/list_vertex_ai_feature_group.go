...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewVertexAIFeatureGroupListResource,
	}.Register()
}

var _ list.ListResource = &VertexAIFeatureGroupListResource{}

type VertexAIFeatureGroupListResource struct {
	tpgresource.ListResourceMetadata
}

func NewVertexAIFeatureGroupListResource() list.ListResource {
	listR := &VertexAIFeatureGroupListResource{}
	listR.TypeName = "google_vertex_ai_feature_group"
	listR.SDKv2Resource = ResourceVertexAIFeatureGroup()
...
// VertexAIFeatureGroupListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type VertexAIFeatureGroupListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *VertexAIFeatureGroupListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data VertexAIFeatureGroupListModel
...
func ListVertexAIFeatureGroups(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceVertexAIFeatureGroup().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{VertexAIBasePath}}projects/{{project}}/locations/{{region}}/featureGroups")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceVertexAIFeatureGroup(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "featureGroups",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceVertexAIFeatureGroupFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

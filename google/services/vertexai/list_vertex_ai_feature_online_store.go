...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewVertexAIFeatureOnlineStoreListResource,
	}.Register()
}

var _ list.ListResource = &VertexAIFeatureOnlineStoreListResource{}

type VertexAIFeatureOnlineStoreListResource struct {
	tpgresource.ListResourceMetadata
}

func NewVertexAIFeatureOnlineStoreListResource() list.ListResource {
	listR := &VertexAIFeatureOnlineStoreListResource{}
	listR.TypeName = "google_vertex_ai_feature_online_store"
	listR.SDKv2Resource = ResourceVertexAIFeatureOnlineStore()
...
// VertexAIFeatureOnlineStoreListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type VertexAIFeatureOnlineStoreListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *VertexAIFeatureOnlineStoreListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data VertexAIFeatureOnlineStoreListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListVertexAIFeatureOnlineStores(
			listR.Client,
			region,
			project,
...
func ListVertexAIFeatureOnlineStores(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceVertexAIFeatureOnlineStore().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{VertexAIBasePath}}projects/{{project}}/locations/{{region}}/featureOnlineStores")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceVertexAIFeatureOnlineStore(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "featureOnlineStores",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceVertexAIFeatureOnlineStoreFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewVertexAIIndexListResource,
	}.Register()
}

var _ list.ListResource = &VertexAIIndexListResource{}

type VertexAIIndexListResource struct {
	tpgresource.ListResourceMetadata
}

func NewVertexAIIndexListResource() list.ListResource {
	listR := &VertexAIIndexListResource{}
	listR.TypeName = "google_vertex_ai_index"
	listR.SDKv2Resource = ResourceVertexAIIndex()
...
// VertexAIIndexListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type VertexAIIndexListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *VertexAIIndexListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data VertexAIIndexListModel
...
func ListVertexAIIndexs(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceVertexAIIndex().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{VertexAIBasePath}}projects/{{project}}/locations/{{region}}/indexes")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceVertexAIIndex(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceVertexAIIndexFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewVertexAIPersistentResourceListResource,
	}.Register()
}

var _ list.ListResource = &VertexAIPersistentResourceListResource{}

type VertexAIPersistentResourceListResource struct {
	tpgresource.ListResourceMetadata
}

func NewVertexAIPersistentResourceListResource() list.ListResource {
	listR := &VertexAIPersistentResourceListResource{}
	listR.TypeName = "google_vertex_ai_persistent_resource"
	listR.SDKv2Resource = ResourceVertexAIPersistentResource()
...
// VertexAIPersistentResourceListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type VertexAIPersistentResourceListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *VertexAIPersistentResourceListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data VertexAIPersistentResourceListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListVertexAIPersistentResources(
			listR.Client,
			location,
			project,
...
func ListVertexAIPersistentResources(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceVertexAIPersistentResource().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{VertexAIBasePath}}projects/{{project}}/locations/{{location}}/persistentResources")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceVertexAIPersistentResource(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "persistentResources",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceVertexAIPersistentResourceFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

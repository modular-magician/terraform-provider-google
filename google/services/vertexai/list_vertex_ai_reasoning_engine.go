...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewVertexAIReasoningEngineListResource,
	}.Register()
}

var _ list.ListResource = &VertexAIReasoningEngineListResource{}

type VertexAIReasoningEngineListResource struct {
	tpgresource.ListResourceMetadata
}

func NewVertexAIReasoningEngineListResource() list.ListResource {
	listR := &VertexAIReasoningEngineListResource{}
	listR.TypeName = "google_vertex_ai_reasoning_engine"
	listR.SDKv2Resource = ResourceVertexAIReasoningEngine()
...
// VertexAIReasoningEngineListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type VertexAIReasoningEngineListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *VertexAIReasoningEngineListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data VertexAIReasoningEngineListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListVertexAIReasoningEngines(
			listR.Client,
			region,
			project,
...
func ListVertexAIReasoningEngines(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceVertexAIReasoningEngine().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{VertexAIBasePath}}projects/{{project}}/locations/{{region}}/reasoningEngines")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceVertexAIReasoningEngine(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "reasoningEngines",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceVertexAIReasoningEngineFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

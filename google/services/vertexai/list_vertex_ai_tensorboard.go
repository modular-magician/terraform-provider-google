...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewVertexAITensorboardListResource,
	}.Register()
}

var _ list.ListResource = &VertexAITensorboardListResource{}

type VertexAITensorboardListResource struct {
	tpgresource.ListResourceMetadata
}

func NewVertexAITensorboardListResource() list.ListResource {
	listR := &VertexAITensorboardListResource{}
	listR.TypeName = "google_vertex_ai_tensorboard"
	listR.SDKv2Resource = ResourceVertexAITensorboard()
...
// VertexAITensorboardListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type VertexAITensorboardListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *VertexAITensorboardListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data VertexAITensorboardListModel
...
func ListVertexAITensorboards(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceVertexAITensorboard().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{VertexAIBasePath}}projects/{{project}}/locations/{{region}}/tensorboards")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceVertexAITensorboard(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "tensorboards",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceVertexAITensorboardFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}

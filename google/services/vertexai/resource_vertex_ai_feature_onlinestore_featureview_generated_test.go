...
func testAccCheckVertexAIFeatureOnlinestoreFeatureviewDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			if rs.Type != "google_vertex_ai_feature_onlinestore_featureview" {
				continue
			}
			...
			url, err := tpgresource.ReplaceVarsForTest(config, rs, "{{VertexAIBasePath}}{{feature_online_store}}/featureViews/{{name}}")
			if err != nil {
				return err
			}

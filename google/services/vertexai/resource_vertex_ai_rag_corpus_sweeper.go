...
func listAndActionVertexAIRagCorpus(action sweeper.ResourceAction) error {
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://{{region}}-aiplatform.googleapis.com/v1/projects/{{project}}/locations/{{region}}/ragCorpora", "?")[0]
		...
		resourceList, ok := res["ragCorpuses"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'ragCorpuses'")
		} else {
...
func deleteResourceVertexAIRagCorpus(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://{{region}}-aiplatform.googleapis.com/v1/projects/{{project}}/locations/{{region}}/ragCorpora/{{name}}"

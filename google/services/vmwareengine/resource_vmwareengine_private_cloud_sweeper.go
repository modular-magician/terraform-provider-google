...
func testSweepVmwareenginePrivateCloud(_ string) error {
	return listAndActionVmwareenginePrivateCloud(deleteResourceVmwareenginePrivateCloud)
}

func listAndActionVmwareenginePrivateCloud(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'privateClouds'")
		} else {
...
func deleteResourceVmwareenginePrivateCloud(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {

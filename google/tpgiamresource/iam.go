...
type (
	...
	// ResourceIdentityParserFunc builds the canonical resource identifier (first segment of
	// google_*_iam_member import id) from Terraform import identity when d.Id() is empty.
	ResourceIdentityParserFunc func(identity *schema.IdentityData, config *transport_tpg.Config) (resourceID string, err error)
)
...
type IamSettings struct {
	...
	SchemaVersion      int
	CreateTimeOut      int64
	// ResourceIdentityParser, when non-nil, enables Terraform resource identity for this IAM
	// member resource (identity schema, create/read identity sync, and import when d.Id() is empty).
	ResourceIdentityParser ResourceIdentityParserFunc
}
...
// IamWithResourceIdentity enables Terraform resource identity for this IAM member and sets the
// parser that derives the canonical resource id from import identity when d.Id() is empty.
func IamWithResourceIdentity(parser ResourceIdentityParserFunc) func(*IamSettings) {
	return func(s *IamSettings) {
		s.ResourceIdentityParser = parser
	}
}

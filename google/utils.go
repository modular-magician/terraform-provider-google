...
func retryTimeDuration(retryFunc func() error, duration time.Duration, retryPredicates ...func(e error) bool) error {
	return resource.Retry(duration, func() *resource.RetryError {
		...
		for _, e := range errwrap.GetAllType(err, &googleapi.Error{}) {
			if isRetryableError(e, retryPredicates) {
...
func isRetryableError(err error, retryPredicates []func(e error) bool) bool {
	...
	for _, pred := range retryPredicates {
		if pred(err) {
			log.Printf("[DEBUG] Dismissed an error as retryable per the request of the resource: %s", err)
			return true
		}
	}
